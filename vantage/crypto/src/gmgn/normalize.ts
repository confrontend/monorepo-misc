...
export const numberValueByAliases = (source: Record<string, unknown>, keys: string[]): number | null => {
...
export const normalizeGmgnProfitStat = (payload: unknown): GmgnProfitStat => {
  ...
  return {
    ...
    realizedPnlPercent: numberValueByAliases(source, ['realized_profit_pnl', 'realized_profit_pnl_percent', 'pnl', 'pnl_percent']),
    ...
    averageHoldingSeconds: numberValueByAliases(source, ['avg_holding_period', 'avg_holding_period_seconds', 'avg_hold_time']),
    ...
    risk: {
      noBuyHold: numberValueByAliases(risk, ['no_buy_hold']), noBuyHoldRatio: numberValueByAliases(risk, ['no_buy_hold_ratio']),
      sellPassBuy: numberValueByAliases(risk, ['sell_pass_buy']), sellPassBuyRatio: numberValueByAliases(risk, ['sell_pass_buy_ratio']),
      fastTx: numberValueByAliases(risk, ['fast_tx']), fastTxRatio: numberValueByAliases(risk, ['fast_tx_ratio']),
    },
    pnlDistribution: Object.fromEntries(Object.entries(source).filter(([key, value]) => /pnl|profit/i.test(key) && (typeof value === 'number' || typeof value === 'string'))) as Record<string, number | string>,
  };
};
