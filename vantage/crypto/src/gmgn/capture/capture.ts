...
const extractEvents = (value: unknown): unknown[] => {
  ...
  if (value && typeof value === 'object') {
    ...
    if (
      ...
      return (record.data as Record<string, unknown>).list as unknown[];
...
export const captureGmgnSignals = async (database: DatabaseSync): Promise<GmgnCaptureResult> => {
  ...
  const pollId = startGmgnPoll(database, {
  ...
  try {
    ...
    const { stdout } = await execFileAsync(
      process.execPath,
      [script, 'market', 'signal', '--chain', 'sol', '--raw'],
      {
        cwd: projectRoot,
    ...
    const manifest = Buffer.from(
      JSON.stringify(
        { capturedAt, eventCount: events.length, stored, repeated, validationErrors: errors },
        null,
        2,
      ),
    );
    const archive = zipStored([
      { name: 'gmgn-signal-response.json', data: source },
      { name: 'manifest.json', data: manifest },
    ]);
    const hash = createHash('sha256').update(archive).digest('hex');
    const archivePath = path.join(
      archiveDirectory,
      `gmgn-capture-${capturedAt.replace(/[:.]/g, '-')}-${hash.slice(0, 16)}.zip`,
    );
    ...
    completeGmgnPoll(database, pollId, {
    ...
    return {
      ...
      errors,
      gapDetected: Boolean(
        previousNewest && bounds.oldestTriggerAt && bounds.oldestTriggerAt > previousNewest,
      ),
      archivePath,
      archiveSha256: hash,
      capturedAt,
