...
export const startGmgnPoll = (
  database: DatabaseSync,
  input: { startedAt: string; source: string; chain: string; cliVersion: string },
): number => {
...
export const completeGmgnPoll = (
  ...
  input: {
    ...
    errors: number;
    bounds: PollBounds;
    previousNewest: string | null;
    archivePath: string;
    archiveSha256: string;
  },
): void => {
  const gap =
    input.previousNewest &&
    input.bounds.oldestTriggerAt &&
    input.bounds.oldestTriggerAt > input.previousNewest;
  database
    .prepare(
      `
  ...
  `,
    ...
    .run(
      ...
      input.errors,
      input.bounds.oldestTriggerAt,
      input.bounds.newestTriggerAt,
      input.previousNewest,
      gap ? input.previousNewest : null,
      gap ? input.bounds.oldestTriggerAt : null,
      input.archivePath,
      input.archiveSha256,
      pollId,
    );
