...
type BrowserCapture = {
  ...
  status: number;
  responseBody?: { channel?: unknown; data?: unknown };
};
type BrowserCoverageWindow = {
  startedAt: string;
  endedAt?: string | null;
...
const capturePath = (capture: BrowserCapture): string =>
  typeof capture.requestPath === 'string' ? capture.requestPath : '';
...
const isSignalCapture = (capture: BrowserCapture): boolean =>
  capture.responseBody?.channel === 'token_signal' ||
  capturePath(capture).includes(SIGNAL_REQUEST_PATH);
const isRawEndpointCapture = (capture: BrowserCapture): boolean => {
  const path = capturePath(capture);
  return (
    path.includes(ENDPOINT_PATHS.radar) ||
    path.includes(ENDPOINT_PATHS.walletRank) ||
    path.includes(ENDPOINT_PATHS.smartMoney) ||
    path.includes(ENDPOINT_PATHS.twitter)
  );
};
const isRecognizedCapture = (capture: BrowserCapture): boolean =>
  isSignalCapture(capture) || isRawEndpointCapture(capture);
...
const asCoverageWindow = (value: unknown): BrowserCoverageWindow | null => {
  ...
  if (typeof record.startedAt !== 'string' || typeof record.lastHeartbeatAt !== 'string')
    return null;
  return {
...
export const importGmgnBrowserCapture = (
  ...
  const existing = database
    .prepare(
      `SELECT id, imported_count AS imported, skipped_count AS skipped, error_count AS errors, archive_path AS archivePath, archive_sha256 AS archiveSha256, raw_source AS rawSource, raw_endpoints_json AS rawEndpointsJson FROM gmgn_browser_import_batches WHERE source_sha256 = ?`,
    )
    .get(sourceSha256) as
    | {
        id: number;
        ...
        archiveSha256: string | null;
        rawSource: string;
        rawEndpointsJson: string | null;
      }
    | undefined;
  if (existing) {
    const coverageWindowsImported = Number(
      (
        database
          .prepare(`SELECT COUNT(*) AS count FROM gmgn_browser_coverage_windows WHERE batch_id = ?`)
    ...
    try {
      ...
      if (Array.isArray(capturesArray))
        otherCaptures = capturesArray.filter(
          (capture) => capture && !isRecognizedCapture(capture),
        ).length;
    } catch {
      /* already validated on first import; a parse failure here would be unexpected, not fatal */
    }
    ...
    if (existing.rawEndpointsJson) {
      try {
        rawEndpoints = JSON.parse(existing.rawEndpointsJson) as RawEndpointBreakdown;
      } catch {
        /* fall back to empty */
      }
    }
  ...
  const batch = database
    .prepare(
      `INSERT INTO gmgn_browser_import_batches (source_path, source_sha256, raw_source, status, imported_at) VALUES (?, ?, ?, 'processing', ?)`,
    )
    .run(`ui-upload/${path.basename(sourceName)}`, sourceSha256, redactedFileContent, importedAt);
  ...
  const recordIssues = (issues: string[]) => {
    for (const issue of issues) issueBreakdown[issue] = (issueBreakdown[issue] ?? 0) + 1;
  };
  try {
    const parsed = JSON.parse(redactedFileContent) as Partial<BrowserExport>;
    if (
      parsed.formatVersion !== 1 ||
      parsed.source !== 'gmgn-browser-extension' ||
      !Array.isArray(parsed.captures)
    )
      throw new Error(
        'Browser capture export must have source gmgn-browser-extension, formatVersion 1, and a captures array.',
      );
    for (const capture of parsed.captures) {
      if (!capture || typeof capture.capturedAt !== 'string') {
        errors += 1;
        continue;
      }
      ...
      if (isRawEndpointCapture(capture)) {
        try {
          ...
          if (requestPath.includes(ENDPOINT_PATHS.radar)) {
            const result = storeRadarSnapshot(database, {
              chain: queryParam(capture, 'chain'),
              period: queryParam(capture, 'period'),
              category: queryParam(capture, 'type'),
              capturedAt,
              rawPayload: responseBody,
            });
            rawEndpoints.radar.imported += result.inserted;
            rawEndpoints.radar.skipped += result.skipped;
            ...
            const result = storeWalletRankSnapshot(database, {
              window: windowFromOrderby(orderby),
              orderby,
              capturedAt,
              rawPayload: responseBody,
              requestPath,
              requestQuery: capture.requestQuery ?? {},
            });
            rawEndpoints.walletRank.imported += result.inserted;
            rawEndpoints.walletRank.skipped += result.skipped;
            ...
            const result = storeSmartMoneyWalletStats(database, {
              chain: match[1],
              walletAddress: decodeURIComponent(match[2]),
              capturedAt,
              rawPayload: responseBody,
            });
            rawEndpoints.smartMoney.imported += result.inserted;
            rawEndpoints.smartMoney.skipped += result.skipped;
          } else {
            const result = storeTwitterMessages(database, {
              hasToken: queryParam(capture, 'has_token'),
              capturedAt,
              rawPayload: responseBody,
            });
            rawEndpoints.twitter.imported += result.inserted;
            rawEndpoints.twitter.skipped += result.skipped;
    ...
    const coverageWindows = Array.isArray(parsed.coverageWindows)
      ? parsed.coverageWindows
          .map(asCoverageWindow)
          .filter((window): window is BrowserCoverageWindow => window !== null)
      : [];
    ...
    const manifest = JSON.stringify(
      {
        ...
        otherCaptures,
        rawEndpoints,
        coverageWindows: coverageWindows.length,
    ...
    const archive = zipStored([
      { name: path.basename(sourceName), data: Buffer.from(redactedFileContent, 'utf8') },
      { name: 'manifest.json', data: Buffer.from(manifest, 'utf8') },
    ]);
    const archiveSha256 = createHash('sha256').update(archive).digest('hex');
    const archivePath = path.join(
      archiveDirectory,
      `gmgn-browser-batch-${batchId}-${sourceSha256.slice(0, 16)}.zip`,
    );
    if (!existsSync(archivePath)) writeFileSync(archivePath, archive, { flag: 'wx' });
    const completedAt = new Date().toISOString();
    database
      .prepare(
        `UPDATE gmgn_browser_import_batches SET status = 'completed', imported_count = ?, skipped_count = ?, error_count = ?, completed_at = ?, archive_path = ?, archive_sha256 = ?, archived_at = ?, raw_endpoints_json = ? WHERE id = ?`,
      )
      .run(
        imported,
        skipped,
    ...
    return {
      ...
      issueBreakdown,
      otherCaptures,
      coverageWindowsImported: coverageWindows.length,
      duplicateFile: false,
      archivePath,
      archiveSha256,
      rawEndpoints,
    ...
    database
      .prepare(
        `UPDATE gmgn_browser_import_batches SET status = 'failed', error = ?, error_count = ?, completed_at = ? WHERE id = ?`,
      )
      .run(message, errors + 1, new Date().toISOString(), batchId);
    throw error;
  }
};
