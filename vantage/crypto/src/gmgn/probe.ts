...
export const probeGmgn = async (): Promise<GmgnProbeResult> => {
  ...
  if (!secret) {
    return { ok: false, status: 'not-configured', message: 'API key file is empty or missing.', observedItems: null, checkedAt };
  }
  const localScript = path.join(projectRoot, 'node_modules', 'gmgn-cli', 'dist', 'index.js');
  const useLocal = existsSync(localScript);
  const command = useLocal ? process.execPath : (process.platform === 'win32' ? 'gmgn-cli.cmd' : 'gmgn-cli');
  const args = useLocal
  ...
  try {
    ...
    writeDiagnostics({
      ...
      stderr,
      stdoutBytes: error && typeof error === 'object' && 'stdout' in error && typeof (error as { stdout?: unknown }).stdout === 'string'
        ? Buffer.byteLength((error as { stdout: string }).stdout, 'utf8')
        : null,
      apiKeyPresent: true,
    }, secret);
    if (code === 'ENOENT' || details.includes('not recognized') || details.includes('cannot find the path')) {
      return { ok: false, status: 'not-installed', message: 'The project-local gmgn-cli is unavailable. Run npm install, then check again.', observedItems: null, checkedAt };
    }
    if (details.includes('401') || details.includes('unauthorized') || details.includes('invalid api')) {
      return { ok: false, status: 'error', message: 'GMGN rejected the credential (401/unauthorized). Recheck the API key in the local file.', observedItems: null, checkedAt };
    }
    if (details.includes('403') || details.includes('forbidden')) {
      return { ok: false, status: 'error', message: 'GMGN denied this read-only request (403/forbidden). Check API-key reading permission and IP restrictions.', observedItems: null, checkedAt };
    }
    if (details.includes('429') || details.includes('rate limit')) {
      return { ok: false, status: 'error', message: 'GMGN rate-limited the request (429). Wait for the documented reset time before retrying.', observedItems: null, checkedAt };
    }
    return { ok: false, status: 'error', message: 'GMGN read-only request failed. Safe diagnostics saved to .data/gmgn-probe-diagnostics.log.', observedItems: null, checkedAt };
  }
};
