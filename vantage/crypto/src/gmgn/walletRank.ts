...
export const storeWalletRankSnapshot = (
  ...
  const result = insertByHash(
    database,
    `INSERT OR IGNORE INTO gmgn_wallet_rank_snapshots (window, orderby, captured_at, raw_payload, source_sha256) VALUES (?, ?, ?, ?, ?)`,
    [window, orderby, input.capturedAt, raw, sha256],
    sha256,
  );
  ...
  const query =
    input.requestQuery &&
    typeof input.requestQuery === 'object' &&
    !Array.isArray(input.requestQuery)
