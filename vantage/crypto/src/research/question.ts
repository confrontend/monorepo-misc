...
export const RESEARCH_QUESTION = {
  ...
  primary: 'Among tokens in the historical Solana cohort, does receiving a GMGN signal before the token reaches 10x its first-trade market-cap reference increase the probability of becoming an early winner compared with otherwise eligible cohort tokens without that signal?',
  earlyWinnerDefinition: 'A token is an early winner when its maximum observed market cap reaches at least 5x its first-trade market-cap reference within 7 calendar days of first trade.',
  exposureDefinition: 'The exposure is the first observed GMGN signal per token and signal type, with observed_at, signal_type, market_cap, triggering_wallet, and raw_wallet_labels preserved.',
  comparisonDefinition: 'Compare each qualifying signaled token with cohort tokens eligible in the same calendar period and initial market-cap bucket that have no qualifying GMGN signal before the outcome window closes.',
