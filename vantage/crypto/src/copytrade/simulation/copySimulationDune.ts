...
export const sqlFor = (targets: CopySimulationTarget[], searchWindowMinutes = PRECISE_SEARCH_WINDOW_MINUTES): string => {
  ...
  // tokens, no matter how old — the JOIN's own time predicate below can't be pushed down into the
  // table scan because it compares block_time against a per-row `delayed_at` value, not a
  // constant, so Dune had no way to prune on its own. This WHERE-clause bound is a pure
  // optimization: every row it excludes was already unreachable by the JOIN's own predicate, so
  // it can only reduce bytes scanned (and Dune credits spent), never change which trade matches.
  const windowMs = searchWindowMinutes * 60_000;
  const bounds = targets.map((t) => {
    const at = Date.parse(t.delayedTargetAtIso);
    return t.direction === 'before' ? [at - windowMs, at] : [at, at + windowMs];
  });
  const scanFrom = new Date(Math.min(...bounds.map(([lo]) => lo))).toISOString();
  const scanTo = new Date(Math.max(...bounds.map(([, hi]) => hi))).toISOString();
  return `WITH targets (trade_id, token_address, delayed_at, direction) AS (
    ...
    WHERE token_bought_mint_address IN (SELECT token_address FROM targets) AND amount_usd > 0 AND token_bought_amount > 0
      AND block_time >= from_iso8601_timestamp('${scanFrom}') AND block_time <= from_iso8601_timestamp('${scanTo}')
    UNION ALL
