...
export const allowedApplicationExecutions = (
  reportedLimit: number | null,
  margin = configuredSafetyMargin(),
): number => {
  const limit =
    reportedLimit && Number.isFinite(reportedLimit) && reportedLimit > 0
      ? Math.floor(reportedLimit)
      : DEFAULT_DUNE_ACCOUNT_LIMIT;
  return Math.max(1, limit - Math.max(0, Math.floor(margin)));
};

export const activeExecutionCount = (states: readonly string[]): number =>
  states.filter((state) => state === 'submitted' || state === 'running' || state === 'timed_out')
    .length;

export const canSubmitExecution = (
  active: number,
  reportedLimit: number | null,
  margin = configuredSafetyMargin(),
): boolean => active < allowedApplicationExecutions(reportedLimit, margin);
...
export const waitForDuneCapacity = async (
  ...
  options: {
    ...
    onCapacity?: (info: { active: number; limit: number; margin: number }) => void;
    pollMs?: number;
  } = {},
): Promise<void> => {
