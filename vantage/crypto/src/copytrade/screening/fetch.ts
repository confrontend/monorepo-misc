...
export type WalletCoverageRow = { walletAddress: string; coverageComplete: number; requestedPeriodDays: number | null; truncated: number; stopReason: string | null; resumeCursor: string | null };
...
export const readWalletCoverageRows = (database: DatabaseSync, chain: string, walletAddresses: string[]): Map<string, WalletCoverageRow> => {
...
const countStoredTradesSince = (
  database: DatabaseSync,
  options: { chain: string; cutoffSeconds: number } & ({ walletAddress: string } | { walletAddresses: string[] }),
): number => {
...
export const readFetchRunState = (database: DatabaseSync): FetchRunState => {
  ...
  const currentWalletStoredTrades = row.currentWalletAddress && row.currentWalletExpectedTrades !== null
    ? countStoredTradesSince(database, { chain: 'sol', walletAddress: row.currentWalletAddress, cutoffSeconds: Math.floor(Date.now() / 1000) - (row.periodDays ?? 30) * 86_400 })
    : null;
...
export const runCopyTradeFetch = async (
  ...
  try {
    ...
    database.prepare(
      ...
      countStoredTradesSince(database, { chain, walletAddresses: wallets.map((wallet) => wallet.walletAddress), cutoffSeconds }),
      runId,
    );
    ...
    for (const wallet of wallets) {
      ...
      const currentWalletInitialTrades = countStoredTradesSince(database, { chain, walletAddress: wallet.walletAddress, cutoffSeconds });
      database.prepare(
        `UPDATE copytrade_fetch_runs
      ...
      const previousCoverage = readWalletCoverageRows(database, chain, [wallet.walletAddress]).get(wallet.walletAddress);
