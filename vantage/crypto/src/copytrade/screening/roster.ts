...
export const rankItemToWallet = (item: RankItem, index: number, chain: string): RosterWallet | null => {
  ...
  return {
    ...
    riskFlags: extractRiskFlags(itemGmgnTags(item)),
    gmgnTags: itemGmgnTags(item),
  };
};
  ...
  const insert = database.prepare(
    `INSERT OR IGNORE INTO copytrade_wallets
       (wallet_address, chain, name, icon_url, source_snapshot_id, rank_position,
        reported_pnl_30d, reported_winrate_30d, risk_flags, gmgn_tags, added_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  );
  const updateTags = database.prepare(
    'UPDATE copytrade_wallets SET risk_flags = ?, gmgn_tags = ? WHERE wallet_address = ? AND chain = ? AND source_snapshot_id = ?',
  );
