 ...
 */
export type ThirtyDayDecision = 'Tested candidate' | 'Watch' | 'Needs data' | 'Historical / stale' | 'Not copyable' | 'Historical screen failed';
...
export const decideThirtyDayVerdict = (evidence: ThirtyDayDecisionEvidence): ThirtyDayDecision => {
  ...
  if (evidence.delayedCopySurvived && evidence.gmgn30dPositive && evidence.delayedCopyMedianPositive && evidence.consistencyDataMissing) return 'Needs data';
  if (evidence.delayedCopySurvived && evidence.gmgn30dPositive && evidence.consistentlyProfitable) return 'Tested candidate';
...
export const explainThirtyDayDecision = (evidence: ThirtyDayDecisionDetails): string[] => {
  ...
  if (verdict === 'Watch') {
    ...
    if (!evidence.delayedCopySurvived) reasons.push('The delayed-copy portfolio was not positive after delay, fees, slippage, and gas.');
    if (!evidence.delayedCopyMedianPositive) reasons.push('The median copied trade was not positive.');
    if (evidence.historicalConsistency === 'inconsistent') reasons.push('The earlier and recent parts of the 30-day history were not both positive.');
    if (evidence.historicalConsistency === 'insufficient') reasons.push('Historical consistency could not be established from enough data.');
    if (evidence.noLosingWeek === false) {
      const measured = evidence.measuredWeeks ?? 0;
      const minimum = evidence.minimumMeasuredWeeks ?? 0;
      reasons.push(measured < minimum ? `Only ${measured} measured week${measured === 1 ? '' : 's'} were available; ${minimum} are required.` : 'At least one measured week was not positive.');
    }
    return reasons.length > 0 ? reasons : ['It passed the base data gates but did not pass the final consistency gates.'];
  }
  if (verdict === 'Needs data') {
    const reasons: string[] = [];
    if (evidence.historyIncomplete) reasons.push('The requested GMGN history window is incomplete.');
    ...
    if (evidence.consistencyDataMissing) reasons.push('Historical consistency has not finished loading.');
    return reasons.length > 0 ? reasons : ['Required evidence is incomplete.'];
  }
  if (verdict === 'Historical / stale') return ['GMGN or Dune evidence is older than the current freshness window.'];
  if (verdict === 'Not copyable') return ['The typical holding time is shorter than the configured copy delay.'];
