...
export const downloadRosterIcons = async (
  database: DatabaseSync,
): Promise<{ attempted: number; saved: number; failed: number }> => {
  mkdirSync(iconDirectory, { recursive: true });
  const rows = database
    .prepare(
      `SELECT DISTINCT icon_url AS iconUrl FROM copytrade_wallets WHERE icon_url IS NOT NULL AND trim(icon_url) <> ''`,
    )
    .all() as unknown as Array<{ iconUrl: string }>;
  let saved = 0;
  let failed = 0;
