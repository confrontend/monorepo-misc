...
import { DORMANT_AFTER_DAYS, MAX_CONCENTRATION_PERCENT, MIN_MEDIAN_HOLD_SECONDS } from './copyCandidates.js';
import {
  computeCopySimulationReport, DEFAULT_COPIER_DELAY_SECONDS, MIN_COPY_SIMULATION_SAMPLE,
  TAIL_THRESHOLD_PERCENT, type CopySimulationTradeResult, type CopySimulationWalletReport,
} from './copySimulation.js';
import { splitByDate, suggestSplitDate, type DatedObservation } from '../stats/holdout.js';
...
export const COPYABILITY_MIN_DELAY_MULTIPLE = MIN_MEDIAN_HOLD_SECONDS / DEFAULT_COPIER_DELAY_SECONDS;
...
type ScrutinyTrade = { sourceId: number; timestamp: number; returnRatio: number; tokenAddress: string };

type RawTradeRow = {
  id: number; walletAddress: string; observedTimestamp: number; eventType: string;
  tokenAddress: string; costUsd: string | null; buyCostUsd: string | null;
};
...
const readWalletTrades = (
  database: DatabaseSync, walletAddress: string, chain: string, cutoffSeconds: number | null,
): { completed: ScrutinyTrade[]; buyCount: number; sellCount: number; excludedNoCostBasis: number } => {
  ...
  for (const row of rows) {
    if (row.eventType === 'buy') { buyCount += 1; continue; }
    if (row.eventType !== 'sell') continue;
    sellCount += 1;
    const proceeds = parseAmount(row.costUsd);
    const costBasis = parseAmount(row.buyCostUsd);
    if (proceeds === null || costBasis === null || costBasis <= 0) { excludedNoCostBasis += 1; continue; }
    completed.push({
      sourceId: row.id, timestamp: row.observedTimestamp,
      returnRatio: (proceeds - costBasis) / costBasis, tokenAddress: row.tokenAddress,
    });
  }
...
type CoverageMetrics = {
  inWindowMatched: number; inWindowTotal: number; inWindowPercent: number | null;
  fullHistoryMatched: number; fullHistoryTotal: number; fullHistoryPercent: number | null;
};
...
type ConcentrationMetrics = {
  bestTokenSymbol: string | null; bestTokenSharePercent: number | null;
  medianWithToken: number | null; medianWithoutToken: number | null; tradesWithoutToken: number;
};
type RepeatEntryMetrics = {
  repeatEntryMedianReturnPercent: number | null; repeatEntryN: number;
  singleEntryMedianReturnPercent: number | null; singleEntryN: number;
};
type BuySellMetrics = { buyCount: number; sellCount: number; buySharePercent: number | null };
type DivergenceMetrics = { medianReturnPercent: number | null; averageReturnPercent: number | null; diverges: boolean };
type TailFragilityMetrics = {
  top3SharePercent: number | null; tradesAboveThreshold: number; thresholdPercent: number; simulatedTrades: number;
};
type CopyabilityMetrics = {
  medianHoldSeconds: number | null; copierDelaySeconds: number; delayMultiple: number | null; minRequiredMultiple: number;
};
type StabilityMetrics = {
  splitDate: string | null; earlyMedianReturnPercent: number | null; earlyN: number;
  lateMedianReturnPercent: number | null; lateN: number;
};
...
const bigWinShare = (trades: CopySimulationTradeResult[]): { sharePercent: number | null; n: number } => {
  const withOwnReturn = trades.filter((t) => t.walletReturnPercent !== null);
  if (withOwnReturn.length === 0) return { sharePercent: null, n: 0 };
  const bigWins = withOwnReturn.filter((t) => (t.walletReturnPercent as number) > TAIL_THRESHOLD_PERCENT).length;
...
export type CoverageGapAssessment = {
  matchedBigWinPercent: number | null; matchedN: number;
  unmatchedBigWinPercent: number | null; unmatchedN: number;
  gapPercentagePoints: number | null; direction: CoverageGapDirection;
...
export const assessCoverageGap = (trades: CopySimulationTradeResult[]): CoverageGapAssessment => {
  ...
  const gapPercentagePoints = matched.sharePercent !== null && unmatched.sharePercent !== null
    ? round(unmatched.sharePercent - matched.sharePercent, 1) : null;
  const upsideBiasWeightedPercentagePoints = gapPercentagePoints === null || matched.n + unmatched.n === 0
    ? null : round(gapPercentagePoints * unmatched.n / (matched.n + unmatched.n), 1);
  const direction: CoverageGapDirection = matched.n === 0 || unmatched.n === 0 ? 'no_gap'
    : gapPercentagePoints === null ? 'unclear'
    : gapPercentagePoints >= COVERAGE_BIAS_GAP_THRESHOLD_PERCENTAGE_POINTS ? 'conservative'
    : gapPercentagePoints <= -COVERAGE_BIAS_GAP_THRESHOLD_PERCENTAGE_POINTS ? 'optimistic'
    : 'unclear';
  ...
  const losers = (list: CopySimulationTradeResult[]): { rate: number | null; n: number } => {
    ...
    return { rate: pct(withOwnReturn.filter((t) => (t.walletReturnPercent as number) < 0).length, withOwnReturn.length), n: withOwnReturn.length };
  };
  ...
  const lossRateUnderstatedPercentagePoints = comparable
    ? round((overall.rate as number) - (shown.rate as number), 1) : null;
  const hiddenLossRisk: HiddenLossRisk = !comparable || lossRateUnderstatedPercentagePoints === null ? 'unknown'
    : lossRateUnderstatedPercentagePoints > HIDDEN_LOSS_HIGH_PERCENTAGE_POINTS ? 'high'
    : lossRateUnderstatedPercentagePoints > HIDDEN_LOSS_MODERATE_PERCENTAGE_POINTS ? 'moderate'
    : 'negligible';
  ...
  const hiddenUpsideBias: HiddenLossRisk = unmatched.n === 0 ? 'negligible'
    : upsideBiasWeightedPercentagePoints === null || matched.n === 0 ? 'unknown'
    : upsideBiasWeightedPercentagePoints > HIDDEN_LOSS_HIGH_PERCENTAGE_POINTS ? 'high'
    : upsideBiasWeightedPercentagePoints > HIDDEN_LOSS_MODERATE_PERCENTAGE_POINTS ? 'moderate'
    : 'negligible';

  return {
    matchedBigWinPercent: matched.sharePercent, matchedN: matched.n,
    unmatchedBigWinPercent: unmatched.sharePercent, unmatchedN: unmatched.n,
    gapPercentagePoints, upsideBiasWeightedPercentagePoints, direction,
    shownLossRatePercent: shown.rate, trueLossRatePercent: overall.rate,
    lossRateUnderstatedPercentagePoints, hiddenLossRisk, hiddenUpsideBias,
  };
};
...
export const computeCandidateScrutiny = (
  ...
  const dormancy: ScrutinyCheck<DormancyMetrics> = {
    key: 'dormancy', label: 'Dormancy', n: row.lastTradeAt === null ? 0 : 1,
    verdict: row.daysSinceLastTrade === null ? 'insufficient'
      : row.daysSinceLastTrade > DORMANT_AFTER_DAYS ? 'fail' : 'pass',
    detail: row.daysSinceLastTrade === null ? 'No completed trades on record.'
  ...
  const coverage: ScrutinyCheck<CoverageMetrics> = {
    key: 'coverage', label: 'Dune coverage', n: simInWindow.roundTripsConsidered,
    verdict: simInWindow.roundTripsConsidered === 0 || noInWindowCoverage ? 'insufficient'
      : (inWindowPercent ?? 0) >= COVERAGE_FAIL_THRESHOLD_PERCENT ? 'pass' : 'fail',
    detail: simInWindow.roundTripsConsidered === 0 ? 'No round trips with a resolvable buy exist for this wallet in the scoring window.'
      : noInWindowCoverage ? 'No Dune match exists yet within the scoring window.'
    ...
    metrics: {
      inWindowMatched: simInWindow.copiedTrades, inWindowTotal: simInWindow.roundTripsConsidered, inWindowPercent,
      fullHistoryMatched: simFullHistory.copiedTrades, fullHistoryTotal: simFullHistory.roundTripsConsidered, fullHistoryPercent,
    },
  };

  // --- 3. Coverage bias direction -------------------------------------------------------------
  const gapAssessment = assessCoverageGap(simInWindow.trades);
  const matchedShare = { sharePercent: gapAssessment.matchedBigWinPercent, n: gapAssessment.matchedN };
  const unmatchedShare = { sharePercent: gapAssessment.unmatchedBigWinPercent, n: gapAssessment.unmatchedN };
  const gap = gapAssessment.gapPercentagePoints;
  const biasInsufficient = noInWindowCoverage || matchedShare.n === 0 || unmatchedShare.n === 0;
  const direction: CoverageBiasMetrics['direction'] = biasInsufficient ? 'no_gap' : gapAssessment.direction;
  const coverageBias: ScrutinyCheck<CoverageBiasMetrics> = {
    key: 'coverageBias', label: 'Coverage bias direction', n: matchedShare.n + unmatchedShare.n,
    verdict: biasInsufficient ? 'insufficient' : direction === 'unclear' ? 'pass' : 'fail',
    detail: biasInsufficient ? 'Not enough verified and unverified trades to compare.'
    ...
    metrics: {
      matchedBigWinPercent: matchedShare.sharePercent, matchedN: matchedShare.n,
      unmatchedBigWinPercent: unmatchedShare.sharePercent, unmatchedN: unmatchedShare.n,
      gapPercentagePoints: gap, direction,
    },
  };
  ...
  const concentration: ScrutinyCheck<ConcentrationMetrics> = {
    key: 'concentration', label: 'Token concentration', n: row.trades,
    verdict: concentrationShare === null ? 'insufficient'
      : concentrationShare > MAX_CONCENTRATION_PERCENT ? 'fail' : 'pass',
    detail: concentrationShare === null ? 'No positive profit to attribute to any token.'
  ...
  const scoped = readWalletTrades(database, walletAddress, chain, cutoffSecondsFor(context.scopePeriodDays, now));
  ...
  for (const trades of byToken.values()) (trades.length > 1 ? repeatEntryTrades : singleEntryTrades).push(...trades);
  const repeatMedian = medianOf(repeatEntryTrades);
  const singleMedian = medianOf(singleEntryTrades);
  const repeatInsufficient = repeatEntryTrades.length < MIN_GROUP_SAMPLE || singleEntryTrades.length < MIN_GROUP_SAMPLE;
  const repeatEntry: ScrutinyCheck<RepeatEntryMetrics> = {
    key: 'repeatEntry', label: 'Repeat-entry dependence', n: repeatEntryTrades.length + singleEntryTrades.length,
    verdict: repeatInsufficient ? 'insufficient'
      : (repeatMedian !== null && repeatMedian > 0 && singleMedian !== null && singleMedian <= 0) ? 'fail' : 'pass',
    ...
    metrics: {
      repeatEntryMedianReturnPercent: repeatMedian, repeatEntryN: repeatEntryTrades.length,
      singleEntryMedianReturnPercent: singleMedian, singleEntryN: singleEntryTrades.length,
    },
  };
  ...
  const buySellComposition: ScrutinyCheck<BuySellMetrics> = {
    key: 'buySellComposition', label: 'Buy/sell composition', n: totalLegs,
    verdict: totalLegs < MIN_COPY_SIMULATION_SAMPLE ? 'insufficient'
      : buySharePercent !== null && buySharePercent >= BUY_SHARE_MIN_PERCENT && buySharePercent <= BUY_SHARE_MAX_PERCENT ? 'pass' : 'fail',
    detail: totalLegs < MIN_COPY_SIMULATION_SAMPLE ? `Only ${totalLegs} scored legs in the scoring window.`
  ...
  const diverges = row.medianReturnPercent !== null && row.averageReturnPercent !== null
    && Math.sign(row.medianReturnPercent) !== 0 && Math.sign(row.averageReturnPercent) !== 0
  ...
  const medianMeanDivergence: ScrutinyCheck<DivergenceMetrics> = {
    key: 'medianMeanDivergence', label: 'Median vs mean divergence', n: row.trades,
    verdict: row.medianReturnPercent === null || row.averageReturnPercent === null ? 'insufficient'
      : diverges ? 'fail' : 'pass',
    detail: row.medianReturnPercent === null || row.averageReturnPercent === null ? 'Median or mean could not be computed.'
      : diverges
    ...
    metrics: { medianReturnPercent: row.medianReturnPercent, averageReturnPercent: row.averageReturnPercent, diverges },
  };

  // --- 8. Tail fragility -----------------------------------------------------------------------
  const simulatedFull = simInWindow.trades.filter((t) => t.status === 'simulated' && t.simulatedReturnPercent !== null);
  const positiveContributions = simulatedFull.map((t) => t.simulatedReturnPercent as number).filter((v) => v > 0);
  const totalPositive = positiveContributions.reduce((sum, v) => sum + v, 0);
  const top3Sum = [...positiveContributions].sort((a, b) => b - a).slice(0, 3).reduce((sum, v) => sum + v, 0);
  ...
  const tailFragility: ScrutinyCheck<TailFragilityMetrics> = {
    key: 'tailFragility', label: 'Tail fragility', n: simulatedFull.length,
    verdict: totalPositive <= 0 || simulatedFull.length === 0 ? 'insufficient'
      : top3SharePercent !== null && top3SharePercent > TAIL_FRAGILITY_SHARE_THRESHOLD_PERCENT ? 'fail' : 'pass',
    detail: totalPositive <= 0 || simulatedFull.length === 0 ? 'No positive simulated return to attribute to any trades in the scoring window.'
    ...
    metrics: {
      top3SharePercent, tradesAboveThreshold: simInWindow.tradesAbove100Percent,
      thresholdPercent: TAIL_THRESHOLD_PERCENT, simulatedTrades: simulatedFull.length,
    },
  };

  // --- 9. Copyability --------------------------------------------------------------------------
  const holdSeconds = row.riskEvidence.medianHoldSeconds;
  const delayMultiple = holdSeconds === null ? null : round(holdSeconds / DEFAULT_COPIER_DELAY_SECONDS, 2);
  const copyability: ScrutinyCheck<CopyabilityMetrics> = {
    key: 'copyability', label: 'Copyability', n: row.trades,
    verdict: holdSeconds === null ? 'insufficient'
      : delayMultiple !== null && delayMultiple >= COPYABILITY_MIN_DELAY_MULTIPLE ? 'pass' : 'fail',
    detail: holdSeconds === null ? 'No hold time could be computed.'
    ...
    metrics: { medianHoldSeconds: holdSeconds, copierDelaySeconds: DEFAULT_COPIER_DELAY_SECONDS, delayMultiple, minRequiredMultiple: round(COPYABILITY_MIN_DELAY_MULTIPLE, 2) },
  };
  ...
  const observations: (DatedObservation & { returnRatio: number })[] = scoped.completed.map((t) => ({
    observedAt: new Date(t.timestamp * 1000).toISOString(), returnRatio: t.returnRatio,
  }));
  const splitDate = suggestSplitDate(observations);
  let split: ReturnType<typeof splitByDate<DatedObservation & { returnRatio: number }>> | null = null;
  ...
  const stabilityInsufficient = !split || split.discovery.length < MIN_GROUP_SAMPLE || split.test.length < MIN_GROUP_SAMPLE;
  ...
  const outOfSampleStability: ScrutinyCheck<StabilityMetrics> = {
    key: 'outOfSampleStability', label: 'Out-of-sample stability', n: observations.length,
    verdict: stabilityInsufficient ? 'insufficient'
      : (earlyMedianPct !== null && lateMedianPct !== null && earlyMedianPct > 0 && lateMedianPct <= 0) ? 'fail' : 'pass',
    detail: stabilityInsufficient
      ? (split ? `Too few trades on one side of the split (early n=${split.discovery.length}, late n=${split.test.length}).` : 'Fewer than 2 distinct trading dates — no chronological split is meaningful.')
    ...
    metrics: { splitDate, earlyMedianReturnPercent: earlyMedianPct, earlyN: split?.discovery.length ?? 0, lateMedianReturnPercent: lateMedianPct, lateN: split?.test.length ?? 0 },
  };

  // --- 11. Selection context ---------------------------------------------------------------------
  const selectionContext = {
    candidateCount: context.candidateCount, screenedCount: context.screenedCount,
  ...
  return {
    walletAddress, name: row.name, computedAt: now.toISOString(), selectionContext,
    checks: {
      dormancy, coverage, coverageBias, concentration, repeatEntry, buySellComposition,
      medianMeanDivergence, tailFragility, copyability, outOfSampleStability,
    },
  };
};
...
export const computeCandidateScrutinyBatch = (
  ...
  for (const walletAddress of capped) {
    ...
    reports.push(computeCandidateScrutiny(database, walletAddress, {
      ...
      candidateCount: context.candidateCount, screenedCount: context.screenedCount,
      chain: context.chain, scopePeriodDays: context.scopePeriodDays, now: context.now,
    }));
  }
  return reports;
};
