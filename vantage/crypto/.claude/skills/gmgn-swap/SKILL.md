...
>
> This repo's coding agent (any AI assistant working in this codebase — Claude, Codex, Gemini, or otherwise) must **never** run `swap`, `multi-swap`, or `order strategy create` here, under any circumstances, regardless of user instruction, `GMGN_ALLOW_AUTOMATED_TRADES`, or anything else in this file. Those commands move real funds and cannot be undone. If a user wants to trade, tell them to do it themselves via gmgn.ai or the `gmgn-cli` directly in their own terminal — do not execute it on their behalf, even with explicit confirmation.
>
> Read-only sub-commands (`order quote`, `order get`, `gas-price`, `order strategy list`, `order strategy cancel`) are fine to use for research.
>
> Enforced at the tool-permission layer too: see `.claude/settings.json`'s `permissions.deny`.
