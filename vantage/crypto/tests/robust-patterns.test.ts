...
import { computeRobustPatternReport } from '../src/db/robustPatterns.js';

const seedSignal = (database: ReturnType<typeof openDatabase>, id: string, signalType: number, token: string, observedAt: string): number =>
  storeGmgnSignal(database, { id, token_address: token, signal_type: signalType, observed_at: observedAt, market_cap: 1000 }, { source: 'gmgn-cli', chain: 'sol', capturedAt: new Date(observedAt) }).id;

type FixtureRow = { signal_id: number; checkpoint: string; price_usd: number | null; matched_trade_at?: string | null; target_at?: string };
...
const insertCompletedRun = (database: ReturnType<typeof openDatabase>, signalIds: number[], rows: FixtureRow[]) => {
  const resultRows = rows.map((row) => ({
    ...
    matched_trade_at: row.matched_trade_at !== undefined ? row.matched_trade_at : `2026-03-01T00:00:00Z#${row.checkpoint}#${row.signal_id}`,
  }));
...
test('robust report excludes any group below the reliable-sample floor entirely', () => {
  ...
  try {
    const ids = Array.from({ length: 5 }, (_, i) => seedSignal(database, `s${i}`, 7, `Token${i}`, '2026-03-01T00:00:00Z'));
    insertCompletedRun(database, ids, ids.flatMap((id) => [
    ...
    assert.equal(report.groups.length, 0, 'only 5 fresh points exist, below the 10-sample floor — must not appear at all');
...
test('robust report computes a bootstrap CI, a sign-test p-value, and Holm correction for a group that clears the floor', () => {
  ...
  try {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      rows.push({ signal_id: id, checkpoint: '+1h', price_usd: 1.1 + i * 0.01, target_at: observedAt }); // consistently up
    }
...
test('robust report Holm-corrects across every group in the same run, not per-horizon in isolation', () => {
  ...
  try {
    ...
    const strongIds: number[] = []; const strongRows: FixtureRow[] = [];
    for (let i = 0; i < 20; i++) {
    ...
    const borderlineIds: number[] = []; const borderlineRows: FixtureRow[] = [];
    ...
    for (let i = 0; i < 20; i++) {
      ...
      borderlineRows.push({ signal_id: id, checkpoint: 'signal', price_usd: 1, target_at: observedAt });
      borderlineRows.push({ signal_id: id, checkpoint: '+1h', price_usd: i < 11 ? 1.05 : 0.95, target_at: observedAt });
    }
    ...
    assert.ok(strong.holmAdjustedPValue! < borderline.holmAdjustedPValue!, 'the overwhelming group should end up more significant than the coin-flip one after correction');
    assert.equal(borderline.holmRejected, false, 'an 11-vs-9 split must not survive as statistically significant');
  } finally { database.close(); }
});
...
test('robust report holdout: a pattern that holds in both halves reports both_positive', () => {
  const database = openDatabase(':memory:');
  try {
    const ids: number[] = []; const rows: FixtureRow[] = [];
    ...
    insertCompletedRun(database, ids, rows);
    const report = computeRobustPatternReport(database, new Date('2026-03-20T00:00:00Z'), { bootstrapIterations: 300, holdoutTestFraction: 0.3 });
    ...
    assert.ok(group.holdout.discoveryN >= 10 && group.holdout.testN >= 10, 'both sides must clear the reliability floor given 50 points spread over 10 days');
  } finally { database.close(); }
});
...
test('robust report excludes a group that lacks the canonical capture-date reliability gate', () => {
  ...
  try {
    ...
    insertCompletedRun(database, ids, rows);
    const report = computeRobustPatternReport(database, new Date('2026-03-10T00:00:00Z'), { bootstrapIterations: 300 });
...
test('robust report keeps a missing first observation from being replaced by a later repeat', () => {
  ...
  try {
    ...
    const first = seedSignal(database, 'trap-first-missing', 7, 'TrapToken', '2026-03-01T00:30:00Z');
    const repeat = seedSignal(database, 'trap-later-fresh', 7, 'TrapToken', '2026-03-09T00:00:00Z');
    ids.push(first, repeat);
    rows.push({ signal_id: first, checkpoint: 'signal', price_usd: 1, target_at: '2026-03-01T00:30:00Z' });
    rows.push({ signal_id: first, checkpoint: '+1h', price_usd: null, target_at: '2026-03-01T00:30:00Z' });
    rows.push({ signal_id: repeat, checkpoint: 'signal', price_usd: 1, target_at: '2026-03-09T00:00:00Z' });
    rows.push({ signal_id: repeat, checkpoint: '+1h', price_usd: 10, target_at: '2026-03-09T00:00:00Z' }); // +900%, impossible to miss if leaked in
...
test('robust report is reproducible: re-running against unchanged data yields identical bootstrap CIs', () => {
  ...
  try {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      ids.push(id);
      rows.push({ signal_id: id, checkpoint: 'signal', price_usd: 1, target_at: observedAt });
      rows.push({ signal_id: id, checkpoint: '+1h', price_usd: 1 + (i % 4) * 0.1 - 0.15, target_at: observedAt });
    }
    insertCompletedRun(database, ids, rows);
    const a = computeRobustPatternReport(database, new Date('2026-03-10T00:00:00Z'), { bootstrapIterations: 400 });
    const b = computeRobustPatternReport(database, new Date('2026-03-10T00:00:00Z'), { bootstrapIterations: 400 });
    assert.deepEqual(a.groups.find((g) => g.key === '13')?.bootstrap, b.groups.find((g) => g.key === '13')?.bootstrap);
  } finally { database.close(); }
});
