...
test('integrity report never fabricates fields for an empty database', () => {
  ...
  try {
    const report = readIntegrityReport(database, {
      archiveDirectory: emptyArchiveDirectory,
    ...
    assert.deepEqual(report.signals.timestampCoverage, {
      ...
      earliestTriggerAt: null,
      latestTriggerAt: null,
    });
    assert.deepEqual(report.duplicates, {
      pollRepeatedSignals: 0,
      duneImportSkippedRows: 0,
      browserImportSkippedSignals: 0,
    });
...
test('signals are broken down by source and signal type without judging good or bad', () => {
  ...
  try {
    storeGmgnSignal(
      database,
      { observed_at: '2026-08-09T01:00:00Z', token_address: 'A', signal_type: 'buy' },
      {
        capturedAt: new Date('2026-08-09T01:00:01Z'),
        logger: { warn() {} },
        source: 'gmgn-cli',
        chain: 'sol',
      },
    ...
    storeGmgnSignal(
      database,
      { observed_at: '2026-08-09T02:00:00Z', token_address: 'B', signal_type: 'buy' },
      {
        capturedAt: new Date('2026-08-09T02:00:01Z'),
        logger: { warn() {} },
        source: 'gmgn-cli',
        chain: 'sol',
      },
    ...
    storeGmgnSignal(
      database,
      { observed_at: '2026-08-09T03:00:00Z', token_address: 'C', signal_type: 'watch' },
      {
        capturedAt: new Date('2026-08-09T03:00:01Z'),
        logger: { warn() {} },
...
test('poll integrity counts gaps, empty polls, crashed polls, and failures separately', () => {
  ...
  try {
    ...
    const pollA = startGmgnPoll(database, {
    ...
    completeGmgnPoll(database, pollA, {
      completedAt: '2026-08-09T01:00:05Z',
      received: 3,
      stored: 3,
      repeated: 0,
      errors: 0,
    ...
    completeGmgnPoll(database, pollB, {
      completedAt: '2026-08-09T02:00:05Z',
      received: 2,
      stored: 1,
      repeated: 1,
      errors: 0,
      bounds: { oldestTriggerAt: '2026-08-09T01:30:00Z', newestTriggerAt: '2026-08-09T02:00:00Z' },
      previousNewest: '2026-08-09T01:00:00Z',
      archivePath: '/archive/b.zip',
      archiveSha256: 'sha-b',
    });

    // Completed poll that received nothing.
    const pollC = startGmgnPoll(database, {
    ...
    completeGmgnPoll(database, pollC, {
      completedAt: '2026-08-09T03:00:05Z',
      received: 0,
      stored: 0,
      repeated: 0,
      errors: 0,
      bounds: { oldestTriggerAt: null, newestTriggerAt: null },
      previousNewest: '2026-08-09T02:00:00Z',
      archivePath: '/archive/c.zip',
      archiveSha256: 'sha-c',
    });

    // Failed poll.
    const pollD = startGmgnPoll(database, {
...
test('provenance reports archive coverage for dune imports, browser imports, and gmgn capture archives', () => {
  ...
  try {
    ...
    try {
      ...
      const manifest = {
        capturedAt: '2026-08-09T03:00:00.000Z',
        eventCount: 1,
        stored: 1,
        repeated: 0,
        validationErrors: 0,
      ...
      writeFileSync(
        path.join(directory, `gmgn-capture-2026-08-09T03-00-00-000Z-${hash.slice(0, 16)}.zip`),
        buffer,
      );

      const report = readIntegrityReport(database, { archiveDirectory: directory });
      assert.deepEqual(report.provenance.duneImportBatches, {
        total: 2,
        completed: 1,
        failed: 0,
        archived: 1,
      });
      assert.deepEqual(report.provenance.gmgnBrowserImportBatches, {
        total: 1,
        completed: 1,
        failed: 0,
        archived: 1,
      });
      assert.deepEqual(report.provenance.gmgnCaptureArchives, {
        total: 1,
        verified: 1,
        unverified: 0,
      });
  ...
  try {
    ...
    for (const forbidden of ['score', 'rank', 'profit', 'return', 'strategy', 'good', 'bad']) {
      assert.doesNotMatch(
        ...
        `integrity report must not mention "${forbidden}"`,
      );
    }
  } finally {
    database.close();
