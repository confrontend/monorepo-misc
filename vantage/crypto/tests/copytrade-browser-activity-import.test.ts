...
const exportFile = (wallet = 'WALLET_A'): string =>
  JSON.stringify({
    formatVersion: 1,
                ...
                activities: [
                  {
                    ...
                    tx_hash: 'TX-1',
                    event_type: 'sell',
                    token: { address: 'TOKEN-1', symbol: 'AAA' },
                    timestamp: 1786719207,
                    token_amount: '10',
                    cost_usd: '12',
                    buy_cost_usd: '10',
                    price_usd: '1.2',
...
test('browser wallet activity importer normalizes trades and preserves provenance in raw payload', () => {
  ...
  const row = database
    .prepare(
      'SELECT wallet_address, observed_timestamp, fetched_at, raw_payload FROM copytrade_trades',
    )
  ...
  assert.equal(
    (raw.__gmgn_browser_provenance as Record<string, unknown>).sourceName,
    'investigation.json',
  );
  assert.match(
    String((raw.__gmgn_browser_provenance as Record<string, unknown>).sourceUrl),
    /wallet_activity\/sol/,
  );
});
