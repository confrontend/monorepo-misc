...
test('builds an isolated crypto CLI command with chronological holdout settings', () => {
  ...
  assert.deepEqual(command.args.slice(command.args.indexOf('--project'), command.args.indexOf('--project') + 2), ['--project', 'crypto']);
  assert.deepEqual(command.args.slice(command.args.indexOf('--min-n'), command.args.indexOf('--min-n') + 2), ['--min-n', '17']);
  assert.deepEqual(command.args.slice(command.args.indexOf('--holdout-fraction'), command.args.indexOf('--holdout-fraction') + 2), ['--holdout-fraction', '0.2']);
...
test('validates runner inputs and parses only complete crypto reports', () => {
  ...
  const report = parsePatternDiscoveryReport(JSON.stringify({
    project: 'crypto',
    patterns: [],
    status_counts: { 'discovered candidate': 0, 'validation survivor': 0, rejected: 0, 'insufficient data': 0 },
  ...
  assert.throws(() => parsePatternDiscoveryReport('{"project":"unusualwhales","patterns":[],"status_counts":{},"split":{}}'), /incomplete crypto report/);
});

test('configured Python executable takes precedence over fallback paths', () => {
  assert.equal(resolvePatternDiscoveryPython('C:/workspace', 'C:/workspace/research/shared-pattern-discovery', { PATTERN_DISCOVERY_PYTHON: 'C:/tools/python.exe' }), 'C:/tools/python.exe');
});

test('portable home-based bundled runtime is considered after workspace venvs', () => {
  const home = 'C:/portable-home';
  const candidates = patternDiscoveryPythonCandidates('C:/workspace/crypto', 'C:/workspace/research/shared-pattern-discovery', { USERPROFILE: home });
  assert.equal(candidates.at(-1), path.join(home, '.cache', 'codex-runtimes', 'codex-primary-runtime', 'dependencies', 'python', 'python.exe'));
  const root = path.join(tmpdir(), `pattern-discovery-runtime-${Date.now()}`);
  const bundled = path.join(root, '.cache', 'codex-runtimes', 'codex-primary-runtime', 'dependencies', 'python', 'python.exe');
  try {
    mkdirSync(path.dirname(bundled), { recursive: true });
    writeFileSync(bundled, 'placeholder');
    assert.equal(resolvePatternDiscoveryPython('C:/workspace/crypto', 'C:/workspace/research/shared-pattern-discovery', { USERPROFILE: root }), bundled);
  } finally {
