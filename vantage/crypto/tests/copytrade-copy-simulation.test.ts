...
import {
  ...
  rowsToMatches,
  sqlFor,
  WIDE_SEARCH_WINDOW_MINUTES,
} from '../src/copytrade/simulation/copySimulationDune.js';
...
test('Dune retries use the same five-minute accuracy window and preserve provenance', () => {
  const target = {
    tradeId: 1,
    tokenAddress: 'Token',
    delayedTargetAtIso: '2026-08-20T00:00:00.000Z',
  };
  ...
  const matches = rowsToMatches(
    [target],
    [{ trade_id: 1, matched_trade_at: '2026-08-20T00:01:00.000Z', price_usd: 1, amount_usd: 4 }],
    'wide_window',
  );
...
const insertTradeRow = (
  ...
  database
    ...
    .run(
      ...
      over.eventType,
      over.tokenAddress,
      over.observedTimestamp,
      over.tokenAmount ?? '100',
      over.costUsd ?? null,
      over.buyCostUsd ?? null,
      over.priceUsd ?? null,
      `DEDUP${id}`,
    );
  return id;
      ...
      JSON.stringify({
        result: {
          rows: [
            {
              trade_id: tradeId,
              matched_trade_at: matchedTradeAtIso,
              price_usd: priceUsd,
              matched_tx_id: `TX_MATCH_${tradeId}`,
...
const seedRoundTrip = (
  ...
  const buyId = insertTradeRow(database, {
    ...
    eventType: 'buy',
    tokenAddress,
    observedTimestamp: baseTimestamp,
    buyCostUsd: '100',
    priceUsd: '1',
  });
  ...
  const sellId = insertTradeRow(database, {
    ...
    eventType: 'sell',
    tokenAddress,
    observedTimestamp: baseTimestamp + 100,
    costUsd: String(100 * sellPrice),
    buyCostUsd: '100',
    priceUsd: String(sellPrice),
  });
  seedDuneMatch(
    database,
    buyId,
    new Date((baseTimestamp + DEFAULT_COPIER_DELAY_SECONDS) * 1000).toISOString(),
    1,
  );
  seedDuneMatch(
    database,
    sellId,
    new Date((baseTimestamp + 100 + DEFAULT_COPIER_DELAY_SECONDS) * 1000).toISOString(),
    sellPrice,
  );
};
...
test('tail metrics use only simulated trades and expose median/mean divergence plus extreme wins', () => {
  ...
  try {
    ...
    insertTradeRow(database, {
      walletAddress: 'W1',
      eventType: 'buy',
      tokenAddress: 'UNQUERIED',
    ...
    assert.equal(
      wallet.tailShareOfMeanPercent,
      100.8,
      'tail share is not clamped when negative trades make the denominator smaller',
    );
...
test('a sell with no preceding buy in stored history is excluded, not treated as a round trip at all', () => {
  ...
  try {
    ...
    assert.equal(
      report.wallets[0]?.roundTripsConsidered,
      0,
      'no buy to pair with means no round trip, not a missing/zero one',
    );
  } finally {
    database.close();
  }
...
test('a round trip with both legs matched well within the gap tolerance is simulated, fees and slippage applied', () => {
  ...
  try {
    ...
    const expectedReturnPercent =
      Math.round(((expectedExit - expectedEntry) / expectedEntry) * 100 * 100) / 100;
    assert.equal(wallet.trades[0]?.simulatedReturnPercent, expectedReturnPercent);
    assert.ok(
      (wallet.simulatedMedianReturnPercent ?? wallet.trades[0].simulatedReturnPercent) < 50,
      'fees and slippage must reduce the return below the raw price delta',
    );
  } finally {
    database.close();
  }
...
test('a round trip with one leg queried-no-match and the other never queried attributes exactly one leg to each bucket', () => {
  ...
  try {
    ...
    assert.equal(
      wallet.pendingDuneTargets,
      1,
      'the never-queried sell leg is pending, not a Dune miss',
    );
...
test('per-wallet pending/no-match/matched target counts sum to the report-level totals across multiple wallets', () => {
  ...
  try {
    ...
    assert.equal(
      sum('pendingDuneTargets'),
      report.pendingDuneTargets,
      'per-wallet pending totals must match the report-level total the fetch button reads',
    );
    ...
    assert.deepEqual(
      [w1.pendingDuneTargets, w1.duneNoMatchTargets, w1.duneMatchedTargets],
      [0, 0, 2],
    );
    assert.deepEqual(
      [w2.pendingDuneTargets, w2.duneNoMatchTargets, w2.duneMatchedTargets],
      [2, 1, 1],
    );
  } finally {
    database.close();
...
test('a match beyond MAX_MATCH_GAP_SECONDS is rejected as stale, not silently used', () => {
  ...
  try {
    ...
    const staleEntryAt = new Date(
      (1000 + DEFAULT_COPIER_DELAY_SECONDS + MAX_MATCH_GAP_SECONDS + 60) * 1000,
    ).toISOString();
    ...
    assert.equal(
      wallet.trades[0]?.walletReturnPercent,
      50,
      "the wallet's own real return is still reported even when the simulation cannot be",
    );
    assert.equal(
      wallet.trades[0]?.simulatedReturnPercent,
      null,
      'a stale match must never be used, not even as a fallback',
    );
  } finally {
    database.close();
  }
...
test("delayCostPercentagePoints is the gap between the simulated and the wallet's own median return", () => {
  ...
  try {
    ...
    seedDuneMatch(
      database,
      sellId,
      new Date((2000 + DEFAULT_COPIER_DELAY_SECONDS) * 1000).toISOString(),
      1.5,
    );
...
test('assumptions in the report reflect the actual parameters used, not just the defaults', () => {
  ...
  try {
    ...
    assert.deepEqual(report.assumptions, {
      ...
      feeBps: 20,
      slippageBps: 10,
      gasPriorityFeeSolPerTx: report.assumptions.gasPriorityFeeSolPerTx,
      maxMatchGapSeconds: MAX_MATCH_GAP_SECONDS,
      maxRoundTripsPerWallet: report.assumptions.maxRoundTripsPerWallet,
...
test('walletMedianReturnPercent and coverageRatePercent are computed on the same subset as the simulated median, not every round trip considered', () => {
  ...
  try {
    ...
    const buy1 = insertTradeRow(database, {
      walletAddress: 'W1',
      eventType: 'buy',
      tokenAddress: 'TOKA',
      observedTimestamp: 1000,
    ...
    const sell1 = insertTradeRow(database, {
      walletAddress: 'W1',
      eventType: 'sell',
      tokenAddress: 'TOKA',
      observedTimestamp: 2000,
    ...
    assert.equal(wallet.copiedTrades, 1);
    assert.equal(
      wallet.walletMedianReturnPercent,
      100,
      'the wallet median must reflect only the trade that was actually simulated, not the uncopied -90% one',
    );
...
test('gas fee is reported separately in SOL, two transactions per copied round trip, and is null for anything not actually simulated', () => {
  ...
  try {
    ...
    const buy2 = insertTradeRow(database, {
      walletAddress: 'W1',
      eventType: 'buy',
      tokenAddress: 'TOKB',
      observedTimestamp: 3000,
    ...
    const sell2 = insertTradeRow(database, {
      walletAddress: 'W1',
      eventType: 'sell',
      tokenAddress: 'TOKB',
      observedTimestamp: 4000,
    ...
    seedDuneMatch(
      ...
      buy2,
      new Date(
        (3000 + DEFAULT_COPIER_DELAY_SECONDS + MAX_MATCH_GAP_SECONDS + 60) * 1000,
      ).toISOString(),
      1.0,
    );
    seedDuneMatch(
      database,
      sell2,
      new Date((4000 + DEFAULT_COPIER_DELAY_SECONDS) * 1000).toISOString(),
      1.1,
    );

    const report = computeCopySimulationReport(database, {
      walletAddresses: ['W1'],
      gasPriorityFeeSolPerTx: 0.002,
    });
    const wallet = report.wallets[0];
    assert.equal(wallet.copiedTrades, 1);
    assert.equal(
      wallet.trades.find((t) => t.tokenAddress === 'TOKA')?.gasFeeSol,
      0.004,
      'one buy + one sell = two gas payments',
    );
    assert.equal(
      wallet.trades.find((t) => t.tokenAddress === 'TOKB')?.gasFeeSol,
      null,
      'a trade that was never actually copied paid no gas',
    );
...
test('fixed-stake portfolio never reinvests the full bankroll and caps each loss at its stake', () => {
  ...
  assert.equal(
    portfolio.endingCapitalUsd,
    95,
    '$10 gains $5, then the next $10 can lose at most $10',
  );
  ...
  assert.equal(
    portfolio.gasFeeSol,
    0.008,
    'fixed SOL gas is reported, not silently converted with a guessed price',
  );
});
...
test('partial sells allocate one copied position proportionally instead of opening duplicate stakes', () => {
  ...
  try {
    ...
    const sellOneId = insertTradeRow(database, {
      walletAddress: 'PARTIAL',
      eventType: 'sell',
      tokenAddress: 'TOKP',
    ...
    const sellTwoId = insertTradeRow(database, {
      walletAddress: 'PARTIAL',
      eventType: 'sell',
      tokenAddress: 'TOKP',
      observedTimestamp: 1200,
      tokenAmount: '60',
      costUsd: '180',
      buyCostUsd: '60',
      priceUsd: '3',
    });
    for (const [id, at, price] of [
      [buyId, 1000, 1],
      [sellOneId, 1100, 2],
      [sellTwoId, 1200, 3],
    ] as const) {
      seedDuneMatch(
        database,
        id,
        new Date((at + DEFAULT_COPIER_DELAY_SECONDS) * 1000).toISOString(),
        price,
      );
    }
    const wallet = computeCopySimulationReport(database, {
    ...
    assert.deepEqual(
      wallet.trades.map((trade) => trade.copyStakeUsd),
      [4, 6],
    );
    assert.equal(
      wallet.portfolio.endingCapitalUsd,
      115.97,
      'one $10 position is split 40/60 and recorded gas is charged once for the buy and once per sell',
    );
  } finally {
    database.close();
  }
...
test('copy simulation exposes the fixed-stake portfolio from the same delayed fee-adjusted prices', () => {
  ...
  try {
    ...
    assert.equal(
      wallet.portfolio.endingCapitalUsd,
      104.54,
      'the $10 stake receives the fee/slippage-adjusted return minus recorded gas USD',
    );
...
const makeTrade = (
  over: Partial<CopySimulationTradeResult> & { entryTradeAmountUsd: number | null },
): CopySimulationTradeResult => ({
  tokenAddress: 'TOK',
  tokenSymbol: null,
  walletReturnPercent: null,
...
const makeReport = (trades: CopySimulationTradeResult[]): CopySimulationReport => ({
  ...
  assumptions: {
    copierDelaySeconds: 15,
    feeBps: 100,
    slippageBps: 50,
    gasPriorityFeeSolPerTx: 0.002,
    maxMatchGapSeconds: MAX_MATCH_GAP_SECONDS,
    maxRoundTripsPerWallet: 150,
    startingCapitalUsd: 100,
    stakePerTradeUsd: 10,
    maxOpenPositions: 10,
  },
  wallets: [
    {
      walletAddress: 'W1',
      roundTripsConsidered: trades.length,
      copiedTrades: trades.filter((t) => t.status === 'simulated').length,
      missedTrades: trades.filter((t) => t.status !== 'simulated').length,
      coverageRatePercent: null,
      walletMedianReturnPercent: null,
      simulatedMedianReturnPercent: null,
      delayCostPercentagePoints: null,
      walletMeanReturnPercent: null,
      simulatedMeanReturnPercent: null,
      tradesAbove100Percent: 0,
      tradesAbove300Percent: 0,
      bestSimulatedReturnPercent: null,
      tailShareOfMeanPercent: null,
      worstSimulatedReturnPercent: null,
      totalGasFeeSol: null,
...
test('liquidity impact: trades with no entry match at all are excluded from every band, not defaulted into one', () => {
  const trades = [
    ...
    makeTrade({
      entryTradeAmountUsd: 100,
      status: 'simulated',
      simulatedReturnPercent: 5,
      walletReturnPercent: 6,
    }),
    makeTrade({
      entryTradeAmountUsd: 200,
      status: 'simulated',
      simulatedReturnPercent: 5,
      walletReturnPercent: 6,
    }),
    makeTrade({
      entryTradeAmountUsd: 300,
      status: 'simulated',
      simulatedReturnPercent: 5,
      walletReturnPercent: 6,
    }),
  ...
  assert.equal(
    impact.unbandableCount,
    1,
    'the never-queried trade has no entryTradeAmountUsd and must be excluded, not zero-banded',
  );
...
test('liquidity impact: terciles split into low/medium/high by entry trade USD size, win rate and delay cost computed per band', () => {
  const trades = [
    ...
    makeTrade({
      entryTradeAmountUsd: 10,
      status: 'simulated',
      simulatedReturnPercent: 20,
      walletReturnPercent: 20,
    }),
    makeTrade({
      entryTradeAmountUsd: 20,
      status: 'simulated',
      simulatedReturnPercent: 10,
      walletReturnPercent: 10,
    }),
    makeTrade({
      entryTradeAmountUsd: 30,
      status: 'simulated',
      simulatedReturnPercent: 15,
      walletReturnPercent: 15,
    }),
    // Medium band: one win, one loss, real delay cost.
    makeTrade({
      entryTradeAmountUsd: 1000,
      status: 'simulated',
      simulatedReturnPercent: -5,
      walletReturnPercent: 5,
    }),
    makeTrade({
      entryTradeAmountUsd: 1100,
      status: 'simulated',
      simulatedReturnPercent: 8,
      walletReturnPercent: 8,
    }),
    makeTrade({ entryTradeAmountUsd: 1200, status: 'missing_exit_match' }),
    // High band: large entries, missed trade included.
    makeTrade({
      entryTradeAmountUsd: 50000,
      status: 'simulated',
      simulatedReturnPercent: 3,
      walletReturnPercent: 3,
    }),
    makeTrade({
      entryTradeAmountUsd: 60000,
      status: 'simulated',
      simulatedReturnPercent: -1,
      walletReturnPercent: 1,
    }),
  ...
  assert.equal(
    low.medianDelayCostPercentagePoints,
    0,
    'no fees/slippage modeled here — simulated equals wallet return, so delay cost is 0',
  );
  ...
  for (const band of impact.bands)
    assert.equal(
      band.reliable,
      false,
      `band ${band.band} has fewer than ${MIN_LIQUIDITY_BAND_SAMPLE} simulated trades`,
    );
});
...
test('liquidity impact: a band only becomes reliable once it reaches MIN_LIQUIDITY_BAND_SAMPLE simulated trades', () => {
  ...
  const fewHigh = [
    makeTrade({
      entryTradeAmountUsd: 90000,
      status: 'simulated',
      simulatedReturnPercent: 1,
      walletReturnPercent: 1,
    }),
    makeTrade({
      entryTradeAmountUsd: 95000,
      status: 'simulated',
      simulatedReturnPercent: 1,
      walletReturnPercent: 1,
    }),
...
const makeReportForWallets = (
  wallets: Array<{ walletAddress: string; trades: CopySimulationTradeResult[] }>,
...
test('liquidity impact: byWallet slices each winner using the SAME thresholds as the aggregate, so small trades mean the same size for every wallet', () => {
  ...
  assert.equal(
    impact.bands.find((b) => b.band === 'low')!.tradeCount,
    3,
    "aggregate low band still holds all three wallets' small trades combined",
  );
  ...
  assert.equal(
    lowA.tradeCount,
    1,
    'wallet A contributes exactly its own one small trade to its own slice',
  );
  ...
  assert.equal(
    lowB.medianSimulatedReturnPercent,
    -5,
    "wallet B's small trade is a real loss — the whole point of a per-wallet slice",
  );

  const walletC = impact.byWallet.find((w) => w.walletAddress === 'WC')!;
  assert.equal(
    walletC.bands.find((b) => b.band === 'low')!.tradeCount,
    1,
    "wallet C's 30-size trade still lands in low using the shared threshold, not a wallet-specific one",
  );
