...
import {
  assessWalletRiskGuardrails, HIGH_VOLUME_30D_THRESHOLD, HIGH_CREATED_TOKEN_COUNT_THRESHOLD,
  THIN_SAMPLE_30D_THRESHOLD, WEAK_WIN_RATE_PERCENT, ONE_SIDED_IMBALANCE_PERCENT,
  type WalletRiskStats30d,
} from '../src/copytrade/scrutiny/walletRiskGuardrails.js';

const clean: WalletRiskStats30d = {
  tags: [], realizedProfitPnlPercent: 15, buyCount: 40, sellCount: 40, createdTokenCount: 0, winRatePercent: 55,
};
...
test('high 30d volume is flagged only strictly above the threshold', () => {
  const atThreshold = { ...clean, buyCount: HIGH_VOLUME_30D_THRESHOLD / 2, sellCount: HIGH_VOLUME_30D_THRESHOLD / 2 };
  assert.equal(assessWalletRiskGuardrails(atThreshold).length, 0);
  const overThreshold = { ...clean, buyCount: HIGH_VOLUME_30D_THRESHOLD / 2 + 1, sellCount: HIGH_VOLUME_30D_THRESHOLD / 2 };
...
test('mass token creation is flagged only strictly above the threshold', () => {
  assert.equal(assessWalletRiskGuardrails({ ...clean, createdTokenCount: HIGH_CREATED_TOKEN_COUNT_THRESHOLD }).length, 0);
  assert.equal(assessWalletRiskGuardrails({ ...clean, createdTokenCount: HIGH_CREATED_TOKEN_COUNT_THRESHOLD + 1 }).length, 1);
});
...
test('weak win rate is flagged only strictly below the threshold', () => {
  assert.equal(assessWalletRiskGuardrails({ ...clean, winRatePercent: WEAK_WIN_RATE_PERCENT }).length, 0);
  assert.equal(assessWalletRiskGuardrails({ ...clean, winRatePercent: WEAK_WIN_RATE_PERCENT - 0.1 }).length, 1);
});
...
test('a wallet can trip multiple guardrails at once, and every reason is reported', () => {
  ...
  const reasons = assessWalletRiskGuardrails({
    tags: ['wash_trader'], realizedProfitPnlPercent: -50, buyCount: 6000, sellCount: 0,
    createdTokenCount: 500, winRatePercent: 5,
  });
...
test('null fields (stats not yet available for that metric) are never treated as a violation', () => {
  const allNull: WalletRiskStats30d = {
    tags: [], realizedProfitPnlPercent: null, buyCount: null, sellCount: null, createdTokenCount: null, winRatePercent: null,
  };
