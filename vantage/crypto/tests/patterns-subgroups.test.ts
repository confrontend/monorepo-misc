...
test('launch platform resolves from either raw_payload shape (verbose data.launchpad or abbreviated cur_data.lp)', () => {
  ...
  try {
    const verbose = seedSignal(database, 'v1', 7, 'TokenV', {
      trigger_at: 1000,
      data: { launchpad: 'Pump.fun', created_timestamp: 0 },
    });
    const abbreviated = seedSignal(database, 'a1', 7, 'TokenA', {
      trigger_at: 1000,
      cur_data: { lp: 'meteora_virtual_curve', ct: 0 },
    });
    insertCompletedRun(
      database,
      [verbose, abbreviated],
      [...priceRows(verbose, 1, 1.1), ...priceRows(abbreviated, 1, 1.1)],
    );

    const report = computeSignalPatternSubgroupReport(
      database,
      'launchPlatform',
    ...
    assert.ok(
      horizon.groups.some((g) => g.key === '7::Pump.fun'),
      'verbose data.launchpad shape must resolve',
    );
    assert.ok(
      horizon.groups.some((g) => g.key === '7::meteora_virtual_curve'),
      'abbreviated cur_data.lp shape must resolve',
    );
...
test('a signal with no extractable launch platform is excluded from grouping and counted, not guessed', () => {
  const database = openDatabase(':memory:');
  try {
    const known = seedSignal(database, 'k1', 7, 'TokenK', {
      trigger_at: 1000,
      data: { launchpad: 'Pump.fun', created_timestamp: 0 },
    });
    ...
    insertCompletedRun(
      database,
      [known, unknown],
      [...priceRows(known, 1, 1.1), ...priceRows(unknown, 1, 1.1)],
    );
...
test('malformed raw_payload cannot crash extraction and is treated as unextractable', () => {
  const database = openDatabase(':memory:');
  try {
    const id = seedSignal(database, 'm1', 7, 'TokenM', {
      trigger_at: 1000,
      data: { launchpad: 'Pump.fun', created_timestamp: 0 },
    });
    insertCompletedRun(database, [id], priceRows(id, 1, 1.1));
    database
      .prepare('UPDATE gmgn_signals SET raw_payload = ? WHERE id = ?')
...
test('token age buckets correctly, and a negative age (clock skew) is excluded rather than clipped into the youngest bucket', () => {
  const database = openDatabase(':memory:');
  try {
    const fresh = seedSignal(database, 'f1', 7, 'TokenF', {
      trigger_at: 1000,
      data: { created_timestamp: 1000 - 60 },
    }); // 1 minute old -> <1h
    const mid = seedSignal(database, 'm2', 7, 'TokenM2', {
      trigger_at: 100_000,
      data: { created_timestamp: 100_000 - 7200 },
    }); // 2h old -> 1-24h
    const days = seedSignal(database, 'd1', 7, 'TokenD', {
      trigger_at: 1_000_000,
      data: { created_timestamp: 1_000_000 - 200_000 },
    }); // ~2.3 days old -> 1-7d
    const weeks = seedSignal(database, 'w1', 7, 'TokenW', {
      trigger_at: 2_000_000,
      data: { created_timestamp: 2_000_000 - 15 * 86400 },
    }); // 15 days old -> 7-30d
    const old = seedSignal(database, 'o1', 7, 'TokenO', {
      trigger_at: 6_000_000,
      data: { created_timestamp: 6_000_000 - 60 * 86400 },
    }); // 60 days old -> >30d
    const skewed = seedSignal(database, 's1', 7, 'TokenS', {
      trigger_at: 1000,
      data: { created_timestamp: 5000 },
    }); // "created" after "triggered" -> invalid
    insertCompletedRun(
      database,
      [fresh, mid, days, weeks, old, skewed],
      [
        ...priceRows(fresh, 1, 1.1),
        ...priceRows(mid, 1, 1.1),
        ...priceRows(days, 1, 1.1),
        ...priceRows(weeks, 1, 1.1),
        ...priceRows(old, 1, 1.1),
        ...priceRows(skewed, 1, 1.1),
      ],
    );
    ...
    assert.ok(
      horizon.groups.some((g) => g.key === '7::1-7d'),
      'a token a few days old must land in 1-7d, not a single catch-all ">24h" bucket',
    );
    assert.ok(horizon.groups.some((g) => g.key === '7::7-30d'));
    assert.ok(horizon.groups.some((g) => g.key === '7::>30d'));
    assert.equal(
      horizon.nUnextractable,
      1,
      'the negative-age signal must be excluded, not bucketed as <1h',
    );
  } finally {
    database.close();
  }
...
test('cellCount matches the number of distinct signalType::bucket combinations that actually have data', () => {
  const database = openDatabase(':memory:');
  try {
    const a = seedSignal(database, 'c1', 7, 'TokenC1', {
      trigger_at: 1000,
      data: { launchpad: 'Pump.fun', created_timestamp: 0 },
    });
    const b = seedSignal(database, 'c2', 7, 'TokenC2', {
      trigger_at: 1000,
      data: { launchpad: 'Pump.fun', created_timestamp: 0 },
    }); // same cell as a
    const c = seedSignal(database, 'c3', 12, 'TokenC3', {
      trigger_at: 1000,
      data: { launchpad: 'meteora_virtual_curve', created_timestamp: 0 },
    }); // distinct cell
    insertCompletedRun(
      database,
      [a, b, c],
      [...priceRows(a, 1, 1.1), ...priceRows(b, 1, 1.2), ...priceRows(c, 1, 0.9)],
    );
    ...
    assert.equal(
      horizon.cellCount,
      2,
      'two signals sharing a cell must not inflate the cell count',
    );
...
test('a subgroup cell reuses the same reliability/verdict logic as the top-level report (small sample stays unreliable)', () => {
  const database = openDatabase(':memory:');
  try {
    const id = seedSignal(database, 'r1', 7, 'TokenR', {
      trigger_at: 1000,
      data: { launchpad: 'Pump.fun', created_timestamp: 0 },
    });
    ...
    assert.equal(
      cell.reliable,
      false,
      'one observation must not be reliable, same gate as the top-level report',
    );
...
test('"combined" joins both properties into one bucket, and requires both to resolve', () => {
  const database = openDatabase(':memory:');
  try {
    const both = seedSignal(database, 'b1', 7, 'TokenB', {
      trigger_at: 1000,
      data: { launchpad: 'Pump.fun', created_timestamp: 1000 - 60 },
    }); // <1h
    const onlyPlatform = seedSignal(database, 'p1', 7, 'TokenP', {
      trigger_at: 1000,
      data: { launchpad: 'Pump.fun' },
    }); // no created_timestamp -> age missing
    const onlyAge = seedSignal(database, 'a1', 7, 'TokenA', {
      trigger_at: 1000,
      data: { created_timestamp: 1000 - 60 },
    }); // no launchpad -> platform missing
    insertCompletedRun(
      database,
      [both, onlyPlatform, onlyAge],
      [
        ...priceRows(both, 1, 1.1),
        ...priceRows(onlyPlatform, 1, 1.1),
        ...priceRows(onlyAge, 1, 1.1),
      ],
    );
    ...
    const horizon = report.horizons.find((h) => h.horizon === '+1h')!;
    assert.ok(
      horizon.groups.some((g) => g.key === '7::Pump.fun / <1h'),
      'both properties resolved -> a combined cell must exist',
    );
    assert.equal(
      horizon.nUnextractable,
      2,
      'a signal missing either property must be excluded, not partially bucketed',
    );
  } finally {
    database.close();
  }
