...
const seedSignal = (
  ...
  storeGmgnSignal(
    ...
    { source: 'gmgn-cli', chain: 'sol', capturedAt: new Date('2026-03-01T00:00:01Z') },
  ).id;
...
`,
      ...
      .run(
        JSON.stringify(options.signalIds),
...
const fetchMock =
  (state: string, rows: Array<Record<string, unknown>> = []) =>
  async (url: string) => {
    if (url.includes('/status')) return { ok: true, json: async () => ({ state }) } as Response;
    if (url.includes('/results'))
      return { ok: true, text: async () => JSON.stringify({ result: { rows } }) } as Response;
...
test('reconcileDuneRun reports completed without any network call for an already-completed run', async (t) => {
  ...
  try {
    ...
    const runId = Number(
      database
        .prepare(
          `INSERT INTO dune_outcome_runs (signal_ids, query_sql, status, requested_at, completed_at) VALUES (?, 'select 1', 'completed', ?, ?)`,
        )
        .run(JSON.stringify([signalId]), new Date().toISOString(), new Date().toISOString())
        .lastInsertRowid,
    );
...
test('reconcileDuneRun gives a run with no execution id a grace period before writing it off', async () => {
  ...
  try {
    ...
    const runId = insertStuckRun(database, {
      signalIds: [signalId],
      status: 'submitted',
      executionId: null,
    ...
    assert.equal(
      await reconcileDuneRun(database, runId),
      'still_running',
      'a run submitted moments ago may still be genuinely in flight over the network',
    );
    const row = database
      .prepare('SELECT status FROM dune_outcome_runs WHERE id = ?')
...
test('reconcileDuneRun writes off a stale run that never received an execution id', async () => {
  ...
  try {
    ...
    const row = database
      .prepare('SELECT status, completed_at AS completedAt FROM dune_outcome_runs WHERE id = ?')
      .get(runId) as { status: string; completedAt: string | null };
    assert.equal(row.status, 'failed');
    assert.ok(
      row.completedAt,
      'completed_at must be recorded so this run stops being treated as stuck',
    );
  } finally {
    database.close();
  }
...
test('reconcileDuneRun finalizes a run that actually completed on Dune, and the result becomes visible through readAllDuneOutcomes', async (t) => {
  ...
  try {
    ...
    t.mock.method(
      globalThis,
      'fetch',
      fetchMock('QUERY_STATE_COMPLETED', [
        {
          signal_id: signalId,
          checkpoint: 'signal',
          target_at: '2026-03-01 00:00:00.000 UTC',
          price_usd: 1,
          matched_trade_at: '2026-03-01 00:00:00.000 UTC',
        },
      ]),
    );
    assert.equal(await reconcileDuneRun(database, runId), 'completed');
    const row = database
      .prepare(
        'SELECT status, raw_result AS rawResult, archive_path AS archivePath FROM dune_outcome_runs WHERE id = ?',
      )
      .get(runId) as { status: string; rawResult: string | null; archivePath: string | null };
    assert.equal(row.status, 'completed');
    assert.ok(row.rawResult);
    assert.ok(
      row.archivePath,
      'a reconciled run must be archived exactly like a freshly measured one',
    );
    ...
    assert.equal(
      ...
      1,
      'reconciled data must flow through the same read path as directly measured data',
    );
  } finally {
    database.close();
  }
...
test('reconcileStuckDuneRuns sweeps every stuck run and tallies outcomes independently per run', async (t) => {
  ...
  try {
    ...
    const completeRunId = insertStuckRun(database, {
      signalIds: [completing],
      status: 'running',
      executionId: 'exec-a',
    });
    const failRunId = insertStuckRun(database, {
      signalIds: [failing],
      status: 'running',
      executionId: 'exec-b',
    });
    const executingRunId = insertStuckRun(database, {
      signalIds: [executing],
      status: 'timed_out',
      executionId: 'exec-c',
    });
    t.mock.method(globalThis, 'fetch', async (url: string) => {
      if (url.includes('exec-a'))
        return fetchMock('QUERY_STATE_COMPLETED', [
          {
            signal_id: completing,
            checkpoint: 'signal',
            target_at: '2026-03-01 00:00:00.000 UTC',
            price_usd: 1,
          },
