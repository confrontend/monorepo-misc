...
test('scoring transparently credits Dune provenance and supporting fields', () => {
  ...
  try {
    database
      .prepare(
        `INSERT INTO tokens (token_address, first_trade_time, first_dex, first_tx, source, imported_at, raw_payload, validation_errors) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      )
      .run(
        'TokenA',
    ...
    storeGmgnSignal(
      database,
      { id: 'b', token_address: 'TokenB', signal_type: 14 },
      { source: 'gmgn-cli', chain: 'sol', capturedAt: new Date('2026-03-01T00:01:01Z') },
    );
