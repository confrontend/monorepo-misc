...
test('Winner Policy v2 reproduces the spec worked example (strong-but-risky wallet)', () => {
  ...
  assert.equal(WINNER_POLICY_VERSION, 'winner-policy-v2');
  ...
  assert.equal(result.gmgnRiskScore!.score, 18);
  assert.equal(result.finalScore, 81);
});

test('Winner Policy v2 rewards clean execution over risky execution at identical profitability', () => {
...
test('Winner Policy v2 rejects on a non-positive median despite excellent GMGN data', () => {
...
test('Winner Policy v2 rejects when the canonical portfolio does not end above $100', () => {
...
test('Winner Policy v2 is UNPROVEN below minimum evidence and reports no score', () => {
...
test('Winner Policy v2 never treats a missing GMGN risk bundle as zero risk', () => {
...
test('Winner Policy v2 always flags a missing honeypot ratio when a risk bundle is present', () => {
...
test('Winner Policy v2 caps the combined execution-speed penalty at its budget (worst-of, not summed)', () => {
  ...
  assert.equal(result.gmgnRiskScore!.deductions.executionSpeed, 15);
});

test('Winner Policy v2 is identical between Decision Lab and Live Evaluation for identical evidence', () => {
...
test('Discovered Rules mode cannot override fixed policy gates or scores', () => {
  const result = evaluateWinnerPolicy(evidence({ endingCapitalUsd: 100 }), {
    mode: 'discovered_rules',
  });
  assert.equal(result.mode, 'discovered_rules');
  assert.equal(result.status, 'REJECTED');
  assert.ok(result.warnings.some((warning) => warning.includes('cannot override')));
});
