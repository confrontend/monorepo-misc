...
import {
  listRosterWallets,
  rankItemToWallet,
  readLeaderboardProvenance,
  readWalletRankHistory,
  resolveSingleTrader,
  syncCopyTradeRoster,
} from '../src/copytrade/screening/roster.js';
...
const insertTrade = (database: DatabaseSync, over: Record<string, unknown>): void => {
  storeActivityPage(database, [activity(over)], {
    chain: 'sol',
...
test('resolveSingleTrader: a real-looking Solana address is used directly, no database lookup needed', () => {
  ...
  try {
    ...
    assert.deepEqual(result, {
      kind: 'address',
      walletAddress: 'DxM1hfY8FQ8dNGrucuJzhJcF8KRbjk8WBwrgKvQ9spPv',
    });
  } finally {
    database.close();
  }
...
test('resolveSingleTrader: a name matching a wallet already captured from a real leaderboard snapshot resolves to its address, case-insensitively', () => {
  ...
  try {
    seedRankSnapshot(database, [
      { wallet_address: 'W1', name: 'alpha', pnl_30d: '5.31', winrate_30d: 0.254, tags: [] },
    ]);
...
test('fetch: activity pages parse from both the unwrapped and enveloped shapes', () => {
  const unwrapped = parseActivityPage(
    JSON.stringify({ activities: [activity()], next: 'CURSOR1' }),
  );
  assert.equal(unwrapped.activities.length, 1);
  assert.equal(unwrapped.next, 'CURSOR1');

  const enveloped = parseActivityPage(
    JSON.stringify({ code: 0, data: { activities: [activity()], next: '' } }),
  );
...
test('fetch: rate limits are detected with their reset time, other failures are not', () => {
  const limited = detectRateLimit(
    'Request failed: 429 {"code":429,"error":"RATE_LIMIT_BANNED","reset_at":1775184222}',
  );
  ...
  assert.equal(
    detectRateLimit('connect ETIMEDOUT'),
    null,
    'a network failure must not be mistaken for throttling',
  );
});
...
test('fetch: rows missing identity fields are skipped, never stored half-formed', () => {
  ...
  try {
    ...
    assert.equal(
      result.malformed,
      3,
      'unparseable rows are reported as data loss, not as duplicates',
    );
    assert.equal(result.duplicates, 0);
    assert.equal(
      (database.prepare('SELECT COUNT(*) AS c FROM copytrade_trades').get() as { c: number }).c,
      0,
    );
  } finally {
    database.close();
...
const seedRoster = (
  ...
  wallets: Array<string | Record<string, unknown>>,
): void => {
  seedRankSnapshot(
    database,
    wallets.map((w) => (typeof w === 'string' ? { wallet_address: w, tags: [] } : w)),
  );
  syncCopyTradeRoster(database);
};
...
const markCoverage = (
  ...
  database
    ...
    .run(
      wallet,
      Number(over.requestsUsed ?? 200),
      Number(over.truncated ?? 1),
      Number(over.periodDays ?? 30),
      String(over.stopReason ?? 'request_cap'),
    );
};
...
test('evaluate: reducing the trader count actually shrinks the report', () => {
  ...
  try {
    ...
    assert.equal(
      computeCopyTradeReport(database, { now }).rows.length,
      2,
...
test('roster: "top N" follows the newest snapshot instead of every snapshot ever taken', () => {
  ...
  try {
    ...
    assert.deepEqual(
      everything.map((w) => w.rankPosition),
      [1, 1],
    );
  } finally {
    database.close();
...
test('roster: an explicit snapshot keeps reports on the selected wallet population', () => {
  const database = setup();
  try {
    const selectedSnapshotId = seedRankSnapshot(database, [
      { wallet_address: 'SELECTED', name: 'selected', tags: [] },
    ]);
    syncCopyTradeRoster(database);
    ...
    assert.deepEqual(
      listRosterWallets(database, { snapshotId: selectedSnapshotId }).map(
        (wallet) => wallet.walletAddress,
      ),
      ['SELECTED'],
    );
    assert.deepEqual(
      listRosterWallets(database).map((wallet) => wallet.walletAddress),
      ['NEWER'],
    );
  } finally {
    database.close();
...
test('REGRESSION: a wallet from an older roster cannot leak into the default report', () => {
  ...
  try {
    seedRankSnapshot(database, [
      { wallet_address: 'DROPPED', name: 'was ranked', tags: ['wash_trader'] },
    ]);
    syncCopyTradeRoster(database);
    ...
    const report = computeCopyTradeReport(database, { now });
    assert.deepEqual(
      report.rows.map((r) => r.walletAddress),
      ['CURRENT'],
    );
    assert.equal(
      report.overall.trades,
      1,
      'the dropped wallet must not reach the pooled row either',
    );
  } finally {
    database.close();
  }
  ...
  try {
    ...
    for (let i = 0; i < 120; i += 1) {
      insertTrade(database, {
        wallet: 'S',
        tx_hash: `v${i}`,
        timestamp: T - 3600 - i * 60,
        cost_usd: '110',
        buy_cost_usd: '100',
        token_amount: String(i),
      });
    }
    ...
    assert.ok(
      (row.coveredDays ?? 0) < 1,
      `span must come from usable trades only, got ${row.coveredDays}`,
    );
...
test('REGRESSION: a truncated wallet cannot receive a positive verdict', () => {
  ...
  try {
    ...
    assert.equal(
      row.verdict,
      'descriptive_only',
      'truncated history is descriptive only and cannot be compared',
    );
...
test('REGRESSION: unparseable risk flags are treated as risk, not as safety', () => {
  ...
  try {
    ...
    syncCopyTradeRoster(database);
    database
      .prepare(`UPDATE copytrade_wallets SET risk_flags = '{not json' WHERE wallet_address = 'W'`)
      .run();
...
test('REGRESSION: a saved snapshot freezes the same scope the report was computed under', () => {
  ...
  try {
    ...
    const stored = database
      .prepare(
        'SELECT params_json AS p, report_json AS r FROM copytrade_result_snapshots WHERE id = ?',
      )
      .get(saved.snapshotId) as { p: string; r: string };

    assert.deepEqual(
      JSON.parse(stored.r),
      displayed,
      'the frozen report is byte-identical to the displayed one',
    );
...
test('evaluate: a risk flag comes with measured evidence, not just the tag', () => {
  ...
  try {
    seedRankSnapshot(database, [
      { wallet_address: 'W', name: 'flagged one', tags: ['wash_trader'] },
    ]);
    syncCopyTradeRoster(database);
    database
      .prepare(
    ...
    const row = computeCopyTradeReport(database, { now }).rows[0];
    assert.equal(
      row.riskEvidence.fastRoundTripPercent,
      100,
      'both measurable round trips closed inside a minute',
    );
    assert.equal(
      row.riskEvidence.under15SecondsPercent,
      100,
      'both measurable round trips closed within 15 seconds',
    );
    assert.equal(row.riskEvidence.under15SecondsCount, 2);
    assert.equal(row.riskEvidence.pairedTradeCount, 2);
    assert.equal(
      row.riskEvidence.noCostBasisPercent,
      33.3,
      '1 of 3 sells had no recorded purchase',
    );
    ...
    assert.match(
      notes,
      /does not publish how it decides/,
      'the tag is presented as an opinion, not proof',
    );
...
test('evaluate: median resists the outlier that swings the average', () => {
  ...
  const trades = [
    { timestamp: 1, returnRatio: -0.5 },
    { timestamp: 2, returnRatio: -0.5 },
    { timestamp: 3, returnRatio: -0.5 },
    { timestamp: 4, returnRatio: -0.5 },
...
test('evaluate: sequential compounding is time-ordered and a total loss is terminal', () => {
  ...
  assert.equal(
    compoundCapital(
      [
        { timestamp: 1, returnRatio: 0.1 },
        { timestamp: 2, returnRatio: 0.1 },
      ],
      100,
    ),
    121,
  ...
  assert.equal(
    compoundCapital(
      [
        { timestamp: 1, returnRatio: -1 },
        { timestamp: 2, returnRatio: 100 },
      ],
      100,
    ),
    0,
  ...
  assert.equal(
    compoundCapital([{ timestamp: 1, returnRatio: -5 }], 100),
    0,
    'capital can never go negative',
  );
...
test('evaluate: the headline figure is equal-weight, which one outlier cannot explode', () => {
  ...
  assert.equal(
    summarizeTrades(skewed).medianReturnPercent,
    -50,
    'median still reports the typical trade',
  );
});
...
test('evaluate: every verdict gate is enforced, and risk outranks a thin sample', () => {
  ...
  assert.equal(
    decideVerdict({ ...passing, medianReturnPercent: 0 }).verdict,
    'no',
    'zero median is not positive',
  );
...
test('evaluate: buys are ignored and only trades inside the period are used', () => {
  ...
  try {
    ...
    insertTrade(database, {
      ...
      tx_hash: 'S2',
      event_type: 'sell',
      timestamp: nowSeconds - 60 * 86_400,
      cost_usd: '10',
      buy_cost_usd: '100',
    });
...
test('a saved snapshot freezes the report it was computed from', () => {
  ...
  try {
    ...
    const stored = database
      .prepare('SELECT report_json AS json FROM copytrade_result_snapshots WHERE id = ?')
      .get(saved.snapshotId) as { json: string };
...
test('evaluate: trade-weighted and wallet-weighted overalls are both reported and differ', () => {
  ...
  try {
    ...
    assert.equal(
      report.overall.medianReturnPercent,
      -10,
      'pooled median is BUSY’s, by weight of numbers',
    );

    assert.equal(report.overallByWallet.weighting, 'wallet-weighted');
    assert.equal(report.overallByWallet.wallets, 2);
    assert.equal(
      report.overallByWallet.trades,
      21,
      'trade count is the same population, only the weighting differs',
    );
...
test('coverage history keeps one immutable event per wallet per run while latest state stays current', () => {
  ...
  try {
    ...
    const latestCount = database
      .prepare('SELECT COUNT(*) AS count FROM copytrade_wallet_coverage WHERE wallet_address = ?')
      .get('WALLET_A') as { count: number };
    assert.equal(latestCount.count, 1);
    const latest = database
      .prepare(
        'SELECT last_run_id AS runId, truncated, stop_reason AS stopReason FROM copytrade_wallet_coverage WHERE wallet_address = ?',
      )
      .get('WALLET_A') as { runId: number; truncated: number; stopReason: string };
...
test('coverage history preserves a truncated event after a later complete run', () => {
  ...
  try {
    ...
    assert.deepEqual(
      { truncated: earlier?.truncated, stopReason: earlier?.stopReason },
      { truncated: true, stopReason: 'request_cap' },
    );
    assert.deepEqual(
      { truncated: later?.truncated, stopReason: later?.stopReason },
      { truncated: false, stopReason: 'window_covered' },
    );
  } finally {
    database.close();
  }
...
test('coverage history filters by wallet and run without clamping the requested limit', () => {
  ...
  try {
    ...
    for (let index = 0; index < 505; index += 1) {
      ...
      recordCoverage(database, {
        walletAddress: index === 504 ? 'WALLET_B' : 'WALLET_A',
        chain: 'sol',
        runId,
    ...
    assert.ok(
      listWalletCoverageHistory(database, { walletAddress: 'WALLET_B' }).every(
        (event) => event.walletAddress === 'WALLET_B',
      ),
    );
    ...
    assert.ok(bEvent);
    assert.deepEqual(
      listWalletCoverageHistory(database, { runId: bEvent.runId }).map((event) => event.id),
      [bEvent.id],
    );
  } finally {
    database.close();
...
test('recordCoverage persists a resume cursor for a truncated wallet and clears it once the window is covered', () => {
  ...
  try {
    ...
    recordCoverage(database, {
      ...
      periodDays: 90,
      stopReason: 'request_cap',
      resumeCursor: 'CURSOR_AT_REQUEST_CAP',
    });
    const afterFirst = database
      .prepare(
        'SELECT resume_cursor AS resumeCursor FROM copytrade_wallet_coverage WHERE wallet_address = ?',
      )
      .get('WALLET_A') as { resumeCursor: string | null };
    assert.equal(
      afterFirst.resumeCursor,
      'CURSOR_AT_REQUEST_CAP',
      'a truncated wallet must save where to resume next time',
    );
    ...
    const afterSecond = database
      ...
      .get('WALLET_A') as { resumeCursor: string | null };
    assert.equal(
      afterSecond.resumeCursor,
      null,
      'once the window is fully covered there is nothing left to resume',
    );
  } finally {
    database.close();
  }
...
test('storeActivityPage: a daily cap stops inserting once a calendar day has enough trades, without touching other days', () => {
  ...
  try {
    ...
    const storedDay1 = storeActivityPage(database, day1, {
      chain: 'sol',
      fetchedAt: 'now',
      dailyCap,
    });
    assert.equal(
      storedDay1.inserted,
      2,
      'only the first two trades of the day are stored, matching the cap',
    );
    assert.equal(
      storedDay1.dailyCapped,
      2,
      'the remaining two are explicitly counted as capped, not silently dropped',
    );
    assert.equal(
      storedDay1.duplicates,
      0,
      'capped trades are not duplicates — they were never seen before',
    );

    const day2Start = day1Start + 86_400;
    const day2 = [
      activity({ tx_hash: 'D2_A', timestamp: day2Start + 1 }),
      activity({ tx_hash: 'D2_B', timestamp: day2Start + 2 }),
    ];
    const storedDay2 = storeActivityPage(database, day2, {
      chain: 'sol',
      fetchedAt: 'now',
      dailyCap,
    });
    assert.equal(
      storedDay2.inserted,
      2,
      'a new calendar day gets its own fresh cap, unaffected by the previous day being full',
    );

    const totalStored = database
      .prepare('SELECT COUNT(*) AS count FROM copytrade_trades')
      .get() as { count: number };
...
test('storeActivityPage: the daily cap is seeded from trades already stored, so a resumed run does not exceed it', () => {
  ...
  try {
    ...
    const dailyCap = { limit: 2, countsByDay: new Map<string, number>() };
    const stored = storeActivityPage(
      database,
    ...
    assert.equal(
      stored.inserted,
      0,
      'the day already has 2 stored trades from a prior run, matching the cap of 2',
    );
...
test('storeActivityPage: re-confirming an already-stored trade on a day at its cap counts as a duplicate, not capped', () => {
  ...
  try {
    ...
    assert.equal(
      stored.duplicates,
      1,
      'OLD_A was already stored — a harmless re-confirm, not data the cap turned away',
    );
    assert.equal(
      stored.dailyCapped,
      1,
      'only NEW_C, which would have been genuinely new, was actually capped',
    );
...
test('storeActivityPage: without a dailyCap option, behavior is unchanged (uncapped, as before this feature)', () => {
  ...
  try {
    ...
    const many = Array.from({ length: 10 }, (_, index) =>
      activity({ tx_hash: `T${index}`, timestamp: dayStart + index }),
    );
...
test('leaderboard provenance preserves the exact query even when the normalized response repeats', () => {
  ...
  try {
    ...
    const first = storeWalletRankSnapshot(database, {
      ...
      requestQuery: { orderby: 'pnl_30d', winrate: '0.5', direction: 'desc' },
      rawPayload: payload,
    });
    const second = storeWalletRankSnapshot(database, {
      ...
      requestPath: '/api/v1/rank/sol/wallets/30d',
      requestQuery: { orderby: 'pnl_30d', winrate: '0.6', direction: 'desc' },
      rawPayload: payload,
    });
    ...
    assert.equal(
      (
        database
          .prepare('SELECT COUNT(*) AS count FROM gmgn_wallet_rank_capture_provenance')
          .get() as { count: number }
      ).count,
      2,
      'both request observations remain immutable',
    );
...
test('weekly and monthly performance use UTC periods and preserve every usable trade', () => {
  const trades = [
    {
      sourceId: 1,
      timestamp: Date.parse('2026-08-03T10:00:00Z') / 1000,
      returnRatio: 0.1,
      profitUsd: 10,
      tokenAddress: 'T1',
      tokenSymbol: 'ONE',
    ...
    {
      sourceId: 2,
      timestamp: Date.parse('2026-08-10T10:00:00Z') / 1000,
      returnRatio: -0.2,
      profitUsd: -20,
      tokenAddress: 'T2',
      tokenSymbol: 'TWO',
    ...
    {
      sourceId: 3,
      timestamp: Date.parse('2026-09-01T10:00:00Z') / 1000,
      returnRatio: 0.3,
      profitUsd: 30,
      tokenAddress: 'T3',
      tokenSymbol: 'THREE',
  ...
  assert.deepEqual(
    weeks.map((row) => row.period),
    ['2026-08-03', '2026-08-10', '2026-08-31'],
  );
  assert.deepEqual(
    months.map((row) => [row.period, row.trades]),
    [
      ['2026-08', 2],
      ['2026-09', 1],
    ],
