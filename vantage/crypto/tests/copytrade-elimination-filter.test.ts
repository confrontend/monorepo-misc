...
import {
  ...
  ELIMINATION_MIN_TRADES,
  ELIMINATION_MIN_DUNE_ROUND_TRIPS,
  ELIMINATION_STATS_MAX_AGE_HOURS,
  STRONGLY_NEGATIVE_PNL_PERCENT,
  TRUSTED_DUNE_COVERAGE_PERCENT,
...
const fixtureTrade = (
  status: 'simulated' | 'not_yet_queried',
...
const baseSim = (
  ...
  over: Partial<CopySimulationWalletReport> = {},
): CopySimulationWalletReport => ({
  walletAddress,
  roundTripsConsidered: 100,
  copiedTrades: 100,
  missedTrades: 0,
  coverageRatePercent: 100,
  walletMedianReturnPercent: 10,
  simulatedMedianReturnPercent: 5,
  walletMeanReturnPercent: 10,
  simulatedMeanReturnPercent: 5,
  tradesAbove100Percent: 0,
  tradesAbove300Percent: 0,
  bestSimulatedReturnPercent: 20,
  tailShareOfMeanPercent: null,
  delayCostPercentagePoints: -5,
  worstSimulatedReturnPercent: -10,
  totalGasFeeSol: 0.1,
...
test('a wallet at mid-range Dune coverage is never eliminated, however bad it looks', () => {
  const row = baseRow('MID_COVERAGE', {
    gmgnAggregate: {
      period: '30d',
      fetchedAt: '2026-08-22T00:00:00.000Z',
      realizedProfit: -1000,
      realizedProfitPnlPercent: -80,
      nativeBalance: null,
      buyCount: null,
      sellCount: null,
      boughtCost: null,
  ...
  const sim = baseSim('MID_COVERAGE', {
    coverageRatePercent: 55,
    missedTrades: 45,
    simulatedMedianReturnPercent: -30,
  });
...
test('a fully covered wallet with a strongly negative 30-day PnL is eliminated', () => {
  ...
  const row = baseRow('BAD_PNL', {
    gmgnAggregate: {
      period: '30d',
      fetchedAt: '2026-08-22T00:00:00.000Z',
      realizedProfit: -1000,
      realizedProfitPnlPercent: STRONGLY_NEGATIVE_PNL_PERCENT - 5,
      nativeBalance: null,
      buyCount: null,
      sellCount: null,
      boughtCost: null,
...
test('a fully covered wallet whose hold time is shorter than the copy delay is eliminated', () => {
  const row = baseRow('TOO_FAST', {
    riskEvidence: {
      fastRoundTripPercent: 5,
      noCostBasisPercent: 2,
      medianHoldSeconds: DEFAULT_COPIER_DELAY_SECONDS - 1,
      fundedByAddress: null,
      walletAgeDays: 300,
    },
  });
...
test('assessCoverageGap reports an ordinary-looking gap as unclear, not as a hidden problem', () => {
  const trades = [
    tradeAt('simulated', 500),
    ...Array.from({ length: 3 }, () => tradeAt('simulated', 10)),
    tradeAt('not_yet_queried', 500),
    ...Array.from({ length: 3 }, () => tradeAt('not_yet_queried', 10)),
  ];
  const result = assessCoverageGap(trades);
...
test('assessCoverageGap never infers a direction from a one-sided population', () => {
  ...
  const onlyUnmatched = assessCoverageGap(
    Array.from({ length: 5 }, () => tradeAt('not_yet_queried', 10)),
  );
...
test('assessCoverageGap excludes trades with no wallet return rather than scoring them as losses', () => {
  const trades = [
    tradeAt('simulated', 500),
    tradeAt('simulated', null),
    tradeAt('not_yet_queried', 10),
    tradeAt('not_yet_queried', null),
  ];
  const result = assessCoverageGap(trades);
  ...
  const withSim = computeEliminationReport(
    ...
    new Map([
      [
        'HAS_SIM',
        baseSim('HAS_SIM', {
          coverageRatePercent: 60,
          trades: [tradeAt('simulated', 10), tradeAt('not_yet_queried', 500)],
        }),
      ],
    ]),
  );
...
test('hidden-loss risk never flags a wallet whose unmeasured trades look better than its measured ones', () => {
  ...
  assert.ok(
    (result.lossRateUnderstatedPercentagePoints ?? 0) < 0,
    'true loss rate should be BELOW the shown rate',
  );
});

test('a fully covered wallet has negligible risk, not unknown — nothing missing is not the same as nothing measured', () => {
  const fullyCovered = assessCoverageGap(
    Array.from({ length: 5 }, () => tradeAt('simulated', -10)),
  );
  assert.equal(fullyCovered.hiddenLossRisk, 'negligible');
  assert.equal(
    fullyCovered.lossRateUnderstatedPercentagePoints,
    0,
    'with nothing unmatched the true rate IS the shown rate',
  );
  // Nothing measured at all stays genuinely unknown.
  assert.equal(assessCoverageGap([]).hiddenLossRisk, 'unknown');
  assert.equal(
    assessCoverageGap(Array.from({ length: 5 }, () => tradeAt('not_yet_queried', -10)))
      .hiddenLossRisk,
    'unknown',
  );
});
...
test('the shown-vs-true loss rates reported are the real rates a reader can check by hand', () => {
  const trades = [
    tradeAt('simulated', 10),
    tradeAt('simulated', -10), // measured: 1 of 2 lose = 50%
    tradeAt('not_yet_queried', -10),
    tradeAt('not_yet_queried', -10), // all 4 together: 3 of 4 = 75%
  ];
  const result = assessCoverageGap(trades);
...
const statsAt = (fetchedAt: string, pnlPercent: number): CopyTradeRow['gmgnAggregate'] => ({
  ...
  fetchedAt,
  realizedProfit: -1000,
  realizedProfitPnlPercent: pnlPercent,
  nativeBalance: null,
  buyCount: null,
  sellCount: null,
  boughtCost: null,
...
test('a wallet whose GMGN history fetch FAILED is never trustworthy, even at full Dune coverage', () => {
  ...
  const sim = baseSim('HISTORY_FAILED', {
    coverageRatePercent: 100,
    simulatedMedianReturnPercent: -1,
  });
...
test('100% Dune coverage over too few round trips is not enough to eliminate', () => {
  ...
  const thin = baseSim('THIN_SAMPLE', {
    coverageRatePercent: 100,
    simulatedMedianReturnPercent: -1,
    roundTripsConsidered: ELIMINATION_MIN_DUNE_ROUND_TRIPS - 1,
    copiedTrades: ELIMINATION_MIN_DUNE_ROUND_TRIPS - 1,
  });
  assert.equal(
    computeEliminationReport([row], new Map([['THIN_SAMPLE', thin]])).eliminated.length,
    0,
  );
  const enough = baseSim('THIN_SAMPLE', {
    coverageRatePercent: 100,
    simulatedMedianReturnPercent: -1,
    roundTripsConsidered: ELIMINATION_MIN_DUNE_ROUND_TRIPS,
    copiedTrades: ELIMINATION_MIN_DUNE_ROUND_TRIPS,
  });
  assert.equal(
    computeEliminationReport([row], new Map([['THIN_SAMPLE', enough]])).eliminated.length,
    1,
  );
});
...
test('a stale GMGN stats snapshot cannot drive the negative-PnL elimination', () => {
  ...
  const staleAt = new Date(
    now.getTime() - (ELIMINATION_STATS_MAX_AGE_HOURS + 1) * 3_600_000,
  ).toISOString();
  const freshAt = new Date(now.getTime() - 1 * 3_600_000).toISOString();
  const sim = baseSim('W', { coverageRatePercent: 100 });
  const stale = computeEliminationReport(
    [baseRow('W', { gmgnAggregate: statsAt(staleAt, STRONGLY_NEGATIVE_PNL_PERCENT - 50) })],
    new Map([['W', sim]]),
    now,
  );
  assert.equal(stale.eliminated.length, 0, 'stale stats must not eliminate');
  const fresh = computeEliminationReport(
    [baseRow('W', { gmgnAggregate: statsAt(freshAt, STRONGLY_NEGATIVE_PNL_PERCENT - 50) })],
    new Map([['W', sim]]),
    now,
  );
...
test('the triage trade bar is intentionally forked below the shared candidacy bar, and stays high enough to exclude thin samples', () => {
  ...
  assert.ok(
    ELIMINATION_MIN_TRADES < RULES.minTrades,
    'triage bar must stay below the candidacy bar',
  );
  assert.ok(
    ELIMINATION_MIN_TRADES >= 50,
    'must stay high enough to exclude the four-trade-winner failure mode',
  );

  const clean = (address: string) =>
    baseSim(address, {
      coverageRatePercent: 95,
      simulatedMedianReturnPercent: -5,
      roundTripsConsidered: 60,
      copiedTrades: 57,
    });
  const justUnder = computeEliminationReport(
    [baseRow('THIN', { trades: ELIMINATION_MIN_TRADES - 1 })],
    new Map([['THIN', clean('THIN')]]),
  );
  assert.equal(
    justUnder.surviving[0]?.trustworthy,
    false,
    'one trade under the bar is not judgeable',
  );
  const atBar = computeEliminationReport(
    [baseRow('OK', { trades: ELIMINATION_MIN_TRADES })],
    new Map([['OK', clean('OK')]]),
  );
...
test('"needs Dune" excludes a wallet that more Dune cannot help', () => {
  ...
  const complete = baseSim('DONE', {
    coverageRatePercent: 100,
    missedTrades: 0,
    roundTripsConsidered: 60,
    copiedTrades: 60,
  });
  const report = computeEliminationReport(
    [baseRow('DONE', { trades: ELIMINATION_MIN_TRADES - 1 })],
    new Map([['DONE', complete]]),
  );
  assert.equal(report.surviving[0]?.trustworthy, false);
