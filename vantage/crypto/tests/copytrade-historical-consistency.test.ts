...
import {
  ...
  type HistoricalConsistencyTrade,
...
test('historical consistency marks under-30-day wallets insufficient', () => {
  const report = computeHistoricalConsistency([sell('SHORT', 20, 25), sell('SHORT', 2, 10)], NOW);
...
test('a wallet with real data in both periods that is simply negative both times is consistently_negative, not insufficient', () => {
  const report = computeHistoricalConsistency(
    [sell('BAD', 80, -10), sell('BAD', 45, -5), sell('BAD', 20, -20), sell('BAD', 4, -15)],
    NOW,
  );
  const row = report.rows[0];
  ...
  assert.notEqual(
    row.verdict,
    'insufficient',
    'plenty of real data exists in both periods — this must never be confused with "not enough data to judge"',
  );
