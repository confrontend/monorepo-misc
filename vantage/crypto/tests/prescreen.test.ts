...
const seedSignals = (
  ...
  const insert = database.prepare(
    'INSERT INTO gmgn_signals (id, token_address, signal_type, observed_at, raw_payload, captured_at) VALUES (?, ?, ?, ?, ?, ?)',
  );
  for (const row of candidates)
    insert.run(row.id, row.tokenAddress, row.signalType, row.observedAt, '{}', row.capturedAt);
};
...
test('pre-screen selects only the lifetime-first token/type observation and preserves later repeats as deferred', () => {
  ...
  try {
    ...
    assert.notEqual(
      result.decisions.find((row) => row.signalId === 2)?.disposition,
      'eligible_core',
    );
    assert.notEqual(
      result.decisions.find((row) => row.signalId === 3)?.disposition,
      'eligible_core',
    );
    assert.equal(
      (
        database.prepare('SELECT COUNT(*) AS count FROM dune_measurement_prescreen').get() as {
          count: number;
        }
      ).count,
      4,
  ...
  try {
    const candidates = Array.from({ length: 20 }, (_, index) =>
      candidate(
        ...
        `Token${index + 1}`,
        String((index % 2) + 1),
        `2026-08-${String((index % 3) + 1).padStart(2, '0')}T01:00:00.000Z`,
      ),
    );
    seedSignals(database, candidates);
...
test('pre-screen prioritizes never-measured rows before retries', () => {
  const database = openDatabase(':memory:');
  try {
    const newRows = Array.from({ length: 500 }, (_, index) =>
      candidate(
        index + 1,
        `NewToken${index + 1}`,
        '7',
        `2026-08-${String((index % 10) + 1).padStart(2, '0')}T01:00:00.000Z`,
      ),
    );
    const retryRows = Array.from({ length: 100 }, (_, index) => ({
      ...candidate(
        index + 501,
        `RetryToken${index + 1}`,
        '7',
        `2026-08-${String((index % 10) + 1).padStart(2, '0')}T02:00:00.000Z`,
      ),
      plannerState: 'retry_eligible' as const,
    }));
...
test('retry queue applies lifetime-first and required-field screening independently of new-row budget', () => {
  const candidates: PrescreenCandidate[] = [
    {
      ...candidate(1, 'RetryToken', '7', '2026-08-01T01:00:00.000Z'),
      plannerState: 'retry_eligible',
    },
    {
      ...candidate(2, 'RetryToken', '7', '2026-08-01T02:00:00.000Z'),
      plannerState: 'retry_eligible',
    },
    {
      ...candidate(3, 'Malformed', '', '2026-08-01T03:00:00.000Z'),
      plannerState: 'retry_eligible',
    },
    {
      ...candidate(4, 'FreshToken', '7', '2026-08-01T04:00:00.000Z'),
      plannerState: 'pending_target_time',
    },
    {
      ...candidate(5, 'OtherRetry', '8', '2026-08-02T01:00:00.000Z'),
      plannerState: 'retry_eligible',
    },
  ];
...
test('too_fresh candidates are never selected and get an explicit audit reason with eligible_at', () => {
  ...
  try {
    const candidates: PrescreenCandidate[] = [
      {
        ...candidate(1, 'TooFreshToken', '7', '2026-08-13T12:00:00.000Z'),
        plannerState: 'too_fresh',
      },
    ];
    seedSignals(database, candidates);
    const result = runPrescreen(database, candidates, new Date('2026-08-14T00:00:00.000Z'));
    assert.ok(
      !result.selectedIds.includes(1),
      'a too_fresh candidate must never be selected for Dune submission',
    );
    ...
    assert.match(
      decision.reason,
      /eligible_at: 2026-08-14T12:00:00\.000Z/,
      'eligible_at must be exactly observed_at + 24h',
    );
...
test('retry queue re-checks the 24-hour age boundary even when planner state is stale', () => {
  const candidates: PrescreenCandidate[] = [
    {
      ...candidate(1, 'FreshRetry', '7', '2026-08-13T12:01:00.000Z'),
      plannerState: 'retry_eligible',
    },
    {
      ...candidate(2, 'MatureRetry', '7', '2026-08-13T00:00:00.000Z'),
      plannerState: 'retry_eligible',
    },
  ];
  assert.deepEqual(
    selectRetryQueueIds(candidates, Number.MAX_SAFE_INTEGER, new Date('2026-08-14T00:00:00.000Z')),
    [2],
  );
});
