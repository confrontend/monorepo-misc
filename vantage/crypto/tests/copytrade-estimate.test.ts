...
const seedRankSnapshot = (database: DatabaseSync, rank: unknown[]): void => {
  database.prepare(
    `INSERT INTO gmgn_wallet_rank_snapshots (window, orderby, captured_at, raw_payload, source_sha256)
     VALUES (?, ?, ?, ?, ?)`,
  ).run('7d', 'pnl_30d', '2026-08-15T00:00:00.000Z', JSON.stringify({ code: 0, data: { rank } }), `sha-${Math.random()}`);
};

const seedRoster = (database: DatabaseSync, wallets: string[]): void => {
  seedRankSnapshot(database, wallets.map((w) => ({ wallet_address: w, tags: [] })));
...
const seedRun = (
  ...
  options: {
    status?: string; startedAt: string; completedAt: string | null; requestsMade: number; periodDays?: number;
  ...
  database.prepare(
    `INSERT INTO copytrade_fetch_runs
     ...
     VALUES (?, ?, ?, ?, ?, 0, ?, ?, ?)`,
  ).run(options.startedAt, options.completedAt, options.status ?? 'completed', options.events.length, options.events.length, options.requestsMade, options.periodDays ?? 30, options.events.length);
  ...
  for (const [index, event] of options.events.entries()) {
    database.prepare(
      `INSERT INTO copytrade_wallet_coverage_events
       ...
       VALUES (?, ?, 'sol', ?, ?, 0, ?, ?)`,
    ).run(runId, `W${index}`, options.periodDays ?? 30, event.requestsUsed, event.stopReason, options.completedAt ?? options.startedAt);
  }
  return runId;
};
...
test('estimate: with no completed run yet, the basis is the seeded default', () => {
  ...
  try {
    ...
    assert.equal(projection.freshWallets, 25, 'no roster captured yet, so every requested slot counts as fresh ground');
...
test('estimate: folding in one completed run switches the basis to measured and matches the run exactly', () => {
  ...
  try {
    seedRun(database, {
      startedAt: '2026-08-15T00:00:00.000Z', completedAt: '2026-08-15T00:16:40.000Z', // 1000s
      requestsMade: 100, periodDays: 30,
      events: [{ stopReason: 'window_covered', requestsUsed: 90 }, { stopReason: 'up_to_date', requestsUsed: 10 }],
    });
    ...
    assert.equal(basis.requestsPerFreshWallet, 90, 'the one non-up_to_date wallet used 90 requests');
...
test('estimate: folding in the same run twice is a no-op (idempotent by watermark)', () => {
  ...
  try {
    seedRun(database, {
      startedAt: '2026-08-15T00:00:00.000Z', completedAt: '2026-08-15T00:16:40.000Z',
      requestsMade: 100, events: [{ stopReason: 'window_covered', requestsUsed: 100 }],
    });
    assert.equal(recordFetchRunEstimate(database, 1), true);
    assert.equal(recordFetchRunEstimate(database, 1), false, 'a run at or below the watermark must be ignored');
...
test('estimate: a cancelled or failed run is never folded in, even if requested directly', () => {
  const database = setup();
  try {
    seedRun(database, { status: 'cancelled', startedAt: '2026-08-15T00:00:00.000Z', completedAt: '2026-08-15T00:05:00.000Z', requestsMade: 300, events: [{ stopReason: 'cancelled', requestsUsed: 300 }] });
    seedRun(database, { status: 'failed', startedAt: '2026-08-15T01:00:00.000Z', completedAt: '2026-08-15T01:05:00.000Z', requestsMade: 300, events: [{ stopReason: 'request_cap', requestsUsed: 300 }] });
    ...
    assert.equal(readFetchEstimateBasis(database).source, 'default', 'basis must remain seeded — no completed run exists');
  } finally { database.close(); }
});
...
test('estimate: two completed runs accumulate additively, not by replacement', () => {
  const database = setup();
  try {
    seedRun(database, { startedAt: '2026-08-15T00:00:00.000Z', completedAt: '2026-08-15T00:10:00.000Z', requestsMade: 60, events: [{ stopReason: 'window_covered', requestsUsed: 60 }] }); // 600s / 60 req = 10 s/req
    recordFetchRunEstimate(database, 1);
    seedRun(database, { startedAt: '2026-08-15T02:00:00.000Z', completedAt: '2026-08-15T02:03:20.000Z', requestsMade: 40, events: [{ stopReason: 'window_covered', requestsUsed: 40 }] }); // 200s / 40 req = 5 s/req
...
test('estimate: fresh vs covered wallet classification matches what the fetcher itself would do', () => {
  ...
  try {
    ...
    storeActivityPage(database, [{
      wallet: 'COVERED_WALLET', chain: 'sol', tx_hash: 'TX1', event_type: 'sell',
      token: { address: 'TOKEN_A', symbol: 'AAA' }, timestamp: oldTs,
      token_amount: '1', cost_usd: '1', buy_cost_usd: '1', price_usd: '1',
...
test('estimate: confidence rises with the number of completed runs folded in', () => {
  ...
  try {
    ...
    for (let i = 0; i < 2; i += 1) {
      seedRun(database, { startedAt: `2026-08-15T0${i}:00:00.000Z`, completedAt: `2026-08-15T0${i}:01:00.000Z`, requestsMade: 10, events: [{ stopReason: 'window_covered', requestsUsed: 10 }] });
...
test('estimate: remaining time for a live run prefers the run\'s own observed pace over the historical basis', () => {
  const database = setup();
  try {
    seedRun(database, { startedAt: '2026-08-15T00:00:00.000Z', completedAt: '2026-08-15T00:00:10.000Z', requestsMade: 1000, events: [{ stopReason: 'window_covered', requestsUsed: 1000 }] });
    ...
    const remaining = estimateRemainingSeconds(database, { startedAt: '2026-08-15T01:00:00.000Z', walletDone: 10, walletTotal: 15, periodDays: 30 }, now);
    assert.equal(remaining, 50, '10s/wallet observed pace x 5 wallets remaining, not the historical rate');
  } finally { database.close(); }
});
...
test('estimate: remaining time falls back to the historical basis before any wallet has finished', () => {
  ...
  try {
    ...
    const remaining = estimateRemainingSeconds(database, { startedAt: '2026-08-15T00:00:00.000Z', walletDone: 0, walletTotal: 3, periodDays: 30 }, now);
    assert.ok(remaining !== null && remaining > 0, 'must fall back to the seeded/measured basis, not null, before the first wallet completes');
  } finally { database.close(); }
});
