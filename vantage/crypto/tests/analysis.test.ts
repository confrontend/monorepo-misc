...
test('snapshot analysis is descriptive and excludes raw payloads', () => {
  ...
  try {
    database
      .prepare(
        `INSERT INTO tokens (token_address, source, imported_at, raw_payload, validation_errors) VALUES (?, ?, ?, ?, ?)`,
      )
      .run('TokenA', 'dune', '2026-03-01T00:00:00Z', '{}', '[]');
    storeGmgnSignal(
      database,
      {
        id: 'a',
        ...
        signal_type: 13,
        observed_at: '2026-03-01T00:01:00Z',
        market_cap: 100,
      },
      {
        source: 'gmgn-browser-extension',
        chain: 'sol',
        capturedAt: new Date('2026-03-01T00:01:01Z'),
      },
    );
    storeGmgnSignal(
      database,
      {
        ...
        token_address: 'TokenA',
        signal_type: 14,
        observed_at: '2026-03-01T00:02:00Z',
        market_cap: 300,
      },
      {
        source: 'gmgn-browser-extension',
        chain: 'sol',
        capturedAt: new Date('2026-03-01T00:02:01Z'),
      },
    );
    storeGmgnSignal(
      database,
      { id: 'c', token_address: 'TokenB', signal_type: 13, observed_at: '2026-03-01T00:03:00Z' },
      { source: 'gmgn-cli', chain: 'sol', capturedAt: new Date('2026-03-01T00:03:01Z') },
    );
    ...
    assert.deepEqual(report.sources, [
      { source: 'gmgn-browser-extension', count: 2 },
      { source: 'gmgn-cli', count: 1 },
    ]);
    assert.equal(JSON.stringify(report).includes('raw_payload'), false);
    assert.equal(
      report.limitations.some((item) => /returns|scoring/i.test(item)),
      true,
    );
  } finally {
    database.close();
