      ...
      [
        {
          ...
          price_usd: 1.2,
          matched_trade_at: '2026-03-01 00:05:00.000 UTC',
          matched_tx_id: 'tx-1',
        },
      ],
...
test('a received checkpoint from a pre-buffer query is marked for re-verification without changing raw evidence', () => {
  ...
  try {
      ...
      [
        {
          ...
          target_at: '2026-03-01 00:05:00.000 UTC',
          price_usd: 12,
          matched_trade_at: '2026-03-01 00:04:00.000 UTC',
          matched_tx_id: 'early-tx',
        },
      ],
    ...
    const before = (
      database.prepare('SELECT raw_result AS rawResult FROM dune_outcome_runs').get() as {
        rawResult: string;
      }
    ).rawResult;
    const [outcome] = readAllDuneOutcomes(database);
...
test('a checkpoint whose target time is after the run completed is invalidated, not trusted', () => {
  ...
  try {
      ...
      [
        {
          signal_id: a1,
          checkpoint: '+30m',
          target_at: '2026-03-01 00:30:00.000 UTC',
          price_usd: 99,
          matched_trade_at: '2026-03-01 00:01:59.000 UTC',
          matched_tx_id: 'tx-bogus',
        },
      ],
    ...
    assert.equal(
      ...
      null,
      'the raw price must never be surfaced as if it were validated',
    );
    assert.equal(checkpoint.result.status, 'checkpoint not yet reached');
    assert.equal(
      checkpoint.result.matchedTradeAt,
      null,
      'the matched-trade fields are cleared too, since that trade is not meaningfully "the +30m trade"',
    );
    assert.equal(checkpoint.result.matchedTxId, null);
  } finally {
    database.close();
...
test('an unparseable target_at or missing completed_at is treated as pending, never as a validated price (fail-safe)', () => {
  ...
  try {
    ...
    insertOutcomeRun(
      ...
      null,
    ); // completed_at missing/null on this run
    const outcomes = readAllDuneOutcomes(database);
    const garbage = outcomes
      .find((o) => o.signal.id === a1)!
      .checkpoints.find((c) => c.label === '+5m')!;
    const missingCompletedAt = outcomes
      .find((o) => o.signal.id === a2)!
      .checkpoints.find((c) => c.label === '+5m')!;
      ...
      [
        {
          signal_id: a1,
          checkpoint: '+1h',
          target_at: '2026-03-01 01:00:00.000 UTC',
          price_usd: 42,
        },
      ],
      '2026-03-01T00:10:00.000Z',
...
test('the stored raw_result is never modified, even for a run containing an invalidated checkpoint', () => {
  ...
  try {
    ...
    assert.equal(
      afterRead,
      beforeRead,
      'reading outcomes must never mutate the archived raw response',
    );
    assert.equal(
      JSON.parse(afterRead).result.rows[0].price_usd,
      99,
      'the raw stored value stays 99 even though the interpreted result nulls it out',
    );
  } finally {
    database.close();
  }
    ...
    insertOutcomeRun(
      ...
      [
        ...
        },
        {
          ...
          target_at: '2026-03-01 01:00:00.000 UTC',
          price_usd: 2,
          matched_trade_at: '2026-03-01 01:00:00.000 UTC',
        },
      ],
...
test('measureDuneOutcomes refuses a signal already attached to an in-flight run', async () => {
  ...
  try {
    ...
    database
      .prepare(
        `INSERT INTO dune_outcome_runs (signal_ids, query_sql, status, requested_at) VALUES (?, 'select 1', 'running', ?)`,
      )
