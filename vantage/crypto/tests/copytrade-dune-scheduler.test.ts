...
import {
  activeDuneExecutionCount,
  activeExecutionCount,
  allowedApplicationExecutions,
  canSubmitExecution,
  latestReportedLimit,
  waitForDuneCapacity,
  withDuneSubmissionLock,
} from '../src/copytrade/simulation/duneScheduler.js';
...
test('capacity wait drains queued work only after an active execution ends', async () => {
  ...
  database
    .prepare(
      `INSERT INTO copytrade_copy_simulation_runs (trade_refs, query_sql, status, requested_at, dune_max_inflight_interactive_executions)
    VALUES ('[1]', 'SELECT 1', 'running', 'now', 3), ('[2]', 'SELECT 2', 'running', 'now', 3)`,
    )
    .run();
  let reconciled = 0;
  const seen: number[] = [];
  const wait = waitForDuneCapacity(database, {
    pollMs: 1,
    onCapacity: ({ active }) => seen.push(active),
    reconcile: async () => {
      reconciled += 1;
      const active = database
        .prepare(`SELECT id FROM copytrade_copy_simulation_runs WHERE status = 'running' LIMIT 1`)
        .get() as { id: number };
      database
        .prepare(`UPDATE copytrade_copy_simulation_runs SET status = 'completed' WHERE id = ?`)
        .run(active.id);
    },
  });
  await wait;
...
test('restart recovery treats submitted/running executions as covered and never resubmits their trades', () => {
  ...
  database
    .prepare(
      `INSERT INTO copytrade_copy_simulation_runs (trade_refs, query_sql, status, requested_at, execution_id)
    VALUES ('[10,11]', 'SELECT 1', 'submitted', 'now', 'exec-1'), ('[12]', 'SELECT 2', 'running', 'now', 'exec-2')`,
    )
    .run();
  assert.deepEqual(
    [...alreadyCoveredTradeIds(database)].sort((a, b) => a - b),
    [10, 11, 12],
  );
});
...
test('submission lock serializes concurrent capacity-check/submission critical sections', async () => {
  ...
  const second = withDuneSubmissionLock(async () => {
    order.push('second');
  });
