...
const baseRow = (walletAddress: string, over: Partial<CopyTradeRow> = {}): CopyTradeRow => ({
  ...
  profitConcentration: {
    ...
    bestTradeProfitUsd: 100, excludingBestTrade: { trades: 199, medianReturnPercent: 9, endingCapitalUsd: 108 },
...
const report = (rows: CopyTradeRow[]): CopyTradeReport => ({
  computedAt: '2026-08-15T00:00:00.000Z', startingCapitalUsd: 100, periodDays: 90, rows,
  overall: { trades: 0, winRatePercent: null, medianReturnPercent: null, averageReturnPercent: null, endingCapitalUsd: null, endingCapitalUsdCompounded: null, unreliableReason: null, weighting: 'trade-weighted', wallets: rows.length },
  overallByWallet: { trades: 0, winRatePercent: null, medianReturnPercent: null, averageReturnPercent: null, endingCapitalUsd: null, endingCapitalUsdCompounded: null, unreliableReason: null, weighting: 'wallet-weighted', wallets: rows.length },
  rules: { minTrades: 100, minDays: 7, requiresPositiveMedian: true },
  scope: { chain: 'sol', traderLimit: rows.length, rosterSnapshotId: null, rosterSize: rows.length, methodologyVersion: 'test', rosterProvenance: null },
...
const hcRow = (walletAddress: string, verdict: HistoricalConsistencyVerdict) => ({
  walletAddress, availableDays: 90, split: 'fixed_60_30' as const, splitPointAt: null,
  early: { label: 'early' as const, startAt: null, endAt: null, trades: 50, summary: { trades: 50, winRatePercent: 60, medianReturnPercent: 8, averageReturnPercent: 8, endingCapitalUsd: 108, endingCapitalUsdCompounded: 108 }, weeklyPerformance: [], monthlyPerformance: [], weeklyConsistency: { positivePeriods: 4, periodsWithData: 4, positivePercent: 100 }, profitConcentration: { bestToken: null, bestThreeTokens: [], bestTokenSharePositiveProfitPercent: null, bestThreeSharePositiveProfitPercent: null, bestTradeProfitUsd: null, excludingBestTrade: { trades: 0, medianReturnPercent: null, endingCapitalUsd: null }, excludingBestToken: { trades: 0, medianReturnPercent: null, endingCapitalUsd: null } } },
  recent: { label: 'recent' as const, startAt: null, endAt: null, trades: 50, summary: { trades: 50, winRatePercent: 60, medianReturnPercent: 8, averageReturnPercent: 8, endingCapitalUsd: 108, endingCapitalUsdCompounded: 108 }, weeklyPerformance: [], monthlyPerformance: [], weeklyConsistency: { positivePeriods: 4, periodsWithData: 4, positivePercent: 100 }, profitConcentration: { bestToken: null, bestThreeTokens: [], bestTokenSharePositiveProfitPercent: null, bestThreeSharePositiveProfitPercent: null, bestTradeProfitUsd: null, excludingBestTrade: { trades: 0, medianReturnPercent: null, endingCapitalUsd: null }, excludingBestToken: { trades: 0, medianReturnPercent: null, endingCapitalUsd: null } } },
  verdict,
});
...
const hcReport = (rows: ReturnType<typeof hcRow>[]): HistoricalConsistencyReport => ({
  ...
  counts: { consistent: 0, declining: 0, recent_only: 0, consistently_negative: 0, insufficient: 0 },
  rows,
});
...
const survives = (...walletAddresses: string[]): Map<string, CopySimulationSurvivalInput> =>
  new Map(walletAddresses.map((walletAddress) => [walletAddress, { simulatedMedianReturnPercent: 10, coverageRatePercent: 100 }]));

test('a wallet passing every gate becomes a winner, with the GMGN profile URL built from its address', () => {
  const result = computeCopyCandidates(report([baseRow('W1')]), hcReport([hcRow('W1', 'consistent')]), survives('W1'));
...
test('requires historical-consistency verdict exactly "consistent" — recent_only and declining are excluded', () => {
  ...
  assert.deepEqual(result.candidates.map((c) => c.walletAddress), ['W3']);
});

test(`a wallet with a median hold under ${MIN_MEDIAN_HOLD_SECONDS}s is excluded even with a great return`, () => {
  const result = computeCopyCandidates(
    report([baseRow('FAST', { medianReturnPercent: 500, riskEvidence: { fastRoundTripPercent: 1, noCostBasisPercent: 0, medianHoldSeconds: MIN_MEDIAN_HOLD_SECONDS - 1, fundedByAddress: null, walletAgeDays: 300 } })]),
...
test(`a wallet with fast-round-trip over ${MAX_FAST_ROUND_TRIP_PERCENT}% is excluded`, () => {
  const result = computeCopyCandidates(
    report([baseRow('MECH', { riskEvidence: { fastRoundTripPercent: MAX_FAST_ROUND_TRIP_PERCENT + 1, noCostBasisPercent: 0, medianHoldSeconds: 3600, fundedByAddress: null, walletAgeDays: 300 } })]),
...
test(`a wallet whose best token is over ${MAX_CONCENTRATION_PERCENT}% of profit is excluded`, () => {
  const result = computeCopyCandidates(
    report([baseRow('LUCKY', { profitConcentration: { bestToken: { tokenAddress: 'X', tokenSymbol: 'X', trades: 1, profitUsd: 1 }, bestThreeTokens: [], bestTokenSharePositiveProfitPercent: MAX_CONCENTRATION_PERCENT + 1, bestThreeSharePositiveProfitPercent: 90, bestTradeProfitUsd: 1, excludingBestTrade: { trades: 0, medianReturnPercent: null, endingCapitalUsd: null }, excludingBestToken: { trades: 0, medianReturnPercent: null, endingCapitalUsd: null } } })]),
...
test('winners are sorted by median return descending, after every gate already qualified them', () => {
  const result = computeCopyCandidates(
    report([baseRow('LOW', { medianReturnPercent: 5 }), baseRow('HIGH', { medianReturnPercent: 50 }), baseRow('MID', { medianReturnPercent: 20 })]),
    hcReport([hcRow('LOW', 'consistent'), hcRow('HIGH', 'consistent'), hcRow('MID', 'consistent')]),
    survives('LOW', 'HIGH', 'MID'),
  );
  assert.deepEqual(result.candidates.map((c) => c.walletAddress), ['HIGH', 'MID', 'LOW']);
});
...
test('a fast, mechanical, but high-return wallet ranks behind a slower, cleaner one that qualified — speed can never win by outranking a gate', () => {
  const result = computeCopyCandidates(
    report([
      baseRow('FAST_HIGH', { medianReturnPercent: 500, riskEvidence: { fastRoundTripPercent: 60, noCostBasisPercent: 0, medianHoldSeconds: 10, fundedByAddress: null, walletAgeDays: 300 } }),
  ...
  assert.deepEqual(result.candidates.map((c) => c.walletAddress), ['SLOW_MODEST'], 'FAST_HIGH must be gated out entirely, never merely ranked lower');
});

test('a wallet missing from the historical-consistency report at all is excluded, not silently passed', () => {
  const result: CopyCandidatesReport = computeCopyCandidates(report([baseRow('NO_HC')]), hcReport([]), survives('NO_HC'));
  assert.equal(result.candidates.length, 0);
});

test('screenedCount reflects the full report population, independent of how many became winners', () => {
  const result = computeCopyCandidates(
    report([baseRow('W1'), baseRow('W2', { verdict: 'thin' as Verdict }), baseRow('W3', { verdict: 'flagged' as Verdict })]),
    hcReport([hcRow('W1', 'consistent')]),
    survives('W1'),
  );
...
test('a wallet passing every other gate is excluded by default when no copy-simulation data is passed at all — the gate fails closed', () => {
  const result = computeCopyCandidates(report([baseRow('W1')]), hcReport([hcRow('W1', 'consistent')]));
  assert.equal(result.candidates.length, 0, 'omitting the copy-simulation map entirely must not silently pass everyone');
});
