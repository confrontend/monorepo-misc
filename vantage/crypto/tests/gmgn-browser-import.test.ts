...
const exportJson = (
  ...
  JSON.stringify({
    ...
    captures: [
      {
        capturedAt: '2026-03-01T12:00:00.250Z',
        requestPath: '/vas/api/v1/token-signal',
        status: 200,
...
const event = (id: string, token = 'TokenBrowser') => ({
  id,
  data: { chain: 'sol' },
  token_address: token,
  signal_type: 14,
  ...
  market_cap: 1234,
  triggering_wallet: 'WalletBrowser',
  raw_wallet_labels: ['smart-money'],
  source_url: 'https://gmgn.example/signal',
});
...
const websocketExportJson = (items: unknown[]) =>
  JSON.stringify({
    formatVersion: 1,
    exportedAt: '2026-03-01T12:00:00.000Z',
    extensionVersion: '0.1.0',
    ...
    captures: [
      {
        ...
        requestPath: '/ws',
        status: 200,
        responseBody: { channel: 'token_signal', data: items },
      },
    ],
  });
...
test('browser import stores source-tagged events, preserves raw source, and archives the upload', () => {
  ...
  try {
      ...
      [
        {
          ...
          lastHeartbeatAt: '2026-03-01T11:30:00.000Z',
          closedReason: null,
        },
      ],
    );
    const result = importGmgnBrowserCapture(
      database,
      'signals.json',
      raw,
      new Date('2026-03-01T12:01:00Z'),
    );
    ...
    const window = database
      .prepare(
        'SELECT started_at AS startedAt, ended_at AS endedAt, closed_reason AS closedReason FROM gmgn_browser_coverage_windows',
      )
    ...
    const row = database
      .prepare('SELECT source, chain, source_event_id, raw_payload, captured_at FROM gmgn_signals')
    ...
    const batch = database
      .prepare('SELECT status, raw_source, archive_path FROM gmgn_browser_import_batches')
      .get() as Record<string, string>;
...
test('browser import maps real token_signal WebSocket events and skips live tick items', () => {
  const database = openDatabase(':memory:');
  try {
    const raw = websocketExportJson([
      { c: 'sol', d: { a: 'tick-token', mc: 10 }, sig_op_t: 'create' },
      wsSignal,
    ]);
    ...
    assert.equal(
      result.errors,
      1,
      'one imported event with optional-field warnings counts as one error row',
    );
    const row = database
      .prepare(
        'SELECT token_address, signal_type, trigger_at, observed_at, market_cap, source_event_id, chain, raw_payload FROM gmgn_signals',
      )
...
test('browser import never parses non-signal captures (price candles, trades, holder stats, smart-money wallet data) as signal events', () => {
  ...
  try {
    const raw = exportJson(
      [event('browser-signal')],
      [],
      [
        {
          capturedAt: '2026-03-01T12:00:00.300Z',
          requestPath: '/api/v1/token_mcap_candles/sol/TokenBrowser',
          status: 200,
          responseBody: { data: [{ open: 1, close: 2, time: 100 }] },
        },
        {
          capturedAt: '2026-03-01T12:00:00.400Z',
          requestPath: '/vas/api/v1/token_trades/sol/TokenBrowser',
          status: 200,
          responseBody: { data: [{ maker: 'x', amount_usd: 5 }] },
        },
        {
          capturedAt: '2026-03-01T12:00:00.500Z',
          requestPath: '/vas/api/v1/token_holder_stat/sol/TokenBrowser',
          status: 200,
          responseBody: { data: [{ holder_count: 900 }] },
        },
        {
          capturedAt: '2026-03-01T12:00:00.600Z',
          requestPath: '/defi/quotation/v1/smartmoney/sol/wallet/SomeWallet',
          status: 200,
          responseBody: { data: [{ wallet: 'SomeWallet' }] },
        },
      ],
    );
    ...
    assert.equal(
      result.errors,
      0,
      'non-signal captures must not be treated as malformed signal events',
    );
    assert.equal(
      result.otherCaptures,
      4,
      'the four non-signal captures are counted, not silently dropped',
    );
    assert.equal(
      (database.prepare('SELECT COUNT(*) AS count FROM gmgn_signals').get() as { count: number })
        .count,
      1,
      'candle/trade/holder/wallet payloads must never end up in gmgn_signals',
    );

    const reimport = importGmgnBrowserCapture(database, 'mixed-again.json', raw);
    assert.equal(reimport.duplicateFile, true);
    assert.equal(
      reimport.otherCaptures,
      4,
      'otherCaptures is reported the same way on the duplicate-file fast path',
    );
  } finally {
    database.close();
  }
...
test('browser import is idempotent by file hash and event identity', () => {
  ...
  try {
    ...
    const overlap = importGmgnBrowserCapture(
      database,
      'signals-2.json',
      exportJson([event('browser-2'), event('browser-3')]),
    );
    ...
    assert.throws(
      () =>
        importGmgnBrowserCapture(
          database,
          'bad.json',
          JSON.stringify({ formatVersion: 1, source: 'wrong', captures: [] }),
        ),
      /source gmgn-browser-extension/,
    );
    const batch = database
      .prepare('SELECT status, error_count, error FROM gmgn_browser_import_batches')
      .get() as { status: string; error_count: number; error: string };
...
test('browser import stores and deduplicates the four raw GMGN endpoint families, and counts them separately from signals', () => {
  ...
  try {
    const captures = [
      {
        ...
        requestPath: '/vas/api/v1/radar/detail',
        requestQuery: { chain: 'sol', period: '1d', type: '7' },
        status: 200,
        responseBody: { code: 0, data: [{ address: 'TokenRadar' }] },
      },
      ...
      {
        capturedAt: '2026-03-01T12:00:02.000Z',
        requestPath: '/api/v1/rank/sol/wallets/7d',
        requestQuery: { orderby: 'pnl_30d' },
        status: 200,
        responseBody: { code: 0, data: [{ address: 'WalletRank' }] },
      },
      {
        capturedAt: '2026-03-01T12:00:03.000Z',
        requestPath: '/defi/quotation/v1/smartmoney/sol/walletNew/WalletSmart',
        status: 200,
        responseBody: { code: 0, data: { pnl: 12 } },
      },
      {
        capturedAt: '2026-03-01T12:00:03.500Z',
        requestPath: '/defi/quotation/v1/smartmoney/sol/walletNew/WalletSmart',
        status: 200,
        responseBody: { code: 0, data: { pnl: 12 } },
      },
      {
        capturedAt: '2026-03-01T12:00:04.000Z',
        requestPath: '/vas/api/v1/twitter/messages',
        requestQuery: { tw_type: 'kol', has_token: 'false' },
        status: 200,
        responseBody: {
          code: 0,
          data: [{ id: 'tweet-1', tweet_id: 'tweet-1', tw_type: 'kol', text: 'hello' }],
        },
      },
      {
        capturedAt: '2026-03-01T12:00:05.000Z',
        requestPath: '/vas/api/v1/twitter/messages',
        requestQuery: { tw_type: 'kol', has_token: 'false' },
        status: 200,
        responseBody: {
          code: 0,
          data: [{ id: 'tweet-2', tweet_id: 'tweet-2', tw_type: 'kol', text: 'world' }],
        },
      },
    ];
    ...
    assert.equal(
      result.imported,
      0,
      'no real GMGN signals in this batch — the signal counter must stay untouched by raw-endpoint captures',
    );
    assert.equal(result.skipped, 0);
    assert.equal(result.errors, 0);
    ...
    assert.equal(
      (
        database.prepare('SELECT COUNT(*) AS count FROM gmgn_radar_snapshots').get() as {
          count: number;
        }
      ).count,
      1,
    );
    const radarRow = database
      .prepare('SELECT chain, period, category FROM gmgn_radar_snapshots')
      .get() as Record<string, string>;
    ...
    assert.equal(
      radarRow.category,
      '7',
      'category must come from requestQuery.type, recovered via the fixed extraction path',
    );
    const rankRow = database
      .prepare('SELECT window, orderby FROM gmgn_wallet_rank_snapshots')
      .get() as Record<string, string>;
    assert.equal(
      rankRow.window,
      '30d',
      "window must be derived from orderby's trailing token, never the fixed /wallets/7d path segment — a real 30D capture proved the path never changes",
    );
    assert.equal(rankRow.orderby, 'pnl_30d');
    assert.equal(
      (
        database.prepare('SELECT COUNT(*) AS count FROM gmgn_smartmoney_wallet_stats').get() as {
          count: number;
        }
      ).count,
      2,
      'repeated wallet observations are preserved',
    );
    assert.equal(
      (
        database.prepare('SELECT wallet_address FROM gmgn_smartmoney_wallet_stats').get() as {
          wallet_address: string;
        }
      ).wallet_address,
      'WalletSmart',
    );
    const twitter = database
      .prepare('SELECT COUNT(*) AS count, MAX(has_token) AS hasToken FROM gmgn_twitter_messages')
      .get() as { count: number; hasToken: number };
    assert.equal(twitter.count, 2);
    assert.equal(
      twitter.hasToken,
      0,
      'has_token must be recovered from requestQuery, not silently stored as null',
    );

    const repeat = importGmgnBrowserCapture(
      database,
      'endpoint-repeat.json',
      exportJson([], [], captures).replace('2026-03-01T12:00:00.000Z', '2026-03-01T12:01:00.000Z'),
    );
...
test('browser import redacts account-identifying fields before the raw file is persisted or archived, while dedup still keys off the original bytes', () => {
  ...
  try {
    const captures = [
      {
        capturedAt: '2026-03-01T12:00:01.000Z',
        requestPath: '/vas/api/v1/radar/detail',
        requestQuery: { chain: 'sol' },
        status: 200,
        responseBody: {
          code: 0,
          data: [{ address: 'TokenRadar' }],
          user_id: 'ee33acab-1234',
    ...
    const raw = exportJson([], [], captures);
    assert.ok(
      raw.includes('ee33acab-1234'),
      'sanity check: the fixture actually contains the identifier before import',
    );
    const result = importGmgnBrowserCapture(database, 'privacy-sample.json', raw);

    const stored = (
      database
        .prepare('SELECT raw_source AS rawSource FROM gmgn_browser_import_batches WHERE id = ?')
        .get(result.batchId) as { rawSource: string }
    ).rawSource;
    assert.ok(
      !stored.includes('ee33acab-1234'),
      'user_id must never reach the database unredacted',
    );
    assert.ok(
      !stored.includes('D9Km8Jkz'),
      'referral_code must never reach the database unredacted',
    );
    assert.ok(
      stored.includes('TokenRadar'),
      'legitimate research data in the same payload must survive redaction',
    );
    ...
    const raw = exportJson(
      ...
      [
        ...
        },
        {
          capturedAt: '2026-03-01T12:00:02.000Z',
          requestPath: '/vas/api/v1/twitter/messages',
          requestQuery: { has_token: 'true' },
          status: 200,
          responseBody: { code: 0, data: 'not-an-array' },
        },
      ],
    );
