...
const baseRow = (walletAddress: string, over: Partial<CopyTradeRow> = {}): CopyTradeRow => ({
  ...
  profitConcentration: {
    ...
    excludingBestTrade: { trades: 9, medianReturnPercent: 9, endingCapitalUsd: 108 },
  ...
  try {
    const row = baseRow('W_CONC', {
      ...
      profitConcentration: {
        ...
        bestThreeTokens: [],
        bestTokenSharePositiveProfitPercent: 88,
        bestThreeSharePositiveProfitPercent: 95,
        bestTradeProfitUsd: 5000,
        excludingBestTrade: { trades: 9, medianReturnPercent: -5, endingCapitalUsd: 90 },
    ...
    assert.ok(
      result.checks.concentration.metrics.medianWithoutToken <
        result.checks.concentration.metrics.medianWithToken! - 10,
    );
  } finally {
    database.close();
  }
...
test('insufficient and fail are distinguishable verdicts, never rendered or coded the same', () => {
  ...
  try {
    ...
    const failResult = computeCandidateScrutiny(database, 'W_FAIL', {
      row: failRow,
      simInWindow: simFail,
      simFullHistory: simFail,
      candidateCount: 5,
      screenedCount: 50,
      scopePeriodDays: 90,
    });
    const insufficientResult = computeCandidateScrutiny(database, 'W_INSUFFICIENT', {
      row: insufficientRow,
      simInWindow: simInsufficient,
      simFullHistory: simInsufficient,
      candidateCount: 5,
      screenedCount: 50,
      scopePeriodDays: 90,
    });
    ...
    assert.notEqual(
      failResult.checks.concentration.verdict,
      insufficientResult.checks.concentration.verdict,
    );
...
test('a wallet with no Dune coverage yields insufficient for every coverage-dependent check, never a pass', () => {
  ...
  try {
    ...
    for (let i = 0; i < 15; i += 1)
      seedRoundTrip(database, 'W_NODUNE', `TOKEN_${i}`, 1000 + i * 1000, 5, 'none');
    const row = baseRow('W_NODUNE', { trades: 15 });
    const simInWindow = computeCopySimulationReport(database, {
      walletAddresses: ['W_NODUNE'],
      periodDays: 90,
    }).wallets[0];
    const simFullHistory = computeCopySimulationReport(database, { walletAddresses: ['W_NODUNE'] })
      .wallets[0];
    assert.equal(
      simFullHistory.copiedTrades,
      0,
      'test setup sanity: nothing should be Dune-matched',
    );
...
test('out-of-sample split reports both halves, and too few dates on one side is insufficient rather than a one-sided figure', () => {
  ...
  try {
    ...
    const thin = computeCandidateScrutiny(database, 'W_THIN_TAIL', {
      row,
      simInWindow: sim,
      simFullHistory: sim,
      candidateCount: 5,
      screenedCount: 50,
      scopePeriodDays: 9999,
      now: new Date(150 * 86_400 * 1000),
    });
    ...
    for (let day = 0; day < 10; day += 1)
      seedRoundTrip(database, 'W_BALANCED', `EARLY_${day}`, day * 86_400, 8, 'none');
    for (let day = 20; day < 30; day += 1)
      seedRoundTrip(database, 'W_BALANCED', `LATE_${day}`, day * 86_400, 6, 'none');
    const balancedRow = baseRow('W_BALANCED');
    const balancedSim = emptySim('W_BALANCED');
    const balanced = computeCandidateScrutiny(database, 'W_BALANCED', {
      row: balancedRow,
      simInWindow: balancedSim,
      simFullHistory: balancedSim,
      candidateCount: 5,
      screenedCount: 50,
      scopePeriodDays: 9999,
      now: new Date(60 * 86_400 * 1000),
    });
...
test('repeat-entry and single-entry medians are computed over disjoint trade sets that reconstruct the full population', () => {
  ...
  try {
    ...
    let ts = 1000;
    for (let i = 0; i < 3; i += 1) {
      seedRoundTrip(database, 'W_REPEAT', `REPEAT_${i}`, ts, 20, 'none');
      ts += 1000;
      seedRoundTrip(database, 'W_REPEAT', `REPEAT_${i}`, ts, 25, 'none');
      ts += 1000;
    }
    for (let i = 0; i < 4; i += 1) {
      seedRoundTrip(database, 'W_REPEAT', `SINGLE_${i}`, ts, -5, 'none');
      ts += 1000;
    }
