...
import { computeSignalPatternReport, listSignalPatternSnapshots, saveSignalPatternSnapshot } from '../src/signals/patterns.js';
...
test('pattern report defines "up" as return > 0 and excludes missing prices instead of treating them as 0', () => {
  ...
  try {
    ...
    insertCompletedRun(database, [a1, a2, a3], [
    ...
    assert.equal(group.nWithData, 2, 'signal with a missing +1h price must not count toward n with data');
    ...
    assert.equal(group.reliable, false, 'fewer than the minimum reliable sample must be flagged unreliable');
...
test('pattern report counts distinct tokens behind a group, not just signal instances, so a repeating token cannot inflate the apparent sample size unnoticed', () => {
  ...
  try {
    ...
    const report = computeSignalPatternReport(database, new Date('2026-03-01T02:00:00Z'));
    const group = report.horizons.find((h) => h.horizon === '+1h')!.groups.find((g) => g.key === '12')!;
...
test('a checkpoint that reused the signal-time trade (no new trade occurred) is stale, not a genuine 0% result', () => {
  ...
  try {
    ...
    insertCompletedRun(database, [a1, a2], [
      // a1: no new trade before +1h -> same matched_trade_at as the signal checkpoint -> stale
      { signal_id: a1, checkpoint: 'signal', price_usd: 1, matched_trade_at: '2026-03-01T00:00:00Z' },
      { signal_id: a1, checkpoint: '+1h', target_at: '2026-03-01T00:00:01Z', price_usd: 1, matched_trade_at: '2026-03-01T00:00:00Z' },
      // a2: a genuine new trade at +1h that happens to be at the same price -> real, not stale
      { signal_id: a2, checkpoint: 'signal', price_usd: 1, matched_trade_at: '2026-03-01T00:00:00Z' },
      { signal_id: a2, checkpoint: '+1h', price_usd: 1, matched_trade_at: '2026-03-01T00:00:00Z#new-trade' },
    ]);

    const report = computeSignalPatternReport(database, new Date('2026-03-01T02:00:00Z'));
    const group = report.horizons.find((h) => h.horizon === '+1h')!.groups.find((g) => g.key === '9')!;

    assert.equal(group.n, 2);
    assert.equal(group.nWithData, 2, 'both signals have a price at each end, so neither is "missing"');
    assert.equal(group.nStale, 1, 'the no-new-trade comparison must be counted as stale');
    assert.equal(group.nFresh, 1, 'only the genuine new-trade comparison counts toward the stats below');
    assert.equal(group.upPct, 0, 'a flat genuine trade is not "up" (return must be strictly > 0)');
    assert.ok(report.staleNote.toLowerCase().includes('stale'), 'report must explain what "stale" means');
  } finally { database.close(); }
});
...
test('trade-age cutoff excludes a checkpoint whose matched trade is too old for its horizon', () => {
  ...
  try {
    ...
    insertCompletedRun(database, [id], [
      { signal_id: id, checkpoint: 'signal', price_usd: 1, matched_trade_at: '2026-03-01T00:00:00Z' },
      // +1h permits at most 15 minutes of trade age; this is 30 minutes old.
      { signal_id: id, checkpoint: '+1h', target_at: '2026-03-01T01:00:00Z', price_usd: 1.2, matched_trade_at: '2026-03-01T00:30:00Z' },
    ]);
    const report = computeSignalPatternReport(database, new Date('2026-03-01T02:00:00Z'));
    const group = report.horizons.find((h) => h.horizon === '+1h')!.groups.find((g) => g.key === '7')!;
...
test('coverage and capture-date gates use the deduped first row, not repeated matured rows', () => {
  ...
  try {
    ...
    insertCompletedRun(database, [first, repeat], [
...
test('pattern report flags a group reliable only once it reaches the minimum sample size', () => {
  ...
  try {
    ...
    for (let i = 0; i < 12; i += 1) {
      const id = seedSignal(database, `big-${i}`, 7, `TokenBig${i}`, `2026-03-0${1 + (i % 3)}T00:00:00Z`);
      ids.push(id);
    ...
    assert.ok(report.disclaimer.toLowerCase().includes('descriptive'), 'report must carry a plain-language non-profitability disclaimer');
  } finally { database.close(); }
});
...
test('groups are ranked by median, not average, so a single outlier cannot fake the top spot', () => {
  ...
  try {
    ...
    assert.ok(outlierGroup.avgReturnPct! > steadyGroup.avgReturnPct!, 'the outlier group should have the higher average');
    assert.ok(steadyGroup.medianReturnPct! > outlierGroup.medianReturnPct!, 'the steady group should have the better (higher) median');
    assert.equal(horizon.groups[0].key, '7', 'ranking must put the group with the better median first, not the group with the higher average');
  } finally { database.close(); }
});
...
test('saving a snapshot freezes the exact params and source run ids used to compute it', () => {
  ...
  try {
    const a1 = seedSignal(database, 'a1', 6, 'TokenA1');
    insertCompletedRun(database, [a1], [
      { signal_id: a1, checkpoint: 'signal', price_usd: 1 },
    ...
    assert.deepEqual(saved.params, { groupBy: 'signalType', upThreshold: 0, minReliableSample: report.minReliableSample, minCoveragePct: 25, minCaptureDates: 3, analysisUnit: 'first-signal-per-token-type' });
