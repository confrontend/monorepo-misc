...
const seed = (database: ReturnType<typeof openDatabase>, id: string, observedAt: string): number =>
  storeGmgnSignal(
    database,
...
const insertRun = (
  ...
  database
    .prepare(
      `INSERT INTO dune_outcome_runs (signal_ids, query_sql, execution_id, status, raw_result, requested_at, completed_at) VALUES (?, 'select 1', ?, 'completed', ?, ?, ?)`,
    )
    .run(
  ...
  try {
    ...
    insertRun(
      ...
      CHECKPOINT_LABELS.map((label) => ({
        signal_id: pending,
        checkpoint: label,
        target_at: label === '+3h' ? '2026-03-01 04:30:00.000 UTC' : '2026-03-01 01:30:00.000 UTC',
        price_usd: 1,
      })),
      '2026-03-01T01:31:00Z',
      'pending-run',
    );
    insertRun(
      database,
      ...
      CHECKPOINT_LABELS.map((label) => ({
        ...
        checkpoint: label,
        target_at: `2026-02-28 23:00:00.000 UTC`,
        price_usd: null,
      })),
    ...
    insertRun(
      ...
      CHECKPOINT_LABELS.map((label) => ({
        ...
        matched_trade_at:
          label === 'signal'
            ? '2026-02-28 22:59:00.000 UTC'
            : `2026-02-28 22:59:0${label.length}.000 UTC`,
      })),
    ...
    assert.deepEqual(
      plan.eligibleSignalIds.sort((a, b) => a - b),
      [notMeasured, unavailable].sort((a, b) => a - b),
    );
    ...
    assert.deepEqual(
      plan.bySignalType.find((item) => item.signalType === '7'),
      {
        signalType: '7',
        captured: 4,
        measured: 1,
...
test('measurement planner marks a signal younger than the 24h observation buffer as too_fresh, not not_measured', () => {
  ...
  try {
    ...
    assert.ok(
      !plan.eligibleSignalIds.includes(tooFresh),
      'a too-fresh signal must never be selected for Dune submission',
    );
...
test('measurement planner treats a signal exactly at the 24h boundary as eligible, and one second short as too_fresh', () => {
  ...
  try {
    ...
    const plan = buildMeasurementPlan(database, now);
    assert.ok(
      plan.eligibleSignalIds.includes(exactlyAtBoundary),
      'a signal exactly 24h old must already be eligible',
    );
    assert.ok(
      !plan.eligibleSignalIds.includes(oneSecondShort),
      'a signal one second short of 24h must remain too_fresh',
    );
  } finally {
    database.close();
  }
...
test('measurement planner never classifies an already-measured signal as too_fresh (that label is reserved for never-measured signals), even though it now also holds its retry back until 24h', () => {
  ...
  try {
    ...
    insertRun(
      ...
      alreadyMeasured,
      CHECKPOINT_LABELS.map((label) => ({
        signal_id: alreadyMeasured,
        checkpoint: label,
        target_at: '2026-03-01 01:50:00.000 UTC',
        price_usd: null,
      })),
      '2026-03-01T01:51:00Z',
      'early-run',
    );
    const plan = buildMeasurementPlan(database, new Date('2026-03-01T02:00:00Z'));
    assert.notEqual(
      plan.byState.too_fresh,
      1,
      'an already-measured signal must never be classified too_fresh, regardless of its own age',
    );
...
test('measurement planner blocks a retry on a signal under 24h old even though its normal per-attempt delay has already elapsed', () => {
  ...
  try {
    ...
    insertRun(
      ...
      tooYoungForRetry,
      CHECKPOINT_LABELS.map((label) => ({
        signal_id: tooYoungForRetry,
        checkpoint: label,
        target_at: '2026-03-01 00:05:00.000 UTC',
        price_usd: null,
      })),
    ...
    assert.equal(
      plan.byState.retry_eligible,
      0,
      'a retry must not fire before the signal itself is 24h old',
    );
    assert.equal(plan.byState.elapsed_but_unavailable, 1);
  } finally {
    database.close();
...
test('measurement planner forgives attempts fired before the 24h buffer, so a signal stuck by pre-rule premature retries is retry_eligible again, not retry_exhausted', () => {
  ...
  try {
    ...
    assert.equal(
      plan.byState.retry_exhausted,
      0,
      'premature pre-buffer attempts must not exhaust the retry budget',
    );
    assert.equal(plan.byState.retry_eligible, 1);
    ...
    assert.equal(
      plan.tooFreshCount,
      0,
      'this signal was queried, so it must not also show up as never-measured',
    );
  } finally {
    database.close();
  }
...
test('measurement planner does not count an ordinary retry_eligible signal (one legitimate mature failed attempt) as neverMaturelyAttempted', () => {
  ...
  try {
    ...
    assert.equal(plan.byState.retry_eligible, 1);
    assert.equal(
      plan.neverMaturelyAttemptedCount,
      0,
      'a signal with a real mature attempt must not be double-counted as never-measured-fairly',
    );
  } finally {
    database.close();
  }
...
test('measurement planner still exhausts a signal whose 4 attempts all fired at or after the 24h buffer', () => {
  ...
  try {
    ...
    [
      '2026-03-02T00:00:00Z',
      '2026-03-02T00:16:00Z',
      '2026-03-02T01:20:00Z',
      '2026-03-02T05:25:00Z',
    ].forEach((completedAt, index) => {
      insertRun(
        database,
        genuinelyExhausted,
        CHECKPOINT_LABELS.map((label) => ({
          signal_id: genuinelyExhausted,
          checkpoint: label,
          target_at: '2026-02-28 23:00:00.000 UTC',
          price_usd: null,
        })),
...
test('measurement planner blocks signals in submitted, running, and timed-out runs', () => {
  ...
  try {
    ...
    database
      .prepare(
        `INSERT INTO dune_outcome_runs (signal_ids, query_sql, status, requested_at) VALUES (?, 'select 1', 'running', ?), (?, 'select 1', 'timed_out', ?)`,
      )
      .run(
...
test('measurement planner caches the compact snapshot and invalidates it when signals change', () => {
  ...
  try {
    ...
    assert.equal(
      (
        database.prepare('SELECT count(*) AS count FROM measurement_plan_cache').get() as {
          count: number;
        }
      ).count,
      1,
