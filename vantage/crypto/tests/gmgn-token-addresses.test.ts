...
test('lists unique GMGN-observed token addresses not already in the Dune cohort', () => {
  ...
  try {
    ...
    storeGmgnSignal(database, { observed_at: '2026-08-09T01:00:00Z', token_address: 'AlreadyInCohort', signal_type: 'buy' },
      { capturedAt: new Date('2026-08-09T01:00:01Z'), logger: { warn() {} } });
    storeGmgnSignal(database, { observed_at: '2026-08-09T02:00:00Z', token_address: 'NewFromGmgn2', signal_type: 'buy' },
      { capturedAt: new Date('2026-08-09T02:00:01Z'), logger: { warn() {} } });
    storeGmgnSignal(database, { observed_at: '2026-08-09T03:00:00Z', token_address: 'NewFromGmgn1', signal_type: 'watch' },
    ...
    storeGmgnSignal(database, { observed_at: '2026-08-09T04:00:00Z', token_address: 'NewFromGmgn1', signal_type: 'watch' },
      { capturedAt: new Date('2026-08-09T04:00:01Z'), logger: { warn() {} } });
    storeGmgnSignal(database, { observed_at: '2026-08-09T05:00:00Z', signal_type: 'malformed' },
...
test('returns an empty summary for a database with no GMGN signals', () => {
  const database = openDatabase(':memory:');
  try {
    assert.deepEqual(listGmgnTokenAddresses(database), { addresses: [], total: 0, matchedToCohort: 0, unmatchedToCohort: 0 });
  } finally {
    database.close();
  }
});
