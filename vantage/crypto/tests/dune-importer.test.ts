...
import {
  ...
  importDuneContent,
  type ImportLogger,
} from '../src/dune/ingest/importer.js';
...
test('Dune imports are idempotent and duplicate token addresses are skipped', () => {
  ...
  try {
    ...
    assert.equal(
      (database.prepare('SELECT COUNT(*) AS count FROM tokens').get() as { count: number }).count,
      1,
    );
    assert.equal(
      (database.prepare('SELECT symbol FROM tokens').get() as { symbol: string }).symbol,
      'AAA',
    );
    assert.equal(
      (
        database.prepare('SELECT COUNT(*) AS count FROM dune_import_batches').get() as {
          count: number;
        }
      ).count,
      1,
...
test('Dune first-trade timestamps retain exact source precision and formatting', () => {
  ...
  writeFileSync(
    file,
    `token_address,symbol,first_trade_time,first_dex,first_tx\nTokenB,BBB,${timestamp},raydium,tx-1\n`,
  );
  const database = openDatabase(':memory:');
  ...
  try {
    ...
    const row = database
      .prepare('SELECT first_trade_time FROM tokens WHERE token_address = ?')
...
test('a targeted enrichment import is tagged with its own source and never overwrites an existing cohort row', () => {
  ...
  try {
    importDuneContent(
      database,
      'ui-upload/cohort.json',
      JSON.stringify([
        {
          token_address: 'TokenC',
          symbol: 'FROM_COHORT',
          first_trade_time: '2026-01-01T00:00:00Z',
        },
      ]),
      silentLogger,
    ...
    const enrichment = importDuneContent(
      database,
      'ui-upload/enrichment/lookup.json',
      JSON.stringify([
        {
          token_address: 'TokenC',
          symbol: 'SHOULD_NOT_OVERWRITE',
          first_trade_time: '2026-02-01T00:00:00Z',
        },
        {
          token_address: 'TokenD',
          symbol: 'NEW_FROM_ENRICHMENT',
          first_trade_time: '2026-02-02T00:00:00Z',
        },
      ]),
      silentLogger,
    ...
    const existing = database
      .prepare('SELECT symbol, source FROM tokens WHERE token_address = ?')
      .get('TokenC') as { symbol: string; source: string };
    assert.equal(existing.symbol, 'FROM_COHORT');
    assert.equal(existing.source, 'dune');
    const added = database
      .prepare('SELECT symbol, source FROM tokens WHERE token_address = ?')
      .get('TokenD') as { symbol: string; source: string };
...
test('malformed Dune rows are retained in the audit log', () => {
  ...
  try {
    ...
    const row = database
      .prepare('SELECT status, errors, raw_payload FROM dune_import_records')
      .get() as {
      status: string;
      errors: string;
      raw_payload: string;
    ...
    assert.deepEqual(JSON.parse(row.raw_payload), {
      symbol: 'NO_ADDRESS',
