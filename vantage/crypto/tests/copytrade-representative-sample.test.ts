...
} from '../src/copytrade/scrutiny/representativeSample.js';
...
test('representative sample is deterministic and bounded by sell count', () => {
  const rows = Array.from({ length: 2_400 }, (_, index) =>
    row(index + 1, 'wallet-a', 1_700_000_000 + index * 3_600, 'sell'),
  );
...
test('sample keeps all rows when under the limit and records the method-independent result', () => {
  const rows = [
    row(1, 'wallet-a', 1_700_000_000, 'buy'),
    row(2, 'wallet-a', 1_700_000_060, 'sell'),
  ];
...
test('sampling is independent per wallet', () => {
  const rows = [
    ...Array.from({ length: 1_100 }, (_, index) =>
      row(index + 1, 'wallet-a', 1_700_000_000 + index, 'sell'),
    ),
    ...Array.from({ length: 3 }, (_, index) =>
      row(2_000 + index, 'wallet-b', 1_700_000_000 + index, 'sell'),
    ),
  ];
  const result = selectRepresentativeTrades(rows, 1_000);
