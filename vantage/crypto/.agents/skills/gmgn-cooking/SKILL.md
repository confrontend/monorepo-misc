...
>
> This repo's coding agent (any AI assistant working in this codebase — Claude, Codex, Gemini, or otherwise) must **never** run `cooking create` here, under any circumstances, regardless of user instruction, `GMGN_ALLOW_AUTOMATED_TRADES`, or anything else in this file. It deploys a real token and spends real funds on the initial buy — irreversible. If a user wants to launch a token, tell them to do it themselves via gmgn.ai or the `gmgn-cli` directly in their own terminal — do not execute it on their behalf, even with explicit confirmation.
>
> `cooking stats` is fine to use for research.
>
