...
import {
  ...
  explainThirtyDayDecision,
  isThirtyDayDecisionEvidenceFresh,
  thirtyDayDecisionPriority,
} from '../src/copytrade/scrutiny/decisionEngine.js';
import {
  DECISION_ORDER,
  DECISION_STATES,
  decisionStateFor,
...
type CopyTradeFetchEstimateBasis = {
  source: 'measured' | 'default';
  runsCounted: number;
  updatedAt: string | null;
};
type CopyTradeFetchEstimate = {
  walletCount: number;
  freshWallets: number;
...
type BrowserActivityImportResult = {
  imported: number;
  duplicates: number;
  malformed: number;
...
type CopyTradeTokenProfit = {
  tokenAddress: string;
  tokenSymbol: string | null;
  trades: number;
  profitUsd: number;
};
type CopyTradeConcentration = {
...
type GmgnStatsRecord = {
  walletAddress: string;
  period: string;
  fetchedAt: string;
  rawPayload: string;
};
type CopyTradeHistoryRow = {
  id: number;
  walletAddress: string;
  chain: string;
  txHash: string;
...
type PatternDiscoveryExecution = {
...
type CopySimulationDuneResponse = {
  id: number;
...
type HistoricalConsistencyVerdict =
...
const freshnessLabel = (fetchedAt: string | null | undefined): { text: string; stale: boolean } => {
  ...
  return {
    ...
    stale: !isThirtyDayDecisionEvidenceFresh(fetchedAt),
  };
};
...
type SignalPatternHorizonReport = {
  horizon: string;
  overall: SignalPatternGroup;
  groups: SignalPatternGroup[];
};
type SignalPatternReport = {
  computedAt: string;
  method: string;
  groupBy: string;
  upThreshold: number;
...
type SignalPatternSubgroupHorizonReport = {
  horizon: string;
  cellCount: number;
  nUnextractable: number;
  groups: SignalPatternGroup[];
};
type SignalPatternSubgroupReport = {
  computedAt: string;
  method: string;
...
const meterWidth = (value: number | null): string =>
  `${Math.min(100, Math.max(0, Math.abs(value ?? 0)))}%`;
...
const holdingSecondsBySellId = (rows: CopyTradeHistoryRow[]): Map<number, number | null> => {
  ...
  [...rows]
    .sort((a, b) => a.observedTimestamp - b.observedTimestamp || a.id - b.id)
    .forEach((trade) => {
      ...
      if (side.startsWith('buy')) {
        ...
        holds.set(
          trade.id,
          openedAt === undefined ? null : Math.max(0, trade.observedTimestamp - openedAt),
        );
...
const compactMagnitudeUsd = (
  absolute: number,
  opts: { billions: boolean; upperCase: boolean; round: boolean; decimals: number },
): string | null => {
...
const formatUsd = (value: number | null): string => {
  ...
  const magnitude = compactMagnitudeUsd(absolute, {
    billions: true,
    upperCase: true,
    round: false,
    decimals: 2,
...
const formatCopyCapital = (value: number | null): string => {
  ...
  if (absolute >= 1e21)
    return `${sign}$${Math.round(absolute / 10 ** Math.floor(Math.log10(absolute)))} × 10^${Math.floor(Math.log10(absolute))}`;
  const magnitude = compactMagnitudeUsd(absolute, {
    billions: true,
    upperCase: true,
    round: true,
...
const formatChartDate = (value: string): string => {
  ...
  return Number.isNaN(parsed.getTime())
    ...
    : parsed.toLocaleDateString('en-US', {
        month: 'short',
        day: 'numeric',
        year: /^\d{4}-/.test(day) ? 'numeric' : undefined,
        timeZone: 'UTC',
      });
};

const CapitalPathChart = ({
  points,
  activeTradeId,
  onTradeHover,
  zoomable = false,
}: {
  points: Array<{ day: string; capitalUsd: number; label?: string; tradeId?: number }>;
  activeTradeId?: number | null;
  onTradeHover?: (tradeId: number | null) => void;
  zoomable?: boolean;
}) => {
  const [hovered, setHovered] = useState<{
    point: { day: string; capitalUsd: number; label?: string };
    x: number;
  } | null>(null);
  ...
  if (points.length === 0)
    return (
      <span
        className="capital-path-unavailable"
        title="No complete multi-day capital path is available for this trader"
      >
        No path
      </span>
    );
  ...
  const barData = points.map((point, index) => {
    const x =
      left +
      (index / Math.max(1, points.length)) * plotWidth +
  ...
  return (
    <span
      ...
      style={zoomable ? { width: `${zoom * 100}%` } : undefined}
      title={`$100 path: ${points[0].day} → ${finalPoint.day}; final ${formatUsd(finalPoint.capitalUsd)}`}
    >
      {zoomable && (
        <button
          type="button"
      ...
      <svg
        ...
        role="img"
        aria-label={`$100 path ending at ${formatUsd(finalPoint.capitalUsd)}`}
      ...
      >
        ...
        <line
          ...
          y1={zeroY}
          y2={zeroY}
          className="capital-path-baseline"
        />
        <defs>
          <linearGradient id="capital-path-gradient" x1="0" x2="1" y1="0" y2="0">
            <stop offset="0%" stopColor="#72d8af" />
            <stop offset="50%" stopColor="#f0c875" />
            <stop offset="100%" stopColor="#72a7ff" />
          </linearGradient>
        </defs>
        {barData.map(({ point, x, y, height: barHeight }, index) => (
          <rect
            key={`${point.day}-${index}`}
            x={x}
            y={Math.min(zeroY, y)}
            ...
            className={`${point.capitalUsd >= 100 ? 'capital-path-bar positive' : 'capital-path-bar negative'}${point.tradeId !== undefined && point.tradeId === activeTradeId ? ' active' : ''}`}
            onMouseEnter={() => {
              setHovered({ point, x: x + barWidth / 2 });
              onTradeHover?.(point.tradeId ?? null);
            }}
          />
        ))}
      </svg>
      {/* Rendered as HTML overlays, not SVG <text>, because the svg above uses
        preserveAspectRatio="none" -- correct for bars (a stretched rectangle is still just a
        rectangle) but it visibly warps glyph shapes in text. Percentage-based positioning here
        lands in the same spot the stretched SVG coordinates would, without distorting the type. */}
      <span
        className="capital-path-axis-label"
        style={{
          left: `${((left - 5) / width) * 100}%`,
          top: `${((top + 4) / height) * 100}%`,
      ...
      <span
        ...
        style={{
          left: `${((left - 5) / width) * 100}%`,
          top: `${((height - bottom + 4) / height) * 100}%`,
          transform: 'translate(-100%, -50%)',
        }}
      >
        {formatChartUsd(min)}
      ...
      {hovered && (
        <span className="capital-path-tooltip" style={{ left: hoveredLeft }} role="status">
          <small>{hovered.point.label ?? 'Portfolio point'}</small>
          <strong>{formatUsd(hovered.point.capitalUsd)}</strong>
          <span>{formatChartDate(hovered.point.day)}</span>
        </span>
      )}
    </span>
  );
...
const duneCoverageLabel = (coverage: DuneCoverageSummary | undefined, period: string): string => {
  if (!coverage || typeof coverage !== 'object')
    return `Dune ${period} coverage has not been measured yet.`;
  const percent =
    typeof coverage.percent === 'number' && Number.isFinite(coverage.percent)
      ? `${coverage.percent.toFixed(1)}%`
      : '—';
  const matched =
    typeof coverage.matched === 'number' && Number.isFinite(coverage.matched)
      ? coverage.matched.toLocaleString()
      : '—';
  const eligible =
    typeof coverage.eligible === 'number' && Number.isFinite(coverage.eligible)
      ? coverage.eligible.toLocaleString()
      : '—';
...
const copyTradeVerdictLabel: Record<CopyTradeRow['verdict'], string> = {
...
const copyTradeVerdictIcon: Record<CopyTradeRow['verdict'], string> = {
  screen_pass: '✓',
  no: '✕',
  thin: '—',
  flagged: '⚠',
...
const DECISION_COLUMNS: Array<{ key: DecisionColumnKey; label: string }> = [
  ...
  { key: 'gmgnPnl', label: '30d GMGN PnL' },
  { key: 'gmgnTrades', label: '30d GMGN trades' },
  { key: 'copyMedian', label: '30d copy median' },
  { key: 'copyCapital', label: '$100 after copy' },
  { key: 'evidence', label: 'Evidence' },
  { key: 'duneCoverage', label: 'Dune coverage' },
  { key: 'hold', label: 'Typical hold' },
  { key: 'under15s', label: 'GMGN ≤15s trades' },
...
const WalletIcon = ({ url, name }: { url?: string | null; name: string }) =>
  url ? (
    <img
      className="copytrade-wallet-icon"
      src={url}
      alt=""
      title={`${name} icon`}
      onError={(event) => {
...
const saveCsv = (rows: Array<Record<string, unknown>>, filename: string) => {
  ...
  for (const row of rows)
    parts.push(`${headers.map((header) => csvCell(row[header] ?? '')).join(',')}\r\n`);
...
const InfoTip = ({ label = 'More information', text }: { label?: string; text: string }) => (
  <span className="info-tip" role="note" aria-label={label} title={text}>
...
const parseAggregateRecord = (record: GmgnStatsRecord): GmgnAggregateStats | null => {
  try {
    ...
    return {
      ...
      realizedProfit: numberValue(data, 'realized_profit'),
      realizedProfitPnlPercent: pnlPercent === null ? null : pnlPercent * 100,
      nativeBalance: numberValue(data, 'native_balance'),
      buyCount: numberValue(data, 'buy'),
      sellCount: numberValue(data, 'sell'),
      boughtCost: numberValue(data, 'bought_cost'),
      soldIncome: numberValue(data, 'sold_income'),
      boughtFee: numberValue(data, 'bought_fee'),
      soldFee: numberValue(data, 'sold_fee'),
      totalCost: numberValue(data, 'total_cost'),
      lastTimestamp: numberValue(data, 'last_timestamp'),
      tokenCount: numberValue(pnl, 'token_num'),
      winRatePercent: winRate === null ? null : winRate * 100,
      averageHoldingPeriodSeconds: numberValue(pnl, 'avg_holding_period'),
      buckets: {
        lossOver50: numberValue(pnl, 'pnl_lt_nd5_num'),
        loss50to0: numberValue(pnl, 'pnl_nd5_0x_num'),
        gain0to2x: numberValue(pnl, 'pnl_0x_2x_num'),
        gain2to5x: numberValue(pnl, 'pnl_2x_5x_num'),
        gainOver5x: numberValue(pnl, 'pnl_gt_5x_num'),
      },
      tags: Array.isArray(common.tags)
        ? (common.tags as unknown[]).filter((tag): tag is string => typeof tag === 'string')
        : [],
      walletCreatedAt: numberValue(common, 'created_at'),
      twitterName:
        typeof common.twitter_name === 'string' && common.twitter_name.trim() !== ''
          ? common.twitter_name
          : null,
...
function App() {
  ...
  const selectedRosterSnapshotId = useRosterStore((state) => state.selectedRosterSnapshotId);
  const setSelectedRosterSnapshotId = useRosterStore((state) => state.setSelectedRosterSnapshotId);
  ...
  const [gmgnLeaderboardMetrics, setGmgnLeaderboardMetrics] = useState<
    Record<string, GmgnLeaderboardMetric>
  >({});
  ...
  const [walletStatsEvidenceError, setWalletStatsEvidenceError] = useState<string | null>(null);
  const [walletStatsReloadKey, setWalletStatsReloadKey] = useState(0);
  const [rosterSyncBusy, setRosterSyncBusy] = useState(false);
  const [rosterImportBusy, setRosterImportBusy] = useState(false);
  const [manualRosterImported, setManualRosterImported] = useState(false);
  ...
  const [wideRetryOutcome, setWideRetryOutcome] = useState<{
    before: number | null;
    after: number | null;
    submitted: number;
  } | null>(null);
  ...
  const {
    sort: copyTradeSort,
    toggleSort: toggleCopyTradeSort,
    sortIndicator: copyTradeSortIndicator,
  } = useSortState<CopyTradeSortKey>('endingCapitalUsd', 'desc');
  const {
    sort: decisionSort,
    toggleSort: toggleDecisionSort,
    sortIndicator: decisionSortIndicator,
  } = useSortState<DecisionSortKey>('default', 'asc');
  const {
    sort: gmgnStatsSort,
    toggleSort: toggleGmgnStatsSort,
    sortIndicator: gmgnStatsSortIndicator,
  } = useSortState<GmgnStatsSortKey>('pnl30d', 'desc');
  const {
    sort: copyDelaySort,
    toggleSort: toggleCopyDelaySort,
    sortIndicator: copyDelaySortIndicator,
  } = useSortState<CopyDelaySortKey>('edge', 'desc');
  ...
  const [decisionColumns, setDecisionColumns] = useState<Record<DecisionColumnKey, boolean>>(() => {
    const defaults = Object.fromEntries(DECISION_COLUMNS.map(({ key }) => [key, true])) as Record<
      DecisionColumnKey,
      boolean
    >;
    try {
      const saved = JSON.parse(
        localStorage.getItem(decisionColumnsStorageKey) ?? 'null',
      ) as Partial<Record<DecisionColumnKey, unknown>> | null;
      if (saved)
        for (const { key } of DECISION_COLUMNS)
          if (typeof saved[key] === 'boolean') defaults[key] = saved[key];
    } catch {
      /* use all columns when browser storage is unavailable */
    }
    return defaults;
  });
  ...
  const statsDetailTradesQuery = useQuery({
    queryKey: ['statsDetailTrades', statsDetailWallet],
    queryFn: () =>
      api<CopyTradeHistoryResponse>(
        `/api/copytrade/trades/${encodeURIComponent(statsDetailWallet as string)}`,
      ),
    enabled: statsDetailWallet !== null,
  });
  // Do not render cached query data while a dialog is closed or its wallet is being refreshed.
  // The detail view must represent the current SQLite response, not a previous open.
  const statsDetailTrades =
    statsDetailWallet !== null && !statsDetailTradesQuery.isFetching
      ? (statsDetailTradesQuery.data ?? null)
      : null;
  const statsDetailTradesLoading = statsDetailWallet !== null && statsDetailTradesQuery.isFetching;
  const [statsDetailScrutiny, setStatsDetailScrutiny] = useState<CandidateScrutinyReport | null>(
    null,
  );
  const [statsDetailScrutinyLoading, setStatsDetailScrutinyLoading] = useState(false);
  ...
  const loadCopyTradeEstimate = async (limit: number, periodDays: number) => {
    ...
    try {
      setCopyTradeEstimate(
        await api<CopyTradeFetchEstimate>(
          `/api/copytrade/fetch/estimate?limit=${limit}&periodDays=${periodDays}`,
        ),
      );
    } catch {
      /* advisory only — a failed estimate must never block the fetch controls */
    } finally {
      setCopyTradeEstimateLoading(false);
    }
  };

  const loadHistoricalConsistency = async (
    limit: number,
    ...
    try {
      ...
      const report = await api<HistoricalConsistencyReport>(
        `/api/copytrade/historical-consistency?limit=${limit}${snapshotQuery}`,
      );
      setHistoricalConsistency(report);
      return report;
    } catch (error: unknown) {
      setCopyTradeError(error instanceof Error ? error.message : String(error));
      return null;
    } finally {
      setHistoricalConsistencyLoading(false);
    }
  };

  const loadCaptureHealth = async () => {
    try {
      setCaptureHealth(await api<CaptureHealth>('/api/copytrade/capture-health'));
    } catch {
      /* advisory only — a failed capture-health read must never block the Forward Validation tab */
    }
  };

  const loadElimination = async (
    rosterSnapshotId = selectedRosterSnapshotId,
  ): Promise<EliminationReport | null> => {
    ...
    try {
      ...
      const report = await api<EliminationReport>(
        `/api/copytrade/elimination?${params.toString()}`,
      );
      setEliminationReport(report);
      return report;
    } catch (error: unknown) {
      setEliminationError(error instanceof Error ? error.message : String(error));
      return null;
    } finally {
  ...
  const loadCopySimulation = async (
    rosterSnapshotId = selectedRosterSnapshotId,
    ...
    try {
      const report = await api<CopySimulationReport>(
        `/api/copytrade/copy-simulation${params.toString() ? `?${params.toString()}` : ''}`,
      );
      setCopySimulation(report);
      if (periodDays === 30) {
        setCopySimulation30d(report);
      }
      return report;
    } catch (error: unknown) {
      setCopyTradeError(error instanceof Error ? error.message : String(error));
    } finally {
      setCopySimulationLoading(false);
    }
  };
  ...
  const loadLiquidityImpact = async (rosterSnapshotId = selectedRosterSnapshotId) => {
    ...
    try {
      setLiquidityImpact(
        await api<LiquidityImpactReport>(
          `/api/copytrade/liquidity-impact?periodDays=30${rosterSnapshotId ? `&snapshotId=${rosterSnapshotId}` : ''}`,
        ),
      );
    } catch (error: unknown) {
      setCopyTradeError(error instanceof Error ? error.message : String(error));
    } finally {
      setLiquidityImpactLoading(false);
    }
  };
  ...
  const runCopySimulationBatch = async (
    ...
    try {
      const body = walletAddresses?.length
        ? {
            walletAddresses,
            periodDays,
            ...(wideRetry && searchWindowMinutes ? { searchWindowMinutes } : {}),
          }
        : { periodDays };
      const result = await api<{
        ...
        `/api/copytrade/copy-simulation/${wideRetry ? 'wide-retry' : 'run'}${rosterSnapshotId ? `?snapshotId=${rosterSnapshotId}` : ''}`,
        {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
      ...
      // A targeted retry returns only the requested wallet(s). Re-read the complete
      // 30-day report so the main table does not replace the other rows with that subset.
      if (periodDays === 30 && walletAddresses?.length) {
        const completeTableScope = [
          ...new Set(sortedGmgnStatsRows.map((entry) => entry.walletAddress)),
        ];
        await loadCopySimulation(
          selectedRosterSnapshotId ?? undefined,
          completeTableScope,
          periodDays,
        );
      }
      ...
      setCopySimulationRunBusy(false);
      void api<CopySimulationRunStatus>('/api/copytrade/copy-simulation/status')
        .then(setCopySimulationRunStatus)
        .catch(() => undefined);
    }
  };
  ...
  const stopCopySimulation = async () => {
    ...
    try {
      setCopySimulationRunStatus(
        await api<CopySimulationRunStatus>('/api/copytrade/copy-simulation/stop', {
          method: 'POST',
        }),
      );
    } catch (error: unknown) {
      ...
      setCopySimulationStopBusy(false);
    }
  };
  ...
  const loadCopyTradePage = async (loadWalletEvidence = true) => {
    ...
    try {
      ...
      if (selectedRosterSnapshotId === null && rosterSnapshotId !== null)
        setSelectedRosterSnapshotId(rosterSnapshotId);
      const snapshotQuery = rosterSnapshotId ? `&snapshotId=${rosterSnapshotId}` : '';
      const [summary, results, status, duneStatus] = await Promise.all([
        ...
        api<CopyTradeResults>(
          `/api/copytrade/results?periodDays=${copyTradePeriodDays}&limit=${copyTradeLimit}${snapshotQuery}`,
        ),
      ...
      if (loadWalletEvidence && copyTradeSubTab === 'wallet-stats') {
        ...
        const simulation = await loadCopySimulation(
  ...
  const fetchCopyTrades = async () => {
    ...
    try {
      const response = await fetch('/api/copytrade/fetch', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ limit: copyTradeLimit, periodDays: copyTradePeriodDays }),
      });
      const body = (await response.json()) as {
        runId?: number;
        status?: 'running';
  ...
  const fetchSingleTrader = async () => {
    ...
    try {
      const response = await fetch('/api/copytrade/fetch/single', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ query, periodDays: copyTradePeriodDays }),
      });
      const body = (await response.json()) as {
        runId?: number;
        status?: 'running';
        error?: string;
        resolved?: { kind: string; walletAddress?: string; matchedName?: string };
      };
      if (response.status === 409) {
        setMessage('A CopyTrade fetch is already running. Resuming its progress.');
        await loadCopyTradeStatus();
        ...
        const label =
          body.resolved?.kind === 'name_match'
            ? `${body.resolved.matchedName} (${shortAddress(body.resolved.walletAddress ?? '')})`
            : (body.resolved?.walletAddress ?? query);
  ...
  const fetchWinnersTrades = async () => {
    ...
    try {
      const response = await fetch(
        `/api/copytrade/winners/fetch${selectedRosterSnapshotId ? `?snapshotId=${selectedRosterSnapshotId}` : ''}`,
        { method: 'POST' },
      );
      const body = (await response.json()) as {
        runId?: number;
  ...
  const resetCopyTradeFetch = async () => {
    ...
    try {
      ...
      setMessage(
        body.reset
          ? 'GMGN resume snapshot reset. Saved trades were kept.'
          : 'No GMGN snapshot to reset.',
      );
      await loadCopyTradeStatus();
    } catch (error: unknown) {
      setCopyTradeError(error instanceof Error ? error.message : String(error));
    } finally {
      setCopyTradeResetBusy(false);
    }
  };

  const importTopWalletRoster = async (file: File) => {
    ...
    try {
      const result = await api<{
        snapshotId: number;
        walletCount: number;
        roster?: { snapshotId?: number | null; total?: number };
      }>('/api/copytrade/roster/import', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ name: file.name, content: await file.text() }),
      });
      if (result.roster?.snapshotId !== undefined && result.roster.snapshotId !== null)
        setSelectedRosterSnapshotId(result.roster.snapshotId);
      setManualRosterImported(true);
      setMessage(
        `Imported ${result.walletCount} GMGN wallets from ${file.name}. No live GMGN request was made.`,
      );
      await loadCopyTradePage(false);
      await loadCaptureHealth();
    } catch (error: unknown) {
      setRosterRefreshError(error instanceof Error ? error.message : String(error));
    } finally {
      setRosterImportBusy(false);
    }
  };

  const fetchImportedGmgnHistory = async () => {
    if (researchUpdateBusy || rosterImportBusy || rosterFetchStatus?.running) return;
    setResearchUpdateBusy(true);
    setCopyTradeError(null);
    setResearchUpdateStage('Fetching GMGN history for the imported top 100…');
    try {
      const status = await fetchTop100GmgnHistoryAndWait(30);
      setResearchUpdateStage(
        status.status === 'completed'
          ? 'GMGN history complete. Review the saved screening result.'
          : `GMGN history ${status.status}. Saved progress is retained.`,
      );
      ...
      setResearchUpdateStage(`GMGN history stopped: ${message}`);
    } finally {
  ...
  const importBrowserActivity = async (file: File) => {
    ...
    setCopyTradeError(null);
    try {
      const result = await api<BrowserActivityImportResult>(
        '/api/copytrade/import-browser-activity',
        {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ name: file.name, content: await file.text() }),
      ...
      setMessage(
        `Imported ${result.imported} wallet trades from ${file.name}; ${result.duplicates} duplicates retained as history. ${result.malformed} malformed rows were kept in the raw archive.`,
      );
      await loadCopyTradePage();
      await loadCaptureHealth();
    } catch (error: unknown) {
      ...
      setBrowserActivityImportBusy(false);
    }
  };

  const saveCopyTradeSnapshot = async () => {
    try {
      const result = await api<{ snapshotId: number; computedAt: string }>(
        '/api/copytrade/results/snapshot',
        {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          ...
          body: JSON.stringify({
            periodDays: copyTradePeriodDays,
            limit: copyTradeLimit,
            snapshotId: selectedRosterSnapshotId,
          }),
        },
      );
  ...
  useEffect(() => {
    ...
    if (snapshotForStats !== null) {
      void loadGmgnStats(100, snapshotForStats).then((result) => {
        if (result?.stats)
          setWalletScreenSummary(
            buildWalletScreenSummary(result.stats, Math.max(copyTradeRows.length, 100)),
          );
      });
    }
    ...
    const timer = window.setInterval(async () => {
      ...
      if (
        ...
        if (next.status === 'completed')
          await loadCopyTradeEstimate(copyTradeLimit, copyTradePeriodDays);
      }
    }, 2000);
    return () => window.clearInterval(timer);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(
      () => {
        void loadCopyTradeEstimate(copyTradeLimit, copyTradePeriodDays);
      },
  ...
  useEffect(() => {
    if (copyTradeSubTab === 'wallet-stats') {
      ...
      setWalletStatsEvidenceError(null);
      void (async () => {
        try {
          ...
          const [statsResult, simulation, consistency] = await Promise.all([
            loadGmgnStats(),
          ...
          if (!statsResult?.stats || !simulation || !consistency) {
            throw new Error('Saved decision evidence is incomplete. Retry the SQLite read.');
          }
    ...
    copyTradeRosters?.selectedByDefault,
    walletStatsReloadKey,
  ]);
  useEffect(() => {
    if (WALLET_STATS_ONLY || activeMenu !== 'copytrade' || copyTradeSubTab !== 'wallet-stats')
      return;
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        ...
        /* retry on the next poll while the run remains active */
      } finally {
        ...
        if (!disposed && (next?.running ?? copySimulationRunStatus?.running === true))
  ...
  useEffect(() => {
    if (!statsDetailWallet) setStatsDetailTradeId(null);
  }, [statsDetailWallet]);
  useEffect(() => {
    if (statsDetailTradesQuery.error)
      setMessage(
        statsDetailTradesQuery.error instanceof Error
          ? statsDetailTradesQuery.error.message
          : String(statsDetailTradesQuery.error),
      );
  }, [statsDetailTradesQuery.error]);
  useEffect(() => {
    ...
    const params = new URLSearchParams({ wallets: statsDetailWallet });
    if (selectedRosterSnapshotId) params.set('snapshotId', String(selectedRosterSnapshotId));
    void api<ScrutinyResponse>(`/api/copytrade/scrutiny?${params.toString()}`)
      .then((result) => {
        if (!disposed)
          setStatsDetailScrutiny(
            result.reports.find((report) => report.walletAddress === statsDetailWallet) ?? null,
          );
      })
      .catch((error: unknown) => {
      ...
      .finally(() => {
        if (!disposed) setStatsDetailScrutinyLoading(false);
      });
    return () => {
      disposed = true;
    };
  ...
  const highlightStatsTrade = (tradeId: number | null, scrollToRow = false) => {
    ...
    if (scrollToRow && tradeId !== null)
      window.requestAnimationFrame(() =>
        document
          .querySelector(`[data-detail-trade-id="${tradeId}"]`)
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem(
        'vantage.crypto.excluded-screening-wallets',
        JSON.stringify(excludedScreeningWallets),
      );
    } catch {
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem(
        'vantage.crypto.included-screening-wallets',
        JSON.stringify(includedScreeningWallets),
      );
    } catch {
      /* Persistence is best effort. */
    }
  ...
  const navigateCopyTradeSubTab = (subTab: CopyTradeSubTab) => {
    ...
    setStatsDetailWallet(null);
    setStatsDetailTradeId(null);
    setWideRetryWallet(null);
  ...
  const historicalConsistencyByWallet = new Map(
    (historicalConsistency?.rows ?? []).map((row) => [row.walletAddress, row]),
  );
  const sortedCopyTradeRows = [...copyTradeRows].sort((left, right) => {
    const value = (row: CopyTradeRow): string | number | null =>
      copyTradeSort.key === 'name'
        ? (row.name ?? row.walletAddress)
        : copyTradeSort.key === 'verdict'
          ? row.verdict
          : row[copyTradeSort.key];
  ...
  for (const record of gmgnStatsRecords) {
    ...
    if (!parsed) continue;
    const periods =
      gmgnStatsByWallet.get(record.walletAddress) ?? new Map<string, GmgnAggregateStats>();
    periods.set(record.period, parsed);
    gmgnStatsByWallet.set(record.walletAddress, periods);
  }
  const visibleWalletScreenSummary =
    walletScreenSummary ??
    (gmgnStatsRecords.length > 0
      ? buildWalletScreenSummary(gmgnStatsRecords, Math.max(copyTradeRows.length, 100))
      : null);
  ...
  const staleDataAgeHours = latestGmgnFetchAt
    ? (Date.now() - Date.parse(latestGmgnFetchAt)) / 3_600_000
    : null;
  const estimatedTradesSinceLastFetch =
    staleDataAgeHours !== null
      ? [...gmgnStatsByWallet.values()].reduce((sum, periods) => {
          const stats30d = periods.get('30d');
          const totalTrades30d = stats30d
            ? (stats30d.buyCount ?? 0) + (stats30d.sellCount ?? 0)
            : 0;
          return sum + (totalTrades30d / (30 * 24)) * staleDataAgeHours;
        }, 0)
      : null;
  const latestDuneFetchAt =
    copySimulationRunStatus?.finishedAt ??
    copySimulationRunStatus?.persistedRun?.completedAt ??
    null;
  const triageHasCurrentInputs = Boolean(
    eliminationReport &&
    latestGmgnFetchAt &&
    latestDuneFetchAt &&
  ...
  const triageEliminatedWalletSet = new Set(
    skipEliminatedInDune && triageHasCurrentInputs && eliminationReport
      ? eliminationReport.eliminated.map((entry) => entry.walletAddress)
      : [],
  );
  ...
  const copierDelaySecondsForScope =
    copySimulation30d?.assumptions.copierDelaySeconds ??
    copySimulation?.assumptions.copierDelaySeconds ??
    15;
  const uncopyableWalletSet = new Set(
    skipScopeFiltersInDune
      ? copyTradeRows
          .filter(
            (row) =>
              row.riskEvidence?.medianHoldSeconds !== null &&
              row.riskEvidence?.medianHoldSeconds !== undefined &&
              row.riskEvidence.medianHoldSeconds < copierDelaySecondsForScope,
          )
  ...
  const excludedScreeningWalletSet = new Set(
    [
      ...excludedScreeningWallets,
      ...triageEliminatedWalletSet,
      ...uncopyableWalletSet,
      ...highRiskWalletSet,
    ].filter((wallet) => !includedScreeningWalletSet.has(wallet)),
  );
  const skippedScreeningCount =
    visibleWalletScreenSummary?.activityLeaders.filter((entry) =>
      excludedScreeningWalletSet.has(entry.wallet),
    ).length ?? 0;
  const triageExcludedFromDuneCount =
    visibleWalletScreenSummary?.activityLeaders.filter(
      (entry) =>
        excludedScreeningWalletSet.has(entry.wallet) && triageEliminatedWalletSet.has(entry.wallet),
    ).length ?? 0;
  const uncopyableExcludedFromDuneCount =
    visibleWalletScreenSummary?.activityLeaders.filter(
      (entry) =>
        excludedScreeningWalletSet.has(entry.wallet) && uncopyableWalletSet.has(entry.wallet),
    ).length ?? uncopyableWalletSet.size;
  const researchWalletAddresses = (
    copyTradeRows.length > 0
      ? copyTradeRows.filter((row) => !row.historyFailed).map((row) => row.walletAddress)
      : (visibleWalletScreenSummary?.activityLeaders.map((entry) => entry.wallet) ?? [])
  ).filter((wallet) => !excludedScreeningWalletSet.has(wallet));
  const duneHistoryFailedCount = copyTradeRows.filter((row) => row.historyFailed).length;
  const duneManuallyExcludedCount = (
    copyTradeRows.length > 0
      ? copyTradeRows.map((row) => row.walletAddress)
      : (visibleWalletScreenSummary?.activityLeaders.map((entry) => entry.wallet) ?? [])
  ).filter((wallet) => excludedScreeningWalletSet.has(wallet)).length;
  const duneSelectionNote = [
    duneHistoryFailedCount > 0 ? `${duneHistoryFailedCount} history incomplete` : null,
    duneManuallyExcludedCount > 0 ? `${duneManuallyExcludedCount} excluded` : null,
  ]
    .filter(Boolean)
    .join(' · ');
  const activityWalletAddresses =
    visibleWalletScreenSummary?.activityLeaders.map((entry) => entry.wallet) ?? [];
  const toggleScreeningWallet = (wallet: string) => {
    if (excludedScreeningWalletSet.has(wallet)) {
      ...
      setIncludedScreeningWallets((current) =>
        current.includes(wallet) ? current : [...current, wallet],
      );
    } else {
      setIncludedScreeningWallets((current) => current.filter((entry) => entry !== wallet));
      setExcludedScreeningWallets((current) =>
        current.includes(wallet) ? current : [...current, wallet],
      );
    }
  };
  ...
  const fetchTop100GmgnHistoryAndWait = async (
    periodDays: number,
  ): Promise<CopyTradeFetchStatus> => {
    const response = await fetch('/api/copytrade/fetch', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
    ...
    if (!response.ok && response.status !== 409)
      throw new Error(body.error ?? `GMGN history fetch failed (${response.status}).`);
    let status = await loadCopyTradeStatus();
    ...
    if (status.status !== 'completed')
      throw new Error(status.message || `GMGN history fetch ${status.status}.`);
    return status;
  };
  useEffect(() => {
    ...
    const highActivityWallets = new Set(
      visibleWalletScreenSummary.activityLeaders
        .filter((entry) => entry.trades > 2_000)
        .map((entry) => entry.wallet),
    );
    if (highActivityWallets.size > 0)
      setExcludedScreeningWallets((current) =>
        current.filter((wallet) => !highActivityWallets.has(wallet)),
      );
  ...
  const gmgnStatsValue = (row: CopyTradeRow): string | number | null => {
    ...
    if (gmgnStatsSort.key === 'dailyProfit7d')
      return formatDailyProfit7d(leaderboard?.dailyProfit7d).total;
    if (gmgnStatsSort.key === 'win7d') return short?.winRatePercent ?? null;
    if (gmgnStatsSort.key === 'win30d') return long?.winRatePercent ?? null;
    if (gmgnStatsSort.key === 'activity') {
      const activity = long ?? short;
      return activity ? (activity.buyCount ?? 0) + (activity.sellCount ?? 0) : null;
    }
    if (gmgnStatsSort.key === 'equivalent') {
      const pnl30d =
        long?.realizedProfitPnlPercent ??
        (numeric(leaderboard?.pnl30d) === null ? null : numeric(leaderboard?.pnl30d)! * 100);
      return pnl30d == null ? null : 100 * (1 + pnl30d / 100);
    }
    return long?.fetchedAt
      ? Date.parse(long.fetchedAt)
      : short?.fetchedAt
        ? Date.parse(short.fetchedAt)
        : null;
  };
  const sortedGmgnStatsRows = [...copyTradeRows].sort((left, right) => {
    const leftValue = gmgnStatsValue(left);
    const rightValue = gmgnStatsValue(right);
    const comparison =
      leftValue === null && rightValue === null
  ...
  const copySimulation30dByWallet = new Map(
    (copySimulation30d?.wallets ?? []).map((wallet) => [wallet.walletAddress, wallet]),
  );
  const copyDelayRows = sortedGmgnStatsRows.map((row) => {
    const tradeRow = copyTradeResults?.rows.find(
      (candidate) => candidate.walletAddress === row.walletAddress,
    );
    ...
    const delay =
      ...
      15;
    const delayShare =
      hold !== null && Number.isFinite(hold) && hold > 0 ? (delay / hold) * 100 : null;
    ...
    const evidenceCoverage =
      sim && sim.roundTripsConsidered > 0
        ? Math.round((sim.copiedTrades / sim.roundTripsConsidered) * 1000) / 10
        : null;
    ...
    const edge =
      walletMedian != null &&
      simulatedMedian != null &&
      Number.isFinite(walletMedian) &&
      Number.isFinite(simulatedMedian) &&
      walletMedian > 0
        ? (simulatedMedian / walletMedian) * 100
        : null;
    ...
    const hasNeverQueried =
      sim?.trades.some((trade) => trade.status === 'not_yet_queried') ?? false;
    const reading =
      hold === null
        ? 'No median hold data'
        : !sim
          ? 'Never queried'
          : sim.coverageStatus === 'missing_local_history'
            ? 'Missing local history'
            : sim.coverageStatus === 'no_dune_match' ||
                (hasQueriedNoMatch && sim.copiedTrades === 0)
              ? 'Queried · no match'
              : hasNeverQueried
                ? 'Not queried yet'
                : sim.coverageStatus === 'small_sample'
                  ? 'Small sample'
                  : !sim.gasCostComplete
                    ? 'Gas cost incomplete'
                    : impossible
                      ? 'Delay exceeds median hold'
                      : fragile
                        ? 'Delay is material'
                        : survivedDelay
                          ? 'Portfolio survived delay and costs'
                          : 'Portfolio result is non-positive';
    return {
      row,
      name: row.name?.trim() || shortAddress(row.walletAddress),
      totalTrades7d,
      totalTrades30d,
      hold,
      delayShare,
      ...
      coverage7d: sim7
        ? {
            matched: sim7.copiedTrades,
            eligible: sim7.roundTripsConsidered,
            percent:
              sim7.roundTripsConsidered > 0
                ? Math.round((sim7.copiedTrades / sim7.roundTripsConsidered) * 1000) / 10
                : null,
          }
        : null,
      coverage30d: sim30
        ? {
            matched: sim30.copiedTrades,
            eligible: sim30.roundTripsConsidered,
            percent:
              sim30.roundTripsConsidered > 0
                ? Math.round((sim30.copiedTrades / sim30.roundTripsConsidered) * 1000) / 10
                : null,
          }
        : null,
      edge,
      reading,
      survivedPnl: portfolioPnl,
      impossible,
      fragile,
      survivedDelay,
      sim,
  ...
  const unifiedTraderRows = sortedGmgnStatsRows.map((row) => {
    ...
    const long = periods?.get('30d');
    const leaderboard = gmgnLeaderboardMetrics[row.walletAddress];
    const leaderboardPnl7d =
      typeof leaderboard?.pnl7d === 'number'
        ? leaderboard.pnl7d
        : typeof leaderboard?.pnl7d === 'string' && leaderboard.pnl7d.trim() !== ''
          ? Number(leaderboard.pnl7d)
          : NaN;
    const leaderboardPnl30d =
      typeof leaderboard?.pnl30d === 'number'
        ? leaderboard.pnl30d
        : typeof leaderboard?.pnl30d === 'string' && leaderboard.pnl30d.trim() !== ''
          ? Number(leaderboard.pnl30d)
          : NaN;
    const historical7d =
      short?.realizedProfitPnlPercent ??
      (Number.isFinite(leaderboardPnl7d) ? leaderboardPnl7d * 100 : null);
    const historical30d =
      long?.realizedProfitPnlPercent ??
      (Number.isFinite(leaderboardPnl30d) ? leaderboardPnl30d * 100 : null);
    const delay =
      copyDelayRows.find((entry) => entry.row.walletAddress === row.walletAddress) ?? null;
    ...
    const duneEvidenceAt =
      ...
      null;
    const duneFresh = isThirtyDayDecisionEvidenceFresh(duneEvidenceAt);
    const historyIncomplete =
      row.truncated === true ||
      row.historyFailed === true ||
      row.failedRules.includes('requested history window incomplete');
    ...
    const historicalPositive =
      long?.realizedProfitPnlPercent !== null &&
    ...
    const copyMedianPositive =
      delay?.sim?.simulatedMedianReturnPercent != null &&
    ...
    const consistencyVerdict =
      historicalConsistencyByWallet.get(row.walletAddress)?.verdict ?? null;
    ...
    const noLosingWeek =
      measuredWeeks.length >= MIN_CONSISTENT_WEEKS &&
  ...
  const decisionSortValue = (entry: (typeof unifiedTraderRows)[number]): string | number | null => {
    ...
    if (decisionSort.key === 'copyResult')
      return entry.delay?.sim?.simulatedMedianReturnPercent ?? null;
    if (decisionSort.key === 'copyCapital')
      return entry.delay?.sim?.portfolio?.endingCapitalUsd ?? null;
  ...
  const sortedUnifiedTraderRows = [...unifiedTraderRows].sort((left, right) => {
    const leftValue = decisionSortValue(left);
    const rightValue = decisionSortValue(right);
    const comparison =
      leftValue === null && rightValue === null
        ...
        : leftValue === null
          ...
          : rightValue === null
            ...
            : typeof leftValue === 'number' && typeof rightValue === 'number'
              ...
              : String(leftValue).localeCompare(String(rightValue));
    return (
      (decisionSort.direction === 'asc' ? 1 : -1) * comparison ||
      left.name.localeCompare(right.name)
    );
  });
  const winnerRankedRows = [...unifiedTraderRows].sort((left, right) => {
    ...
    const returnGap =
      (right.delay?.survivedPnl ?? Number.NEGATIVE_INFINITY) -
      (left.delay?.survivedPnl ?? Number.NEGATIVE_INFINITY);
    if (returnGap !== 0) return returnGap;
    const copyGap =
      (right.delay?.sim?.simulatedMeanReturnPercent ?? Number.NEGATIVE_INFINITY) -
      (left.delay?.sim?.simulatedMeanReturnPercent ?? Number.NEGATIVE_INFINITY);
    if (copyGap !== 0) return copyGap;
    const coverageGap =
      (right.coverage ?? Number.NEGATIVE_INFINITY) - (left.coverage ?? Number.NEGATIVE_INFINITY);
    if (coverageGap !== 0) return coverageGap;
    return (
      (left.row.rankHistory.currentRank ?? Number.POSITIVE_INFINITY) -
        (right.row.rankHistory.currentRank ?? Number.POSITIVE_INFINITY) ||
      left.name.localeCompare(right.name)
    );
  });
  const unifiedVerdictCounts = unifiedTraderRows.reduce<Record<string, number>>((counts, entry) => {
  ...
  const researchVerdictChanges = researchUpdateSummary
    ? unifiedTraderRows
        .filter(
          (entry) =>
            researchUpdateSummary.beforeVerdictByWallet[entry.row.walletAddress] &&
            researchUpdateSummary.beforeVerdictByWallet[entry.row.walletAddress] !== entry.verdict,
        )
        .map((entry) => ({
          name: entry.name,
          from: researchUpdateSummary.beforeVerdictByWallet[entry.row.walletAddress],
          to: entry.verdict,
        }))
    : [];
  const walletStatsTableLoading =
    !walletStatsEvidenceError &&
    (!walletStatsReady || (copyTradeLoading && !copyTradeResults) || researchUpdateBusy);
  const researchVerdictLabels = (counts: Record<string, number>) =>
    Object.entries(counts)
      .filter(([, count]) => count > 0)
      .map(([label, count]) => `${count} ${label}`)
      .join(' · ') || 'none';
  const primary30dWinner =
    winnerRankedRows.find((entry) => entry.verdict === 'Tested candidate') ?? null;
  ...
  const sortedCopyDelayRows = [...copyDelayRows].sort((left, right) => {
    ...
    let comparison = 0;
    if (leftValue === null && rightValue === null)
      comparison = left.row.walletAddress.localeCompare(right.row.walletAddress);
  ...
  type DecisionTableRow = (typeof unifiedTraderRows)[number];
  const decisionTableContext = (entry: DecisionTableRow) => {
    const delay = entry.delay;
    const simulatedMedian = delay?.sim?.simulatedMedianReturnPercent ?? null;
    const copyLabel =
      simulatedMedian === null ? (delay?.reading ?? 'Not measured') : formatPct(simulatedMedian);
    const copyCapital = delay?.sim?.portfolio?.endingCapitalUsd ?? null;
    const evidenceLabel = delay?.sim
      ? `${delay.sim.copiedTrades.toLocaleString()} / ${delay.sim.roundTripsConsidered.toLocaleString()}`
      : 'Not measured';
    const coverageClass =
      delay?.coverage === null || delay?.coverage === undefined
        ? 'unknown'
        : delay.coverage >= 90
          ? 'full'
          : delay.coverage >= 70
    ...
    const verdictTooltip =
      entry.verdict === 'Tested candidate'
          ...
          : entry.verdict === 'Needs data'
            ...
            : entry.verdict === 'Not copyable'
              ...
              : entry.verdict === 'Historical / stale'
                ? 'GMGN history is older than 24 hours. Import a fresh GMGN top-100 JSON file from the browser extension, then resume the saved history fetch; the verdict will recalculate automatically.'
    ...
    const walletName =
      walletNameCandidate &&
      normalizedWalletName !== entry.row.walletAddress.toLowerCase() &&
      !entry.row.walletAddress.toLowerCase().startsWith(normalizedWalletName)
        ? walletNameCandidate
        : null;
    return {
      delay,
  ...
  const runWideRetryForWallet = async () => {
    if (!wideRetryWallet || copySimulationRunBusy) return;
    const before =
      unifiedTraderRows.find((entry) => entry.row.walletAddress === wideRetryWallet)?.delay
        ?.coverage ?? null;
    const result = await runCopySimulationBatch(
      [wideRetryWallet],
      30,
      true,
      selectedRosterSnapshotId,
      wideRetryWindowMinutes,
    );
    if (result)
      setWideRetryOutcome({
        before: result.coverageBeforePercent ?? before,
        after: result.coverageAfterPercent ?? null,
        submitted: result.targetsSubmitted,
      });
  };
  const decisionHeaderProps = (key: Exclude<DecisionSortKey, 'default'>, title: string) => ({
  ...
  const decisionTableColumns = [
    {
      key: 'rank',
      header: 'Rank',
      headerProps: decisionHeaderProps('rank', 'Sort by current GMGN rank'),
      render: (entry: DecisionTableRow) =>
        entry.row.rankHistory.currentRank === null ? '—' : `#${entry.row.rankHistory.currentRank}`,
      cellProps: () => ({ title: 'Current rank in the selected GMGN roster' }),
    },
    {
      key: 'gmgn',
      header: 'GMGN',
      headerProps: { title: 'Open this wallet on GMGN' },
      render: (entry: DecisionTableRow) => (
        <a
          className="copytrade-gmgn-link"
          href={`https://gmgn.ai/sol/address/${entry.row.walletAddress}`}
          target="_blank"
          rel="noreferrer"
    ...
    },
    {
      key: 'trader',
      header: 'Trader',
      headerProps: decisionHeaderProps('name', 'Sort by trader'),
      render: (entry: DecisionTableRow) => {
        const { walletName } = decisionTableContext(entry);
        return (
          <>
            <WalletIcon url={entry.row.iconUrl} name={walletName || entry.row.walletAddress} />
            <span>
              {walletName ? (
                <>
                  <strong>{walletName}</strong>
                  <small className="address-compact" title={entry.row.walletAddress}>
                    {shortWalletAddress(entry.row.walletAddress)}
                  </small>
                </>
              ) : (
                <strong className="address-compact" title={entry.row.walletAddress}>
                  {shortWalletAddress(entry.row.walletAddress)}
                </strong>
              )}
              <CopyAddressButton address={entry.row.walletAddress} />
            </span>
          </>
        );
      },
      cellProps: () => ({ title: 'Trader name and wallet address.' }),
    },
    {
      key: 'decision',
      header: 'Decision',
      headerProps: decisionHeaderProps('verdict', 'Sort by verdict'),
      render: (entry: DecisionTableRow) => {
        const { verdictTooltip, delay } = decisionTableContext(entry);
        const state = decisionStateFor(entry.verdict);
        const coverage = delay?.coverage ?? null;
        const pending = delay?.sim?.pendingDuneTargets ?? 0;
        const title =
          state === 'needs_data'
            ? `${verdictTooltip}\n\n${buildUsableCoverageText(coverage, delay?.sim)}\n${pending > 0 ? `${pending.toLocaleString()} Dune price legs remain fetchable.` : 'No unqueried Dune price legs remain; another normal fetch cannot add evidence.'}`
            : verdictTooltip;
        return (
          <span
            title={title}
            aria-label={title}
            className={`copytrade-verdict-badge ${DECISION_STATES[state].tone}`}
          >
            {DECISION_STATES[state].label}
          </span>
        );
      },
    },
    {
      ...
      headerProps: { title: "Age of the GMGN stats response this row's verdict was computed from" },
      render: (entry: DecisionTableRow) => (
        <span className={!entry.freshStats || !entry.duneFresh ? 'copytrade-warning-text' : ''}>
          {entry.long?.fetchedAt
            ? `GMGN ${freshnessLabel(entry.long.fetchedAt).text}`
            : 'GMGN 30d missing'}{' '}
          ·{' '}
          {entry.duneEvidenceAt
            ? `Dune ${freshnessLabel(entry.duneEvidenceAt).text}`
            : 'Dune 30d missing'}
        </span>
      ),
      cellProps: (entry: DecisionTableRow) => ({
        title: `GMGN 30d stats: ${entry.long?.fetchedAt ? formatFetchTime(entry.long.fetchedAt) : 'not available'}. Dune 30d simulation: ${entry.duneEvidenceAt ? formatFetchTime(entry.duneEvidenceAt) : 'not available'}. Both must be current for a candidate verdict.`,
      }),
    },
    {
      key: 'gmgnPnl',
      header: '30d GMGN PnL',
      headerProps: decisionHeaderProps(
        'gmgnPnl',
        'GMGN-reported 30-day realized profit before delay, fees, slippage, or Dune matching',
      ),
      render: (entry: DecisionTableRow) => (
        <>
          <strong
            className={
              entry.historical30d === null ? '' : entry.historical30d >= 0 ? 'positive' : 'negative'
            }
          >
            {entry.long?.realizedProfit === null || entry.long?.realizedProfit === undefined
              ? formatPct(entry.historical30d)
              : formatUsd(entry.long.realizedProfit)}
          </strong>
          {entry.long?.realizedProfit !== null && entry.long?.realizedProfit !== undefined && (
            <small>{formatPct(entry.historical30d)} GMGN return</small>
          )}
        </>
      ),
      cellProps: () => ({
    ...
    {
      key: 'gmgnTrades',
      header: '30d GMGN trades',
      headerProps: { title: 'Total GMGN-reported buy and sell transactions in the 30-day period' },
      render: (entry: DecisionTableRow) => (
        <>
          <strong>
            {entry.long?.buyCount === null ||
            entry.long?.buyCount === undefined ||
            entry.long?.sellCount === null ||
            entry.long?.sellCount === undefined
              ? '—'
              : formatCount(entry.long.buyCount + entry.long.sellCount)}
          </strong>
          <small>
            {entry.long?.buyCount === null ||
            entry.long?.buyCount === undefined ||
            entry.long?.sellCount === null ||
            entry.long?.sellCount === undefined
              ? 'GMGN count unavailable'
              : `${formatCount(entry.long.buyCount)} buys · ${formatCount(entry.long.sellCount)} sells`}
          </small>
        </>
      ),
      cellProps: () => ({
        title: 'Total GMGN-reported buy and sell transactions in the 30-day period.',
      }),
    },
    {
      key: 'copyMedian',
      header: '30d copy median',
      headerProps: decisionHeaderProps(
        'copyResult',
        'Median simulated return per copied trade after delay, fees, and slippage',
      ),
      render: (entry: DecisionTableRow) => {
        const { delay, simulatedMedian, copyLabel } = decisionTableContext(entry);
        return (
          <>
            <strong
              className={
                entry.verdict === 'Tested candidate' && simulatedMedian !== null
                  ? simulatedMedian >= 0
                    ? 'positive'
                    : 'negative'
                  : ''
            ...
            <small>
              {delay?.edge === null || delay?.edge === undefined
                ? 'After delay and costs'
                : `${delay.edge.toFixed(0)}% edge kept`}
            </small>
          </>
        );
      },
      cellProps: () => ({
        title:
          "Median simulated return per copied trade after the configured copy delay, fees, and slippage. The winner panel's portfolio P&L compounds the full trade path, so it can be much larger.",
      }),
    },
    {
      key: 'copyCapital',
      header: '$100 after copy',
      headerProps: decisionHeaderProps(
        'copyCapital',
        'Sort by cash-constrained simulated ending value from $100',
      ),
      render: (entry: DecisionTableRow) => {
        const { copyCapital } = decisionTableContext(entry);
        return (
          <strong
            className={copyCapital !== null ? (copyCapital >= 100 ? 'positive' : 'negative') : ''}
          >
            {formatCopyCapital(copyCapital)}
          </strong>
        );
      },
      cellProps: () => ({
        title:
          'Cash-constrained simulated ending value from a $100 starting portfolio after delay, fees, slippage, and gas.',
      }),
    },
    {
      key: 'evidence',
      header: 'Evidence',
      headerProps: decisionHeaderProps('coverage', 'Sort by Dune coverage'),
      render: (entry: DecisionTableRow) => {
        const { delay, evidenceLabel, coverageClass } = decisionTableContext(entry);
        return (
          <strong>
            <i className={`dune-coverage-dot ${coverageClass}`} /> {evidenceLabel}
          </strong>
        );
      },
      cellProps: (entry: DecisionTableRow) => ({
        title: buildUsableCoverageText(entry.delay?.coverage, entry.delay?.sim),
      }),
    },
    {
      key: 'duneCoverage',
      header: 'Dune coverage',
      headerProps: decisionHeaderProps('coverage', 'Sort by Dune coverage'),
      render: (entry: DecisionTableRow) => {
        const { delay, coverageClass } = decisionTableContext(entry);
        const coverage = delay?.coverage ?? null;
        const canWiden =
          coverage !== null &&
          coverage < 100 &&
          (delay?.sim?.pendingDuneTargets ?? 0) === 0 &&
          delay?.sim;
        return (
          <>
            <strong>
              <i className={`dune-coverage-dot ${coverageClass}`} />{' '}
              {coverage === null ? '—' : `${coverage.toFixed(1)}%`}
            </strong>
            <small>
              {delay?.sim
                ? `${delay.sim.copiedTrades.toLocaleString()} / ${delay.sim.roundTripsConsidered.toLocaleString()} round trips`
                : 'No Dune simulation'}
            </small>
            {canWiden && (
              <button
                type="button"
                className="quiet copytrade-widen-button"
                onClick={(event) => {
                  event.stopPropagation();
                  setWideRetryOutcome(null);
                  setWideRetryWallet(entry.row.walletAddress);
                }}
                title="All current Dune legs were attempted, but some produced no usable price. Preview a wider Dune search window before spending credits."
              >
                Widen search
              </button>
            )}
    ...
    },
    {
      key: 'hold',
      header: 'Typical hold',
      headerProps: decisionHeaderProps('activity', 'Sort by typical holding time'),
      render: (entry: DecisionTableRow) => (
        <strong>{formatHoldingTime(entry.delay?.hold ?? null)}</strong>
      ),
      cellProps: (entry: DecisionTableRow) => ({
        title: `Median time this trader holds a position. Evidence is ${entry.freshStats ? 'fresh' : 'older than 24 hours'}.`,
      }),
    },
    {
      key: 'under15s',
      header: 'GMGN ≤15s trades',
      headerProps: {
        title: 'Percentage of completed GMGN buy/sell pairs held for 15 seconds or less',
      },
      render: (entry: DecisionTableRow) => {
        ...
        return (
          <>
            <strong>{fast === null ? '—' : `${fast}%`}</strong>
            <small>
              {fast === null
                ? 'No paired trades'
                : `${fastCount.toLocaleString()} / ${paired.toLocaleString()} pairs`}
            </small>
          </>
        );
      },
      cellProps: () => ({
        title:
          'GMGN-derived percentage of completed buy/sell pairs held for 15 seconds or less. Only pairs with both timestamps are counted; incomplete or truncated history is not included.',
      }),
    },
    {
      key: 'gmgnTags',
      header: 'GMGN tags',
      headerProps: { title: 'Labels returned by GMGN for this wallet' },
      render: (entry: DecisionTableRow) => (
        <span className="copytrade-tag-list">
          {(entry.row.gmgnTags ?? entry.row.riskFlags).length > 0
            ? (entry.row.gmgnTags ?? entry.row.riskFlags).map((tag) => (
      ...
      cellProps: () => ({
        title:
          'These are GMGN-provided wallet labels, not labels inferred by this application. Hover a tag for a short explanation.',
      }),
    },
    {
      key: 'details',
      ...
      render: (entry: DecisionTableRow) => (
        <button
          type="button"
          className="copytrade-detail-button"
          onPointerUp={(event) => {
            event.stopPropagation();
            openStatsDetail(entry.row.walletAddress);
          }}
          onClick={(event) => {
            event.stopPropagation();
            openStatsDetail(entry.row.walletAddress);
          }}
          title="Open full trader details."
          aria-label="Open trader details"
        >
          ›
        </button>
      ),
      cellProps: () => ({ className: 'copytrade-decision-chevron' }),
    },
  ].filter(
    (column) => column.key === 'details' || decisionColumns[column.key as DecisionColumnKey],
  );
  const hasPositiveCopyGain = (entry: {
    delay?: { sim?: { portfolio?: { realizedPnlUsd?: number | null } | null } | null } | null;
  }): boolean => {
  ...
  const duneNeedsDataCount = unifiedTraderRows.filter(
    (entry) =>
      duneScopeWalletSet.has(entry.row.walletAddress) &&
      decisionStateFor(entry.verdict) === 'needs_data',
  ).length;
  const duneNeedsDataWalletAddresses = unifiedTraderRows
    .filter(
      (entry) =>
        duneScopeWalletSet.has(entry.row.walletAddress) &&
        decisionStateFor(entry.verdict) === 'needs_data',
    ...
    .map((entry) => entry.row.walletAddress);
  const duneResultWalletSet = new Set(
    (copySimulation?.wallets ?? []).map((wallet) => wallet.walletAddress),
  );
  const duneResultCount = researchWalletAddresses.filter((wallet) =>
    duneResultWalletSet.has(wallet),
  ).length;
  const copyDelaySurvivorWallets = new Set(
    copyDelayRows.filter(hasPositiveCopyDelayGain).map((entry) => entry.row.walletAddress),
  );
  ...
  const visibleCombinedStatsRows = visibleGmgnStatsRows.map((row) => ({
    row,
    delay: copyDelayByWallet.get(row.walletAddress) ?? null,
  }));
  ...
  const copyDelayDuneEligibleWallets = [...copyDelayRows]
    .sort(
      (left, right) =>
        (left.totalTrades7d ?? Number.POSITIVE_INFINITY) -
        (right.totalTrades7d ?? Number.POSITIVE_INFINITY),
    )
    .map((entry) => entry.row.walletAddress);
  const wideRetryExhausted =
    copySimulationRunStatus?.mode === 'wide_retry' &&
    !copySimulationRunStatus.running &&
    copySimulationRunStatus.retryableTargetsRemaining === 0;
  const wideRetryRemaining =
    copySimulationRunStatus?.mode === 'wide_retry'
  ...
  const exportCopyDelayCsv = () => {
    ...
    saveCsv(
      visibleCopyDelayRows.map((entry) => ({
        trader: entry.name,
  ...
  const exportUnifiedTraderCsv = async () => {
    ...
    try {
      ...
      let historyResponse: CopyTradeHistoryBulkResponse;
      try {
        historyResponse = await api<CopyTradeHistoryBulkResponse>('/api/copytrade/trades/bulk', {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ walletAddresses: exportWalletAddresses }),
        });
      } catch (error: unknown) {
        throw new Error(
          `Export stopped: stored trade history failed for all ${exportWalletAddresses.length} wallets; no partial CSV was created. ${error instanceof Error ? error.message : String(error)}`,
        );
      }
      const historyByWallet = new Map(
        historyResponse.histories.map((history) => [history.walletAddress, history]),
      );
      const historyResults = sortedUnifiedTraderRows.map((entry) => ({
        entry,
        history: historyByWallet.get(entry.row.walletAddress) ?? null,
        error: historyByWallet.has(entry.row.walletAddress)
          ? null
          : 'Bulk response did not include this wallet.',
      }));
      const failedHistory = historyResults.filter((result) => result.error !== null);
      if (failedHistory.length > 0) {
        const sample = failedHistory
          .slice(0, 3)
          .map(({ entry, error }) => `${entry.row.walletAddress.slice(0, 6)}…: ${error}`)
          .join(' · ');
        throw new Error(
          `Export stopped: stored trade history failed for ${failedHistory.length} wallet${failedHistory.length === 1 ? '' : 's'}; no partial CSV was created. ${sample}`,
        );
      }
      const exportedRows = historyResults.map(({ entry, history }) => {
        const scrutiny = scrutinyByWallet.get(entry.row.walletAddress) ?? null;
        const sells =
          history?.rows.filter((trade) => trade.eventType.toLowerCase().startsWith('sell')) ?? [];
        ...
        const evidenceLabel = entry.delay?.sim
          ? `${entry.delay.sim.copiedTrades.toLocaleString()} / ${entry.delay.sim.roundTripsConsidered.toLocaleString()}`
          : 'Not measured';
        const gmgnRawPayloads = Object.fromEntries(
          gmgnStatsRecords
            .filter((record) => record.walletAddress === entry.row.walletAddress)
            .map((record) => [
              record.period,
              { fetched_at: record.fetchedAt, raw_payload: record.rawPayload },
            ]),
        );
        const tooltipContext = {
          ...
          gmgn_pnl:
            'GMGN-reported 30-day realized profit before delay, fees, slippage, or Dune matching.',
          copy_median:
            'Median simulated return per copied trade after the configured copy delay, fees, and slippage. The portfolio P&L compounds the full trade path and can be much larger.',
          copy_capital:
            'Cash-constrained simulated ending value from a $100 starting portfolio after delay, fees, slippage, and gas.',
          ...
          gmgn_under_15_seconds:
            'GMGN-derived percentage of completed buy/sell pairs held for 15 seconds or less. Only pairs with both timestamps are counted; incomplete or truncated history is not included.',
          gmgn_tags: Object.fromEntries(tags.map((tag) => [tag, gmgnTagInfo(tag).text])),
          dialog: {
            after_copy_path:
              'Simulated cash balance after each completed copied trade using the configured delay, fees, slippage, gas, stake, and position limit.',
            trade_reconciliation:
              'The chart contains the simulated trade path; the stored history table contains locally saved sell rows. Missing rows indicate that the two views cannot be fully reconciled.',
            stored_sell_history:
              'Only stored sell rows are shown because they close positions and provide the realized result. Buy rows are used for matching but are not displayed in this table.',
            outcome_distribution:
              'GMGN-reported counts of positions by result, showing whether gains come from many small wins or a few large ones.',
            flow_and_costs:
              'Gross amounts and fees reported by GMGN for the selected period. These describe the trader, not the delayed-copy result.',
          },
        };
        const sellHistory = sells.map((trade) => ({
          id: trade.id,
          observed_timestamp: trade.observedTimestamp,
          token_address: trade.tokenAddress,
          token_symbol: trade.tokenSymbol,
          hold_seconds: holds.get(trade.id) ?? null,
          realized_return_percent: tradeReturnPercent(
            trade.eventType,
            trade.costUsd,
            trade.buyCostUsd,
          ),
        ...
        return {
          ...
          decision_snapshot_json: JSON.stringify({
            verdict: entry.verdict,
          ...
          scrutiny_checks_json: scrutiny ? JSON.stringify(scrutiny.checks) : null,
          ...
          risk_evidence_json: entry.row.riskEvidence
            ? JSON.stringify(entry.row.riskEvidence)
            : null,
          ...
          gmgn_under_15_seconds_pairs: entry.row.riskEvidence
            ? `${entry.row.riskEvidence.under15SecondsCount}/${entry.row.riskEvidence.pairedTradeCount}`
            : null,
          ...
          dialog_data_json: JSON.stringify({
            ...
            decision_checklist: entry.decisionEvidence,
            scrutiny_checks: scrutiny?.checks ?? null,
            gmgn_7d: entry.short,
            gmgn_30d: entry.long,
            gmgn_tags: tags,
            ...
            trade_path:
              entry.delay?.sim?.portfolio?.tradeCapitalPath ??
              entry.delay?.sim?.portfolio?.capitalPath ??
              null,
      ...
      const compactRows = exportedRows.map(
        ({
          wallet_row_json,
          decision_snapshot_json,
          decision_checklist_json,
          scrutiny_checks_json,
          gmgn_7d_json,
          gmgn_30d_json,
          gmgn_raw_payloads_json,
          risk_evidence_json,
          copy_evidence_json,
          dialog_data_json,
          stored_trade_history_json,
          stored_sell_history_json,
          stored_history_coverage_json,
          ...row
        }) => row,
      );
      ...
      const fullDetails = exportedRows.map((row) => ({
        ...
        table:
          compactRows.find((compactRow) => compactRow.wallet_address === row.wallet_address) ??
          null,
        decision_checklist: parseExportJson(row.decision_checklist_json),
        scrutiny_checks: parseExportJson(row.scrutiny_checks_json),
      ...
      await saveCompressedJson(
        {
          format: 'vantage-crypto-full-export-v2',
          generated_at: new Date().toISOString(),
          wallets: fullDetails,
        },
        `copytrade-full-details-${timestamp}.json.gz`,
      );
    ...
    sortedGmgnStatsRows
      .map((row) => {
        ...
        return {
          ...
          delaySeconds,
          delayShare:
            holdSeconds !== null && Number.isFinite(holdSeconds) && holdSeconds > 0
              ? (delaySeconds / holdSeconds) * 100
              : null,
          walletAgeDays: row.riskEvidence?.walletAgeDays ?? null,
        };
      })
      .find((example) => example.holdSeconds !== null) ?? null;
  const gmgnStatsRowsWithData = copyTradeRows.filter(
    (row) => gmgnLeaderboardMetrics[row.walletAddress] || gmgnStatsByWallet.has(row.walletAddress),
  ).length;
  const gmgnStatsFreshRows = copyTradeRows.filter((row) => {
    const periods = gmgnStatsByWallet.get(row.walletAddress);
    const fetched = periods?.get('30d')?.fetchedAt;
    return isThirtyDayDecisionEvidenceFresh(fetched);
  }).length;
  const exportGmgnAggregateCsv = () => {
    ...
    const rows: Array<Record<string, unknown>> = sortedGmgnAggregateRows.map((row) => {
      ...
      return {
        ...
        hundred_dollar_equivalent:
          stats?.realizedProfitPnlPercent === null || stats?.realizedProfitPnlPercent === undefined
            ? null
            : 100 * (1 + stats.realizedProfitPnlPercent / 100),
  ...
  const loadGmgnStats = async (
    limitOverride = copyTradeLimit,
    snapshotOverride = selectedRosterSnapshotId,
  ) => {
    setGmgnStatsLoading(true);
    try {
      const query = `?limit=${limitOverride}${snapshotOverride ? `&snapshotId=${snapshotOverride}` : ''}`;
      const result = await api<{
        stats: GmgnStatsRecord[];
        leaderboard: Record<string, GmgnLeaderboardMetric>;
      }>(`/api/copytrade/stats${query}`);
      ...
      setGmgnStatsLoading(false);
    }
    return null;
  };

  function buildWalletScreenSummary(
    records: GmgnStatsRecord[],
    walletCount: number,
  ): WalletScreenSummary {
    const periodRows = records.filter((record) => record.period === '30d');
    const aggregateRows = periodRows
      .map(parseAggregateRecord)
      .filter((row): row is GmgnAggregateStats => row !== null);
    const fastWallets = aggregateRows.filter(
      (row) => row.averageHoldingPeriodSeconds !== null && row.averageHoldingPeriodSeconds < 60,
    ).length;
    const activityLeaders = periodRows
      .map((record) => ({ record, aggregate: parseAggregateRecord(record) }))
      .filter(
        (row): row is { record: GmgnStatsRecord; aggregate: GmgnAggregateStats } =>
          row.aggregate !== null,
      )
      .map(({ record, aggregate }) => {
        const trader = copyTradeRows.find((row) => row.walletAddress === record.walletAddress);
        return {
          wallet: record.walletAddress,
          name: trader?.name ?? null,
          trades: (aggregate.buyCount ?? 0) + (aggregate.sellCount ?? 0),
          rank: trader?.rankHistory.currentRank ?? null,
          netProfit: aggregate.realizedProfit,
          averageHoldSeconds: aggregate.averageHoldingPeriodSeconds,
        };
      })
    ...
    return {
      ...
      snapshotId: selectedRosterSnapshotId,
      walletCount,
      statsWalletCount: aggregateRows.length,
      fastWallets,
      notFastWallets: Math.max(0, aggregateRows.length - fastWallets),
      missingStatsWallets: Math.max(0, walletCount - aggregateRows.length),
      totalTrades: activityLeaders.reduce((sum, row) => sum + row.trades, 0),
      maxTrades: activityLeaders[0]?.trades ?? 0,
      maxTradesWallet: activityLeaders[0]?.wallet ?? null,
      activityLeaders,
      periodDays: 30,
      averageHoldThresholdSeconds: 60,
      lastFetchedAt,
    };
  }

  const startGmgnStatsFetch = async (
    limitOverride = copyTradeLimit,
    snapshotOverride = selectedRosterSnapshotId,
  ): Promise<boolean> => {
    ...
    try {
      const body = await api<GmgnStatsFetchStatus>('/api/copytrade/stats/fetch', {
        ...
        body: JSON.stringify({
          limit: limitOverride,
          snapshotId: snapshotOverride ?? undefined,
          periods: ['7d', '30d'],
          maxAgeHours: 24,
        }),
      });
  ...
  const stopGmgnStatsFetch = async () => {
    try {
      setGmgnStatsStatus(
        await api<GmgnStatsFetchStatus>('/api/copytrade/stats/stop', { method: 'POST' }),
      );
    } catch (error: unknown) {
      setCopyTradeError(error instanceof Error ? error.message : String(error));
    }
  ...
  const syncLatestWalletRoster = async (
    ...
    try {
      const result = await api<{
        ...
        researchFilter?: { orderby: string; minWinrate30d: number };
      }>('/api/copytrade/roster/refresh', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ limit: 100, useSavedSnapshot }),
      });
      ...
      setRosterChange({
        joinedWallets: result.live ? (result.joinedWallets ?? []) : [],
        leftWallets: result.live ? (result.leftWallets ?? []) : [],
      ...
      setRosterRefreshError(
        `Refresh failed: ${errorMessage}. The previous roster was left unchanged.`,
      );
  ...
  const openRosterComparison = async () => {
    ...
    try {
      ...
      setRosterComparisonLoading(false);
    }
  };
  ...
  const approveAndResearch = async () => {
    ...
    if (skipEliminatedInDune && eliminationReport?.eliminated.length && !triageHasCurrentInputs) {
      const freshTriage = await loadElimination(
        screeningSummary.snapshotId ?? selectedRosterSnapshotId,
      );
      ...
      approvedWalletAddresses = allWallets.filter(
        (wallet) =>
          !manuallyExcluded.has(wallet) &&
    ...
    try {
      ...
      const beforeVerdictByWallet = Object.fromEntries(
        unifiedTraderRows.map((entry) => [entry.row.walletAddress, entry.verdict]),
      );
      ...
      const duneResult = await runCopySimulationBatch(
        approvedWalletAddresses,
        30,
        false,
        screeningSummary.snapshotId ?? selectedRosterSnapshotId,
      );
      ...
      setResearchUpdateSummary({
        completedAt: new Date().toISOString(),
        rosterSnapshotId: screeningSummary.snapshotId,
        rosterWalletCount: screeningSummary.walletCount,
        rosterAdded: 0,
        rosterAlreadyPresent: screeningSummary.walletCount,
        statsStatus: 'completed',
        statsWalletDone: screeningSummary.statsWalletCount,
        statsWalletTotal: screeningSummary.walletCount,
        statsRequests: 0,
        statsReused: screeningSummary.statsWalletCount,
        duneSubmitted: duneResult?.targetsSubmitted ?? 0,
        duneBatches: duneResult?.batchesRun ?? 0,
        duneExhausted: duneResult?.exhausted ?? false,
        beforeRows,
        beforeStatsRows,
        beforeMatched,
        beforeVerdicts,
        beforeVerdictByWallet,
        leaderboardOrderby: 'pnl_30d',
        leaderboardMinWinrate30d: 0.5,
      });
      setMessage(
  ...
  const toggleDecisionColumn = (key: DecisionColumnKey) =>
    setDecisionColumns((current) => {
      const next = { ...current, [key]: !current[key] };
      try {
        localStorage.setItem(decisionColumnsStorageKey, JSON.stringify(next));
      } catch {
        /* preference is optional */
      }
      return next;
    });
  const medianAverageConflicts = copyTradeRows.filter(
    (row) =>
  ...
  const unifiedTraderRowByWallet = new Map(
    unifiedTraderRows.map((entry) => [entry.row.walletAddress, entry]),
  );
  const scopedCopySimulationWallets = (copySimulation?.wallets ?? []).filter((wallet) =>
    duneNeedsDataWalletSet.has(wallet.walletAddress),
  );
  const sumWalletTargetField = (
    key: 'pendingDuneTargets' | 'duneNoMatchTargets' | 'duneMatchedTargets',
  ): number => scopedCopySimulationWallets.reduce((total, wallet) => total + (wallet[key] ?? 0), 0);
  const duneMatchedTargets = sumWalletTargetField('duneMatchedTargets');
  const preciseTargetsRemaining = copySimulation
    ? sumWalletTargetField('pendingDuneTargets')
    : null;
  ...
  const copyTradeProgressPercent =
    copyTradeStatus?.totalTradeProgressPercent ??
    (copyTradeStatus && (copyTradeStatus.walletTotal ?? 0) > 0
      ? Math.min(
          100,
          ((copyTradeStatus.walletDone ?? 0) / (copyTradeStatus.walletTotal ?? 1)) * 100,
        )
      : null);
  const rosterFetchStatus = copyTradeStatus?.scope === 'roster' ? copyTradeStatus : null;
  const rosterProgressPercent =
    rosterFetchStatus?.totalTradeProgressPercent ??
    (rosterFetchStatus?.walletTotal
      ? Math.min(100, ((rosterFetchStatus.walletDone ?? 0) / rosterFetchStatus.walletTotal) * 100)
      : null);
  const copyTradeProcessedRows = copyTradeStatus
    ? (copyTradeStatus.tradesFetched ?? 0) +
      (copyTradeStatus.tradesDuplicate ?? 0) +
      (copyTradeStatus.tradesDailyCapped ?? 0)
    : 0;
  ...
  const researchStageIndex = researchUpdateStage?.startsWith('Refreshing the top 100')
    ? 0
    : researchUpdateStage?.startsWith('Fetching complete GMGN trade history')
      ? 1
      : researchUpdateStage?.startsWith('Fetching 7-day')
        ? 2
        : researchUpdateStage?.startsWith('Fetching missing Dune')
          ? 3
          : researchUpdateStage?.startsWith('Refreshing the decision')
            ? 4
            : -1;
  const researchStepState = (index: number): 'done' | 'active' | 'waiting' | 'failed' => {
    if (researchUpdateFailedStage !== null)
      return index < researchUpdateFailedStage
        ? 'done'
        : index === researchUpdateFailedStage
          ? 'failed'
          : 'waiting';
    if (!researchUpdateBusy && !researchUpdateStage?.startsWith('Update stopped:')) return 'done';
    return index < researchStageIndex
      ? 'done'
      : index === researchStageIndex
        ? 'active'
        : 'waiting';
  };
  const researchStepDetail = (
    index: number,
    state: ReturnType<typeof researchStepState>,
  ): string => {
    ...
    if (index === 0)
      return state === 'done'
        ? 'Roster saved.'
        : state === 'active'
          ? 'Refreshing the top 100 roster…'
          : 'Waiting to select the roster.';
    if (index === 1) {
      if (copyTradeStatus?.running)
        return `${copyTradeStatus.walletDone ?? 0} / ${copyTradeStatus.walletTotal ?? 100} wallets · ${copyTradeStatus.tradesFetched ?? 0} new trades · ${copyTradeStatus.requestsMade ?? 0} requests`;
      if (state === 'done')
        return `${copyTradeStatus?.walletDone ?? copyTradeStatus?.walletTotal ?? 0} wallets complete · saved in SQLite.`;
      return state === 'active'
        ? 'Starting the GMGN history fetch…'
        : 'Waiting for the roster step.';
    }
    if (index === 2) {
      if (gmgnStatsStatus?.running)
        return `${gmgnStatsStatus.walletDone} / ${gmgnStatsStatus.walletTotal} wallets · ${gmgnStatsStatus.requestsMade} requests`;
      if (state === 'done')
        return `Complete · ${gmgnStatsStatus?.skippedFresh ?? 0} saved summaries reused.`;
      return state === 'active' ? 'Starting the GMGN summary fetch…' : 'Waiting for GMGN history.';
    }
    if (index === 3) {
      if (copySimulationRunStatus?.running)
        return `${copySimulationRunStatus.targetsProcessed} / ${copySimulationRunStatus.targetsTotal} Dune targets · ${copySimulationRunStatus.remainingTargets} remaining`;
      if (state === 'done')
        return `Complete · ${copySimulationRunStatus?.storedTargets ?? 0} targets stored.`;
      return state === 'active'
        ? 'Starting the Dune fetch…'
        : 'Waiting for approval and GMGN summaries.';
    }
    return state === 'done'
      ? 'Decision table refreshed.'
      : state === 'active'
        ? 'Recomputing decisions from saved evidence…'
        : 'Waiting for Dune results.';
  };
  return (
    <main
    ...
    >
      ...
      {copyTradeSubTab === 'wallet-stats' && (
        <section
          ...
          className="menu-section panel copytrade-research-route"
        >
          <PanelHeading
          ...
          />
          {walletStatsTableLoading && (
            <div className="copytrade-analysis-status running" role="status" aria-live="polite">
              <span className="loading-spinner" aria-hidden="true" />
              <div>
                <strong>Loading saved trader evidence…</strong>
                <small>
                  Reading the roster, GMGN summaries, Dune results, and consistency report from
                  SQLite. Decisions stay hidden until every required report finishes.
                </small>
              </div>
            </div>
          )}
          {walletStatsEvidenceError && (
            <div className="copytrade-analysis-status error" role="alert">
              <div>
                <strong>Decision evidence could not be loaded</strong>
                <small>{walletStatsEvidenceError} No temporary verdicts are shown.</small>
              </div>
              <button
                type="button"
                className="secondary"
                onClick={() => setWalletStatsReloadKey((key) => key + 1)}
              >
          ...
          <div className="copytrade-top-fetch-controls">
            ...
            {researchUpdateBusy && (
              <div className="copytrade-live-fetch-status" role="status" aria-live="polite">
                <div>
                  <strong>{researchUpdateStage ?? 'Preparing research…'}</strong>
                  <small>
                    Existing GMGN and Dune responses are retained; only missing work is requested.
                  </small>
                </div>
                {gmgnStatsStatus?.running && (
                  <div>
                    <b>GMGN summaries</b>
                    <span>
                      {gmgnStatsStatus.walletDone} / {gmgnStatsStatus.walletTotal} wallets ·{' '}
                      {gmgnStatsStatus.requestsMade} requests
                    </span>
                    <small>Fetching and parsing wallet statistics</small>
                  </div>
                )}
                {copySimulationRunStatus?.running && (
                  <div>
                    <b>Dune copy prices</b>
                    <span>
                      Query {copySimulationRunStatus.currentBatch || '—'} /{' '}
                      {copySimulationRunStatus.batchesTotal || '—'} ·{' '}
                      {copySimulationRunStatus.targetsProcessed.toLocaleString()} /{' '}
                      {copySimulationRunStatus.targetsTotal.toLocaleString()} targets
                    </span>
                    <small>
                      {copySimulationRunStatus.storedTargets.toLocaleString()} stored ·{' '}
                      {copySimulationRunStatus.failedTargets.toLocaleString()} failed ·{' '}
                      {copySimulationRunStatus.remainingTargets.toLocaleString()} remaining · Dune{' '}
                      {copySimulationRunStatus.duneState
                        ?.replace('QUERY_STATE_', '')
                        .toLowerCase() ?? 'submitting'}
                    </small>
                    <small>{copySimulationRunStatus.message}</small>
                  </div>
                )}
              </div>
            )}
            <div className="copytrade-workflow-actions">
              <div className="copytrade-workflow-row">
                <div className="copytrade-workflow-label">
                  ...
                  <div>
                    <strong>GMGN screening</strong>
                    <small>
                      {visibleWalletScreenSummary
                        ? `${visibleWalletScreenSummary.statsWalletCount}/${visibleWalletScreenSummary.walletCount} wallets · fetched ${formatFetchTime(visibleWalletScreenSummary.lastFetchedAt)}`
                        : 'No saved screening yet'}
                    </small>
                  </div>
                  <button
                    type="button"
                    className="copytrade-icon-button"
                    onClick={() => void openRosterComparison()}
                    disabled={rosterComparisonLoading}
                    title="Compare the current GMGN top 100 with the previous saved list"
                    aria-label="Open GMGN top 100 comparison"
                  >
                    {rosterComparisonLoading ? '…' : '☷'}
                  </button>
                </div>
                <div className="copytrade-workflow-status">
                  <strong>
                    {rosterFetchStatus?.running
                      ? 'Fetching…'
                      : visibleWalletScreenSummary
                        ? 'Saved screening available'
                        : 'Not fetched'}
                  </strong>
                  <small>Complete wallet history and 30-day summaries</small>
                </div>
                {rosterFetchStatus?.running && (
                  <div className="copytrade-top-fetch-detail" role="status" aria-live="polite">
                    <div className="copytrade-top-fetch-detail-head">
                      <strong>
                        {rosterFetchStatus.walletDone ?? 0}/{rosterFetchStatus.walletTotal ?? 0}{' '}
                        wallets
                      </strong>
                      <span>
                        {rosterProgressPercent === null
                          ? 'Progress calculating'
                          : `${rosterProgressPercent.toFixed(1)}%`}
                      </span>
                    </div>
                    <progress max={100} value={rosterProgressPercent ?? 0} />
                    <small>
                      {rosterFetchStatus.currentWalletAddress
                        ? `Current: ${shortAddress(rosterFetchStatus.currentWalletAddress)}`
                        : 'Preparing wallet'}{' '}
                      · {(rosterFetchStatus.storedTradesTotal ?? 0).toLocaleString()} stored ·{' '}
                      {(rosterFetchStatus.tradesFetched ?? 0).toLocaleString()} new ·{' '}
                      {(rosterFetchStatus.tradesDuplicate ?? 0).toLocaleString()} duplicates ·{' '}
                      {(rosterFetchStatus.requestsMade ?? 0).toLocaleString()} requests
                    </small>
                    <small>
                      {(rosterFetchStatus.failedWallets ?? 0).toLocaleString()} failed wallets ·{' '}
                      {rosterFetchStatus.currentWalletEstimatedRemainingSeconds == null
                        ? 'Wallet ETA calculating'
                        : `${formatSeconds(rosterFetchStatus.currentWalletEstimatedRemainingSeconds)} current wallet remaining`}{' '}
                      ·{' '}
                      {rosterFetchStatus.totalEstimatedRemainingSeconds == null
                        ? 'Total ETA calculating'
                        : `${formatSeconds(rosterFetchStatus.totalEstimatedRemainingSeconds)} total remaining`}
                    </small>
                  </div>
                )}
                {rosterFetchStatus?.running ? (
                  <button
                    className="primary copytrade-stop-button"
                    disabled={copyTradeStopBusy}
                    onClick={() => void stopCopyTradeFetch()}
                  >
                    {copyTradeStopBusy ? 'Pausing…' : 'Pause GMGN fetch'}
                  </button>
                  ...
                  <button
                    ...
                    onClick={() => void resumeCopyTradeFetch()}
                  >
                    {copyTradeResumeBusy ? 'Resuming…' : 'Resume GMGN fetch'}
                  </button>
                ) : (
                  <>
                    <label
                      className="primary copytrade-file-button"
                      title="Import a GMGN leaderboard JSON file or Chrome extension export. This does not call GMGN."
                    >
                      {rosterImportBusy ? 'Importing…' : 'Import top 100 JSON'}
                      <input
                        type="file"
                        accept="application/json,.json"
                        disabled={
                        ...
                        onChange={(event) => {
                          const file = event.target.files?.[0];
                          event.target.value = '';
                          if (file) void importTopWalletRoster(file);
                        }}
                      />
                    </label>
                    {(manualRosterImported || selectedRosterSnapshotId !== null) && (
                      <button
                        className="primary"
                        disabled={
                          researchUpdateBusy ||
                        ...
                        onClick={() => void fetchImportedGmgnHistory()}
                        title="Fetch GMGN trade history and 30-day summaries for the imported roster. The leaderboard itself is never fetched by this button."
                      >
                        {researchUpdateBusy ? 'Fetching history…' : 'Fetch imported GMGN history'}
                      </button>
                    )}
                  </>
                )}
                {!rosterFetchStatus?.running && (
                  <small className="copytrade-manual-roster-note">
                    Live GMGN leaderboard fetching is disabled here. Upload the JSON response
                    directly or export a Chrome extension capture containing the leaderboard
                    request.
                  </small>
                )}
                {!rosterFetchStatus?.running &&
                  rosterFetchStatus?.runId !== null &&
                  rosterFetchStatus?.runId !== undefined && (
                    <button
                      className="quiet"
                      disabled={copyTradeResetBusy}
                      onClick={() => void resetCopyTradeFetch()}
                      title="Forget only the resumable cursor. Saved GMGN trades remain in SQLite."
                    >
                      {copyTradeResetBusy ? 'Resetting…' : 'Reset resume snapshot'}
                    </button>
                  )}
                ...
                {!rosterRefreshError && rosterRefreshMessage && (
                  <StatusBanner
                    tone={
                      rosterRefreshMessage.startsWith('Live refresh blocked') ? 'warning' : 'muted'
                    }
                    role="status"
                  >
                    {rosterRefreshMessage}
                  </StatusBanner>
                )}
                {!rosterFetchStatus?.running && rosterFetchStatus?.message && (
                  <StatusBanner tone={rosterFetchStatus.status === 'failed' ? 'warning' : 'muted'}>
                    {rosterFetchStatus.message}
                  </StatusBanner>
                )}
                {visibleWalletScreenSummary && (
              ...
              <div className="copytrade-workflow-row">
                <div className="copytrade-workflow-label">
                  ...
                  <div>
                    <strong>Dune copy test</strong>
                    <small>
                      {copySimulationRunStatus?.persistedRun?.completedAt
                        ? `Last fetched ${formatFetchTime(copySimulationRunStatus.persistedRun.completedAt)}`
                        : 'No saved Dune run yet'}
                    </small>
                  </div>
                </div>
                <div className="copytrade-workflow-status">
                  <strong>
                    {duneRunActive
                      ? 'Processing Dune targets…'
                      : preciseTargetsRemaining === 0
                        ? 'Dune complete · no new targets'
                        : copySimulationRunStatus?.outcome === 'complete'
                          ? 'Dune run complete'
                          : copySimulationRunStatus?.persistedRun
                            ? 'Saved Dune evidence available'
                            : 'Not fetched'}
                  </strong>
                  <small>
                    {duneRunActive && copySimulationRunStatus
                      ? `${copySimulationRunStatus.targetsProcessed.toLocaleString()} / ${copySimulationRunStatus.targetsTotal.toLocaleString()} targets processed · ${copySimulationRunStatus.remainingTargets.toLocaleString()} remaining`
                      : `${duneNeedsDataCount} wallets need more evidence; ${preciseTargetsRemaining === null ? 'Dune target count is loading' : `${preciseTargetsRemaining.toLocaleString()} price targets remain`}`}
                  </small>
                </div>
                {copySimulationRunStatus?.running ? (
                  <button
                    className="primary copytrade-stop-button"
                    onClick={() => void stopCopySimulation()}
                    disabled={copySimulationStopBusy}
                  >
                    {copySimulationStopBusy ? 'Stopping…' : 'Stop Dune fetch'}
                  </button>
                ) : (
                  <button
                    className="primary"
                    onClick={() => void approveAndResearch()}
                    disabled={
                      researchUpdateBusy ||
                      !visibleWalletScreenSummary ||
                      !duneNeedsDataWalletAddresses.length ||
                      !duneHasNewTargets ||
                      gmgnStatsStatus?.running
                    }
                  >
                    {researchUpdateBusy && visibleWalletScreenSummary
                      ? 'Fetching…'
                      : preciseTargetsRemaining === 0
                        ? 'No new Dune data to fetch'
                        : copySimulationRunStatus?.outcome === 'partial' ||
                            copySimulationRunStatus?.outcome === 'stopped'
                          ? `Resume Dune (${preciseTargetsRemaining?.toLocaleString() ?? '…'} targets · ${duneNeedsDataCount} wallets)`
                          : `Fetch ${preciseTargetsRemaining?.toLocaleString() ?? '…'} Dune targets (${duneNeedsDataCount} wallets)`}
                  </button>
                )}
              </div>
          ...
          {rosterComparisonOpen && rosterComparison && (
            <Modal
              onClose={() => setRosterComparisonOpen(false)}
              ariaLabel="GMGN top 100 comparison"
              dialogClassName="copytrade-roster-modal"
            >
              <div className="copytrade-modal-head">
                <div>
                  <p className="eyebrow">GMGN ROSTER</p>
                  <h3>Top 100 wallet comparison</h3>
                  <small>
                    {rosterComparison.currentCapturedAt
                      ? `Current: ${formatFetchTime(rosterComparison.currentCapturedAt)}`
                      : 'No saved snapshot'}
                  </small>
                </div>
                <button className="secondary" onClick={() => setRosterComparisonOpen(false)}>
                  Close
                </button>
              </div>
              {!rosterComparison.baselineAvailable ? (
                <p className="muted">
                  No earlier GMGN list is saved, so there is no baseline for identifying new or
                  departed wallets.
                </p>
              ) : (
                <>
                  <p className="copytrade-roster-modal-summary">
                    <strong>{rosterComparison.joined.length} new today</strong>
                    <span>·</span>
                    <strong>{rosterComparison.left.length} left the previous list</strong>
                  </p>
                  <div className="copytrade-roster-modal-left">
                    <h4>Left the list</h4>
                    {rosterComparison.left.length === 0 ? (
                      <p className="muted">None.</p>
                    ) : (
                      <div>
                        {rosterComparison.left.map((wallet) => (
                          <a
                            key={wallet.walletAddress}
                            href={`https://gmgn.ai/sol/address/${wallet.walletAddress}`}
                            target="_blank"
                            rel="noreferrer"
                          >
                            <span>{wallet.rankPosition ? `#${wallet.rankPosition}` : '—'}</span>
                            {wallet.name?.trim() || shortWalletAddress(wallet.walletAddress)} ↗
                          </a>
                        ))}
                      </div>
                    )}
              ...
              <h4>Current ranked list</h4>
              <div className="table-wrap copytrade-roster-modal-table">
                <table>
                  <thead>
                    <tr>
                      <th>Rank</th>
                  ...
                  <tbody>
                    {rosterComparison.current.map((wallet) => {
                      const isJoined = rosterComparison.joined.some(
                        (entry) => entry.walletAddress === wallet.walletAddress,
                      );
                      return (
                        <tr
                          key={wallet.walletAddress}
                          className={isJoined ? 'copytrade-roster-new-row' : undefined}
                        >
                          <td>#{wallet.rankPosition ?? '—'}</td>
                          <td>
                            <WalletIcon
                              url={wallet.iconUrl}
                              name={wallet.name || wallet.walletAddress}
                            />
                            <a
                              href={`https://gmgn.ai/sol/address/${wallet.walletAddress}`}
                              target="_blank"
                            ...
                            <small title={wallet.walletAddress}>
                              {shortWalletAddress(wallet.walletAddress)}
                            </small>
                          </td>
                          <td>{wallet.reportedPnl30d ?? '—'}</td>
                          <td>
                            {isJoined ? (
                              <span className="copytrade-roster-new-badge">NEW TODAY</span>
                            ) : (
                              'Previously listed'
                            )}
                          </td>
          ...
          {rosterChange && (
            <div className="copytrade-roster-change" role="status">
              <strong>
                {rosterChange.live
                  ? 'GMGN roster source: live request'
                  : 'GMGN roster source: saved snapshot'}
              </strong>
              {rosterChange.live ? (
                <span>
                  {rosterChange.joinedWallets.length} joined · {rosterChange.leftWallets.length}{' '}
                  left · unchanged wallets remain in the saved history.
                </span>
              ) : (
                <span>No new leaderboard was received, so no membership change was measured.</span>
              )}
              {(rosterChange.joinedWallets.length > 0 || rosterChange.leftWallets.length > 0) && (
                <Collapsible summary="Show wallet changes">
                  <div className="copytrade-roster-change-lists">
                    {rosterChange.joinedWallets.length > 0 && (
                      <div>
                        <b>Joined</b>
                        {rosterChange.joinedWallets.map((wallet) => (
                          <a
                            key={`joined-${wallet}`}
                    ...
                    {rosterChange.leftWallets.length > 0 && (
                      <div>
                        <b>Left</b>
                        {rosterChange.leftWallets.map((wallet) => (
                          <a
                            key={`left-${wallet}`}
                            href={`https://gmgn.ai/sol/address/${wallet}`}
                            target="_blank"
          ...
          <div className="copytrade-decision-filters">
            <label className="copytrade-filter-toggle">
              <input
                type="checkbox"
                checked={showDelaySurvivorsOnly}
                onChange={(event) => setShowDelaySurvivorsOnly(event.target.checked)}
              />{' '}
          ...
          <DataTable
            ...
            getRowKey={(entry) => `decision-${entry.row.walletAddress}`}
            wrapClassName="table-wrap copytrade-table-wrap copytrade-decision-table-wrap"
            tableClassName="copytrade-table copytrade-decision-table"
            rowProps={(entry) => {
              const isWinner = primary30dWinner?.row.walletAddress === entry.row.walletAddress;
              return {
                'data-wallet-address': entry.row.walletAddress,
                className: `copytrade-decision-row verdict-${entry.verdict.toLowerCase().replaceAll(' ', '-').replaceAll('/', '-')}${isWinner ? ' copytrade-decision-winner-row' : ''}`,
                onClick: () => openStatsDetail(entry.row.walletAddress),
                onPointerUp: (event) => {
            ...
            emptyMessage={
              walletStatsEvidenceError ? (
                ...
                <span className="copytrade-loading-inline">
                  <span className="loading-spinner" aria-hidden="true" /> Loading saved decision
                  evidence…
                </span>
              ) : (
          ...
          {wideRetryWallet &&
            (() => {
              const entry = unifiedTraderRows.find(
                (candidate) => candidate.row.walletAddress === wideRetryWallet,
              );
              const rank = entry?.row.rankHistory.currentRank ?? null;
              const simulation = entry?.delay?.sim;
              const coverage = entry?.delay?.coverage ?? null;
              const unmatchedTargets = simulation?.duneNoMatchTargets ?? 0;
              const estimatedQueries = Math.ceil(unmatchedTargets / 150);
              return (
                <Modal
                  onClose={() => {
                    if (!copySimulationRunBusy) setWideRetryWallet(null);
                  }}
                  ariaLabel="Widen Dune search window"
                  dialogClassName="copytrade-wide-retry-modal"
                >
                  <div className="copytrade-modal-head">
                    <div>
                      <p className="eyebrow">DUNE MATCH RECOVERY</p>
                      <h3>{entry?.name || shortAddress(wideRetryWallet)}</h3>
                      <small>GMGN rank {rank === null ? '—' : `#${rank}`}</small>
                    </div>
                    <button
                      className="secondary"
                      onClick={() => setWideRetryWallet(null)}
                      disabled={copySimulationRunBusy}
                    >
                      Close
                    </button>
                  </div>
                  <p className="muted">
                    All current trade legs have already been queried, but some returned no usable
                    price. This targeted retry searches only this wallet’s no-match legs with a
                    wider time window.
                  </p>
                  <div className="copytrade-modal-metrics">
                    <div>
                      <strong>{coverage === null ? '—' : `${coverage.toFixed(1)}%`}</strong>
                      <span>Current usable coverage</span>
                    </div>
                    <div>
                      <strong>
                        {simulation
                          ? `${simulation.copiedTrades}/${simulation.roundTripsConsidered}`
                          : '—'}
                      </strong>
                      <span>Matched round trips</span>
                    </div>
                    <div>
                      <strong>{unmatchedTargets.toLocaleString()}</strong>
                      <span>Estimated retry legs</span>
                    </div>
                    <div>
                      <strong>{estimatedQueries || '—'}</strong>
                      <span>Estimated Dune queries</span>
                    </div>
                  </div>
                  {wideRetryOutcome && (
                    <div
                      className={`copytrade-wide-retry-result ${wideRetryOutcome.after !== null && wideRetryOutcome.before !== null && wideRetryOutcome.after > wideRetryOutcome.before ? 'positive' : 'copytrade-warning-text'}`}
                      role="status"
                    >
                      <strong>
                        {wideRetryOutcome.after === null || wideRetryOutcome.before === null
                          ? 'Coverage result unavailable'
                          : wideRetryOutcome.after > wideRetryOutcome.before
                            ? `Coverage increased ${wideRetryOutcome.before.toFixed(1)}% → ${wideRetryOutcome.after.toFixed(1)}% (+${(wideRetryOutcome.after - wideRetryOutcome.before).toFixed(1)} percentage points)`
                            : wideRetryOutcome.after === wideRetryOutcome.before
                              ? `Coverage unchanged at ${wideRetryOutcome.after.toFixed(1)}%`
                              : `Coverage changed ${wideRetryOutcome.before.toFixed(1)}% → ${wideRetryOutcome.after.toFixed(1)}%`}
                      </strong>
                      <small>
                        Dune confirmed this after storing{' '}
                        {wideRetryOutcome.submitted.toLocaleString()} retry legs. The table was
                        reloaded from the saved matches.
                      </small>
                    </div>
                  )}
                  <label className="copytrade-wide-retry-window">
                    <span>Wider search window</span>
                    <select
                      value={wideRetryWindowMinutes}
                      onChange={(event) => setWideRetryWindowMinutes(Number(event.target.value))}
                      disabled={copySimulationRunBusy}
                    >
                      <option value={15}>15 minutes · first recovery step</option>
                      <option value={30}>30 minutes · medium cost</option>
                      <option value={60}>60 minutes · higher cost</option>
                      <option value={120}>120 minutes · last resort</option>
                    </select>
                  </label>
                  <p className="muted">
                    Dune does not provide a reliable credit estimate before execution. The leg count
                    and query count above are the budget preview; longer windows may scan more data.
                    Start with 15 minutes and expand only if coverage improves.
                  </p>
                  {copySimulationRunBusy && copySimulationRunStatus && (
                    <div className="copytrade-wide-retry-progress" role="status">
                      <strong>
                        {copySimulationRunStatus.targetsProcessed.toLocaleString()} /{' '}
                        {copySimulationRunStatus.targetsTotal.toLocaleString()} retry legs
                      </strong>
                      <progress
                        max={100}
                        value={
                  ...
                  <button
                    type="button"
                    className="primary"
                    onClick={() => void runWideRetryForWallet()}
                    disabled={copySimulationRunBusy || unmatchedTargets === 0}
                  >
                    {copySimulationRunBusy
                      ? 'Fetching wider prices…'
                      : `Fetch ${unmatchedTargets.toLocaleString()} missing legs`}
                  </button>
                </Modal>
              );
            })()}
          <Collapsible
            className="copytrade-maintenance"
            open={maintenanceOpen || statsDetailWallet !== null}
            onToggle={setMaintenanceOpen}
            summary="Trader evidence"
          >
            {statsDetailWallet &&
              (() => {
                ...
                const row = copyTradeRows.find(
                  (entry) => entry.walletAddress === statsDetailWallet,
                );
                ...
                const historyCutoffSeconds =
                  Math.floor(Date.now() / 1000) - copyTradePeriodDays * 24 * 60 * 60;
                const allSellHistoryRows =
                  statsDetailTrades?.rows.filter((trade) =>
                    trade.eventType.toLowerCase().startsWith('sell'),
                  ) ?? [];
                const visibleSellHistoryRows = (
                  showOnlyCurrentHistory
                    ? allSellHistoryRows.filter(
                        (trade) => trade.observedTimestamp >= historyCutoffSeconds,
                      )
                    : allSellHistoryRows
                ).sort(
                  (left, right) =>
                    left.observedTimestamp - right.observedTimestamp || left.id - right.id,
                );
                const detailDecision = unifiedTraderRows.find(
                  (entry) => entry.row.walletAddress === statsDetailWallet,
                );
                const detailPortfolio = detailDecision?.delay?.sim?.portfolio;
                const detailTradePath =
                  detailPortfolio?.tradeCapitalPath?.map((point) => ({
                    day: point.day,
                    label: `Trade ${point.trade}`,
                    tradeId: point.tradeId,
                    capitalUsd: point.capitalUsd,
                  })) ?? [];
                const chartTradeIds = new Set(
                  detailTradePath
                    .map((point) => point.tradeId)
                    .filter((tradeId): tradeId is number => tradeId !== undefined),
                );
                const tableTradeIds = new Set(visibleSellHistoryRows.map((trade) => trade.id));
                const canReconcileTrades =
                  detailTradePath.length > 0 &&
                  detailTradePath.every((point) => point.tradeId !== undefined);
                const missingFromChart = canReconcileTrades
                  ? visibleSellHistoryRows.filter((trade) => !chartTradeIds.has(trade.id))
                  : [];
                const missingFromTable = canReconcileTrades
                  ? detailTradePath.filter(
                      (point) => point.tradeId !== undefined && !tableTradeIds.has(point.tradeId),
                    )
                  : [];
                return (
                  <Modal
                    onClose={() => setStatsDetailWallet(null)}
                    ariaLabel={`GMGN saved statistics for ${title}`}
                    dialogClassName="copytrade-stats-modal"
                  >
                    <div className="copytrade-modal-head">
                      <div>
                        <p className="eyebrow">GMGN SAVED RESPONSE</p>
                        <h3>{title}</h3>
                        <small className="address-compact">
                          {statsDetailWallet}
                          <CopyAddressButton address={statsDetailWallet} />
                        </small>
                      </div>
                      <button className="secondary" onClick={() => setStatsDetailWallet(null)}>
                        Close
                      </button>
                    </div>
                    {(() => {
                      const decision = unifiedTraderRows.find(
                        (entry) => entry.row.walletAddress === statsDetailWallet,
                      );
                      const decisionSimulation = decision?.delay?.sim;
                      if (!decision) return null;
                      const ev = decision.decisionEvidence;
                      // The exact same gates decideThirtyDayVerdict/explainThirtyDayDecision use (see
                      // decisionEngine.ts) -- shown here so "why is this Watch, not Tested candidate"
                      // is answered by looking at the dialog instead of having to read the source.
                      const checklist: CriteriaChecklistItem[] = [
                        {
                          key: 'gmgn30dPositive',
                          label: '30-day GMGN result positive',
                          state: ev.gmgn30dPositive ? 'pass' : 'fail',
                          detail: formatPct(decision.historical30d),
                        },
                        {
                          key: 'delayedCopySurvived',
                          label: 'Delayed-copy portfolio survived fees, slippage, delay, and gas',
                          state: ev.delayedCopySurvived ? 'pass' : 'fail',
                          detail: formatUsd(decision.delay?.survivedPnl ?? null),
                        },
                        {
                          key: 'delayedCopyMedianPositive',
                          label: 'Median copied trade positive (not just the total)',
                          state: ev.delayedCopyMedianPositive ? 'pass' : 'fail',
                          detail: formatPct(
                            decisionSimulation?.simulatedMedianReturnPercent ?? null,
                          ),
                        },
                        {
                          key: 'historicalConsistency',
                          label: 'Historical consistency: earlier and recent halves both positive',
                          state: ev.consistencyDataMissing
                            ? 'pending'
                            : ev.historicalConsistency === 'consistent'
                              ? 'pass'
                              : 'fail',
                          detail: ev.consistencyDataMissing
                            ? 'Loading…'
                            : ev.historicalConsistency === 'insufficient'
                              ? 'Not enough data'
                              : ev.historicalConsistency === 'declining'
                                ? 'Declining (recent half worse than earlier half)'
                                : ev.historicalConsistency === 'recent_only'
                                  ? 'Only the recent half is positive'
                                  : ev.historicalConsistency === 'consistently_negative'
                                    ? 'Consistently negative'
                                    : 'Consistent',
                        },
                        {
                          key: 'noLosingWeek',
                          label: 'No losing week',
                          state: ev.noLosingWeek ? 'pass' : 'fail',
                          detail: `${ev.measuredWeeks ?? 0}/${ev.minimumMeasuredWeeks ?? MIN_CONSISTENT_WEEKS} weeks measured, every week's median must be positive`,
                        },
                      ];
                      return (
                        <div className="copytrade-evidence-summary">
                          <div>
                            <span>Decision</span>
                            <strong
                              className={`copytrade-verdict-badge ${decision.verdict === 'Tested candidate' ? 'pass' : decision.verdict === 'Not copyable' || decision.verdict === 'Historical screen failed' ? 'fail' : 'pending'}`}
                            >
                              {decision.verdict}
                            </strong>
                          </div>
                          <div>
                            ...
                            <strong>{formatPct(decision.historical7d)}</strong>
                          </div>
                          <div>
                            <span>Delayed copy result</span>
                            <strong>
                              {formatPct(decisionSimulation?.simulatedMedianReturnPercent ?? null)}
                            </strong>
                          </div>
                          <div>
                            <span>Dune evidence</span>
                            <strong>
                              {decision.coverage === null
                                ? 'Not measured'
                                : `${decision.coverage.toFixed(0)}% usable`}
                            </strong>
                          </div>
                          <CriteriaChecklist items={checklist} />
                          <p>
                            {decision.verdict === 'Tested candidate'
                              ? 'This row passed the current historical, coverage, freshness, and delayed-copy gates.'
                              : decision.verdict === 'Needs data'
                                ? 'This trader is not rejected; the evidence is incomplete or stale.'
                                : decision.verdict === 'Not copyable'
                                  ? 'The typical holding time is shorter than the configured copy delay.'
                          ...
                          {statsDetailScrutinyLoading ? (
                            ...
                            <CriteriaChecklist
                              items={Object.values(statsDetailScrutiny.checks).map((check) => ({
                                key: check.key,
                                label: check.label,
                                state:
                                  check.verdict === 'pass'
                          ...
                          ? [
                              {
                                day: 'start',
                                label: 'Start',
                                capitalUsd: detailPortfolio.startingCapitalUsd,
                              },
                              ...detailTradePath,
                            ]
                          : detailPortfolio.capitalPath;
                      const reconciliationWarning =
                        !canReconcileTrades ||
                        missingFromChart.length > 0 ||
                        missingFromTable.length > 0;
                      return (
                        <div className="copytrade-modal-block copytrade-capital-chart-block">
                          <div className="copytrade-modal-block-head">
                            <p className="compact-info-line">
                              <span>$100 after-copy path</span>
                              <InfoTip
                                label="$100 after-copy path"
                                text="The simulated cash balance after each completed copied trade, using the configured delay, fees, slippage, gas, fixed $10 stake, and position limit. Older saved runs may show one point per day until they are recomputed."
                              />
                            </p>
                            <strong>
                              {detailTradePath.length > 0
                                ? `${detailTradePath.length} completed trades`
                                : 'Saved daily path'}
                            </strong>
                            <small className="copytrade-chart-zoom-hint">
                              Scroll over chart to zoom
                            </small>
                          </div>
                          <div
                            className={`copytrade-trade-reconciliation${reconciliationWarning ? ' warning' : ''}`}
                          >
                            <strong>Trade reconciliation</strong>
                            <span>
                              Chart: {detailTradePath.length} · Table:{' '}
                              {visibleSellHistoryRows.length} sells
                            </span>
                            <small>
                              {!canReconcileTrades
                                ? 'This saved chart predates trade IDs, so its points cannot be matched row-by-row.'
                                : missingFromChart.length === 0 && missingFromTable.length === 0
                                  ? 'Every visible sell row has a matching chart point.'
                                  : `${missingFromChart.length} table row${missingFromChart.length === 1 ? '' : 's'} missing from chart · ${missingFromTable.length} chart point${missingFromTable.length === 1 ? '' : 's'} missing from table.`}{' '}
                              Partial sells split the fixed stake proportionally; open positions
                              remain valued at cost until sold.
                            </small>
                          </div>
                          <div className="copytrade-capital-chart-scroll">
                            <CapitalPathChart
                              points={path}
                              activeTradeId={statsDetailTradeId}
                              onTradeHover={(tradeId) =>
                                highlightStatsTrade(tradeId, tradeId !== null)
                              }
                              zoomable
                            />
                          </div>
                    ...
                    <div className="copytrade-modal-block">
                      ...
                      <div className="copytrade-modal-period-grid">
                        {[
                          ...
                          return (
                            <div className="copytrade-modal-period" key={label as string}>
                              <strong>{label as string}</strong>
                              {stats ? (
                                <>
                                  <span>
                                    Realized PnL <b>{formatPct(stats.realizedProfitPnlPercent)}</b>
                                  </span>
                                  <span>
                                    Profit <b>{money(stats.realizedProfit)}</b>
                                  </span>
                                  <span>
                                    Win rate <b>{formatPct(stats.winRatePercent)}</b>
                                  </span>
                                  <span>
                                    Buys / sells{' '}
                                    <b>
                                      {formatCount(stats.buyCount)} / {formatCount(stats.sellCount)}
                                    </b>
                                  </span>
                                  <span>
                                    Tokens <b>{formatCount(stats.tokenCount)}</b>
                                  </span>
                                  <span>
                                    Average hold{' '}
                                    <b>{formatHoldingTime(stats.averageHoldingPeriodSeconds)}</b>
                                  </span>
                                  <small>Fetched {formatTime(stats.fetchedAt)}</small>
                                </>
                              ) : (
                    ...
                    {!detail ? (
                      <p className="muted">
                        No GMGN statistics response has been saved for this trader yet. Run “Fetch
                        missing stats”.
                      </p>
                    ) : (
                      <>
                        <div className="copytrade-modal-meta">
                          <span>
                            Period <b>{detail.period}</b>
                          </span>
                          <span>
                            Fetched <b>{formatTime(detail.fetchedAt)}</b>
                          </span>
                          {detail.twitterName && (
                            <span>
                              Twitter <b>@{detail.twitterName}</b>
                            </span>
                          )}
                          {detail.walletCreatedAt !== null && (
                            <span>
                              Wallet created{' '}
                              <b>
                                {formatTime(new Date(detail.walletCreatedAt * 1000).toISOString())}
                              </b>
                            </span>
                          )}
                        </div>
                        {detail.tags.length > 0 && (
                          <div className="copytrade-modal-tags">
                            {detail.tags.map((tag) => (
                              <GmgnTag key={tag} tag={tag} />
                            ))}
                          </div>
                        ...
                        <div className="copytrade-modal-metrics">
                          <div>
                            <strong
                              className={
                                (detail.realizedProfit ?? 0) >= 0 ? 'positive' : 'negative'
                              }
                            >
                              {money(detail.realizedProfit)}
                            </strong>
                          ...
                          <div>
                            <strong
                              className={
                                (detail.realizedProfitPnlPercent ?? 0) >= 0
                                  ? 'positive'
                                  : 'negative'
                              }
                            >
                              {formatPct(detail.realizedProfitPnlPercent)}
                            </strong>
                            <span>Return on cost</span>
                          </div>
                          <div>
                            <strong>{formatPct(detail.winRatePercent)}</strong>
                            <span>Win rate</span>
                          </div>
                          <div>
                            <strong>{formatHoldingTime(detail.averageHoldingPeriodSeconds)}</strong>
                            <span>Avg hold (GMGN)</span>
                          </div>
                        </div>
                        ...
                        <div className="copytrade-modal-block copytrade-full-history-block">
                          <div className="copytrade-modal-block-head">
                            <p className="compact-info-line">
                              <span>Stored sell history</span>
                              <InfoTip
                                label="Stored sell history"
                                text="Only sell rows are shown here. They are the rows that close positions and provide realized results; the underlying buy rows remain saved in SQLite for matching and audit."
                              />
                            </p>
                            <strong>
                              {statsDetailTradesLoading
                                ? 'Loading…'
                                : `${visibleSellHistoryRows.length.toLocaleString()} sells`}
                            </strong>
                          </div>
                          <label className="copytrade-filter-toggle">
                            <input
                              type="checkbox"
                              checked={showOnlyCurrentHistory}
                              onChange={(event) => setShowOnlyCurrentHistory(event.target.checked)}
                            />{' '}
                            Only show sells from the current {copyTradePeriodDays}-day window
                          </label>
                          {statsDetailTradesLoading ? (
                            <p className="copytrade-history-loading">
                              <span className="loading-dot" /> Loading saved trades…
                            </p>
                          ) : statsDetailTrades && statsDetailTrades.rows.length > 0 ? (
                            (() => {
                              ...
                              return sellRows.length > 0 ? (
                                <DataTable
                                  ...
                                  rowProps={(trade) => ({
                                    'data-detail-trade-id': trade.id,
                                    className:
                                      statsDetailTradeId === trade.id
                                        ? 'copytrade-trade-row-active'
                                        : '',
                                    onMouseEnter: () => highlightStatsTrade(trade.id),
                                    onMouseLeave: () => highlightStatsTrade(null),
                                  })}
                                  columns={[
                                    {
                                      key: 'index',
                                      header: '#',
                                      render: (_trade, index) => index + 1,
                                    },
                                    {
                                      key: 'sellTime',
                                      header: 'Sell time',
                                      render: (trade) =>
                                        formatTime(
                                          new Date(trade.observedTimestamp * 1000).toISOString(),
                                        ),
                                    },
                                    {
                                      key: 'token',
                                      ...
                                      render: (trade) => (
                                        <>
                                          <strong>
                                            {trade.tokenSymbol?.trim() ||
                                              shortAddress(trade.tokenAddress)}
                                          </strong>
                                          <small className="address-compact">
                                            {shortAddress(trade.tokenAddress)}
                                          </small>
                                    ...
                                    },
                                    {
                                      ...
                                      header: 'Held',
                                      render: (trade) =>
                                        formatHoldingTime(holds.get(trade.id) ?? null),
                                    },
                                    {
                                      key: 'pnl',
                                      header: 'P/L',
                                      cellProps: (trade) => {
                                        const pnl = tradeReturnPercent(
                                          trade.eventType,
                                          trade.costUsd,
                                          trade.buyCostUsd,
                                        );
                                        return {
                                          className:
                                            pnl === null ? '' : pnl >= 0 ? 'positive' : 'negative',
                                          title:
                                            'Realized return using the stored sell proceeds and buy cost basis',
                                        };
                                      },
                                      render: (trade) =>
                                        formatSignedPct(
                                    ...
                                    },
                                    {
                                      ...
                                      render: (trade) => (
                                        <>
                                          <span className="address-compact" title={trade.txHash}>
                                            {shortAddress(trade.txHash)}
                                          </span>
                                          <CopyAddressButton address={trade.txHash} />
                                        </>
                                      ),
                                    },
                                  ]}
                        ...
                        <div className="copytrade-modal-block copytrade-history-notes">
                          <p className="muted">
                            This shows the sell rows available locally. The coverage note below
                            explains if the original provider response was capped or interrupted.
                          </p>
                          <p className="muted">
                            Only stored sell rows are shown because they close a position and
                            provide the realized result. Hold is matched to the earlier stored buy
                            for the same token; “—” means that buy row was not saved.
                          </p>
                          {statsDetailTrades?.coverage && (
                            <small className="muted">
                              Fetch record: {statsDetailTrades.coverage.requestsUsed} provider
                              requests ·{' '}
                              {statsDetailTrades.coverage.truncated
                                ? 'history was truncated'
                                : 'fetch was not marked truncated'}
                              {statsDetailTrades.coverage.stopReason
                                ? ` · ${statsDetailTrades.coverage.stopReason}`
                                : ''}{' '}
                              · saved {formatTime(statsDetailTrades.coverage.updatedAt)}
                            </small>
                          )}
                        </div>
                          ...
                          <div className="copytrade-modal-block">
                            <p className="compact-info-line">
                              ...
                              <InfoTip
                                ...
                                text="GMGN's own count of positions by result. This is the closest thing the aggregate endpoint gives to a return histogram — it shows whether gains come from many small wins or a few large ones."
                              />
                            </p>
                            {bucketRows.map((entry) => (
                              <div className="copytrade-bucket-row" key={entry.label}>
                                <label>{entry.label}</label>
                                <div className="copytrade-bucket-track">
                                  <i
                                    className={entry.tone}
                                    style={{
                                      width: `${((entry.value ?? 0) / bucketTotal) * 100}%`,
                                    }}
                                  />
                                </div>
                            ...
                            <small className="muted">
                              {formatCount(bucketTotal)} positions counted
                              {detail.tokenCount !== null
                                ? ` · ${formatCount(detail.tokenCount)} distinct tokens traded`
                                : ''}
                            </small>
                          </div>
                        )}
                        ...
                        <div className="copytrade-modal-block">
                          <p className="compact-info-line">
                            <span>Flow and costs</span>
                            <InfoTip
                              label="Flow and costs"
                              text="Gross amounts GMGN reports for this window. Fees are already reflected in realized profit; they are shown separately so their size is visible."
                            />
                          </p>
                          <dl className="copytrade-modal-grid">
                            <div>
                              <dt>Buys</dt>
                              <dd>
                                {detail.buyCount === null ? '—' : formatCount(detail.buyCount)}
                              </dd>
                            </div>
                            <div>
                              <dt>Sells</dt>
                              <dd>
                                {detail.sellCount === null ? '—' : formatCount(detail.sellCount)}
                              </dd>
                            </div>
                            <div>
                              <dt>Bought cost</dt>
                              <dd>{money(detail.boughtCost)}</dd>
                            </div>
                            <div>
                              <dt>Sold income</dt>
                              <dd>{money(detail.soldIncome)}</dd>
                            </div>
                            <div>
                              <dt>Buy fees</dt>
                              <dd>{money(detail.boughtFee)}</dd>
                            </div>
                            <div>
                              <dt>Sell fees</dt>
                              <dd>{money(detail.soldFee)}</dd>
                            </div>
                            <div>
                              <dt>SOL balance</dt>
                              <dd>
                                {detail.nativeBalance === null
                                  ? '—'
                                  : `${detail.nativeBalance.toFixed(2)} SOL`}
                              </dd>
                            </div>
                            <div>
                              <dt>Last activity</dt>
                              <dd>
                                {detail.lastTimestamp === null
                                  ? '—'
                                  : formatTime(new Date(detail.lastTimestamp * 1000).toISOString())}
                              </dd>
                            </div>
                          </dl>
                        </div>
                        <small className="muted">
                          Every value here is GMGN's own reported figure for this wallet, saved
                          as-is. It describes what the trader did — not what copying them would have
                          returned.
                        </small>
                      </>
                    )}
            ...
            <div className="copytrade-delay-details">
              {(() => {
                const combinedStatsRows = visibleCombinedStatsRows.map(({ row, delay: entry }) => {
                  ...
                  const copyResult =
                    simulatedMedian === null
                      ? (entry?.reading ?? 'Not measured')
                      : formatPct(simulatedMedian);
                  const evidencePercent =
                    entry?.sim && entry.sim.roundTripsConsidered > 0
                      ? (entry.sim.copiedTrades / entry.sim.roundTripsConsidered) * 100
                      : (entry?.coverage ?? null);
                  const evidenceCoverage: DuneCoverageSummary | undefined =
                    entry?.sim && entry.sim.roundTripsConsidered > 0
                      ? {
                          matched: entry.sim.copiedTrades,
                          eligible: entry.sim.roundTripsConsidered,
                ...
                return (
                  <DataTable
                    ...
                    emptyMessage={
                      showDelaySurvivorsOnly
                        ? 'No wallets currently meet the delay-survivor rule.'
                        : 'Load stats first.'
                    }
                    rowProps={(r) => ({
                      ...
                      onClick: (event) => {
                        ...
                        setStatsDetailWallet(r.row.walletAddress);
                      },
                      onKeyDown: (event) => {
                        if (event.key === 'Enter' || event.key === ' ')
                          setStatsDetailWallet(r.row.walletAddress);
                    ...
                    columns={[
                      {
                        ...
                        render: (r) => (
                          <>
                            ...
                            {r.entry?.survivedDelay && (
                              <span className="copytrade-delay-badge">✓ survived</span>
                            )}
                            <small className="address-compact" title={r.row.walletAddress}>
                              {shortAddress(r.row.walletAddress)}
                            </small>
                            <CopyAddressButton address={r.row.walletAddress} />
                          </>
                        ),
                      },
                      {
                        ...
                        header: '1d PnL',
                        cellProps: (r) => ({
                          className: leaderboardMetricTone(r.leaderboard?.pnl1d),
                        }),
                        render: (r) => formatLeaderboardMetric(r.leaderboard?.pnl1d),
                      },
                      {
                        key: 'pnl7d',
                        header: '7d PnL',
                        cellProps: (r) => ({
                          className: leaderboardMetricTone(r.leaderboard?.pnl7d),
                        }),
                        render: (r) => formatLeaderboardMetric(r.leaderboard?.pnl7d),
                      },
                      {
                        key: 'pnl30d',
                        header: '30d PnL',
                        cellProps: (r) => ({
                          className: leaderboardMetricTone(r.leaderboard?.pnl30d),
                        }),
                        render: (r) => formatLeaderboardMetric(r.leaderboard?.pnl30d),
                      },
                      {
                        key: 'win7d',
                        header: '7d win',
                        render: (r) => formatPct(r.short?.winRatePercent ?? null),
                      },
                      {
                        key: 'win30d',
                        header: '30d win',
                        render: (r) => formatPct(r.long?.winRatePercent ?? null),
                      },
                      {
                        key: 'trades7d',
                        header: 'Trades 7d',
                        render: (r) => (
                          <>
                            {r.entry?.totalTrades7d == null
                              ? '—'
                              : formatCount(r.entry.totalTrades7d)}
                            <span
                              className={`dune-coverage-dot ${duneCoverageClass(r.entry?.coverage7d ?? null)}`}
                            />
                          </>
                        ),
                      },
                      {
                        ...
                        render: (r) => (
                          <>
                            {r.entry?.totalTrades30d == null
                              ? '—'
                              : formatCount(r.entry.totalTrades30d)}
                            <span
                              className={`dune-coverage-dot ${duneCoverageClass(r.entry?.coverage30d ?? null)}`}
                            />
                          </>
                        ),
                      },
                      {
                        key: 'medianHold',
                        header: 'Median hold',
                        render: (r) => formatHoldingTime(r.entry?.hold ?? null),
                      },
                      {
                        key: 'delayHold',
                        header: `${copySimulation?.assumptions.copierDelaySeconds ?? 15}s delay / hold`,
                        cellProps: (r) => ({
                          className: r.entry?.impossible
                            ? 'negative'
                            : r.entry?.fragile
                              ? 'copytrade-warning-text'
                              : '',
                        }),
                        render: (r) =>
                          r.entry?.delayShare == null ? '—' : `${r.entry.delayShare.toFixed(0)}%`,
                      },
                      {
                        key: 'copyTest',
                        header: '30d copy test',
                        cellProps: (r) => ({
                          className: r.entry?.survivedDelay
                            ? 'positive'
                            : r.entry?.impossible
                              ? 'negative'
                              : '',
                        }),
                        render: (r) => r.copyResult,
                      },
                      {
                        key: 'duneCoverage',
                        header: 'Dune coverage',
                        cellProps: (r) => ({
                          title: r.entry
                            ? duneCoverageLabel(r.evidenceCoverage, 'copy simulation')
                            : 'No Dune result',
                        }),
                        render: (r) =>
                          r.evidencePercent == null ? '—' : `${r.evidencePercent.toFixed(0)}%`,
                      },
                      {
                        key: 'reading',
                        header: 'Reading',
                        cellProps: (r) => ({
                          title: r.entry?.sim?.coverageStatusReason,
                          className: r.entry?.impossible
                            ? 'negative'
                            : r.entry?.fragile
                              ? 'copytrade-warning-text'
                        ...
                        render: (r) => r.entry?.reading ?? 'No delay evidence',
                      },
                    ]}
                  />
                );
              })()}
              {selectedCopyDelayEntry && (
                <Modal
                  onClose={() => setSelectedCopyDelayWallet(null)}
                  ariaLabel={`Trade details for ${selectedCopyDelayEntry.name}`}
                  dialogClassName="copytrade-trade-detail-modal"
                >
                  <div className="copytrade-modal-head">
                    <div>
                      <p className="eyebrow">ROUND-TRIP AUDIT</p>
                      <h3>{selectedCopyDelayEntry.name}</h3>
                      <small className="address-compact">
                        {selectedCopyDelayEntry.row.walletAddress}
                        <CopyAddressButton address={selectedCopyDelayEntry.row.walletAddress} />
                      </small>
                    </div>
                    <button className="secondary" onClick={() => setSelectedCopyDelayWallet(null)}>
                      Close
                    </button>
                  </div>
                  <p className="muted">
                    Each row is one stored buy→sell round trip. Copy result uses the configured{' '}
                    {copySimulation?.assumptions.copierDelaySeconds ?? 15}s delay, Dune-matched
                    prices, fees, and slippage. Opening this dialog reads SQLite only.
                  </p>
                  {!selectedCopyDelayEntry.sim ? (
                    <p className="muted">
                      No simulation report is stored for this wallet yet. Fetch missing Dune prices
                      first.
                    </p>
                  ) : (
                    <>
                      <div className="copytrade-modal-metrics">
                        <div>
                          <strong>
                            {selectedCopyDelayEntry.sim.trades.length.toLocaleString()}
                          </strong>
                          <span>round trips stored</span>
                        </div>
                        <div>
                          <strong>
                            {selectedCopyDelayEntry.sim.copiedTrades.toLocaleString()}
                          </strong>
                          <span>copy simulations</span>
                        </div>
                        <div>
                          <strong>
                            {selectedCopyDelayEntry.sim.missedTrades.toLocaleString()}
                          </strong>
                          <span>missing Dune matches</span>
                        </div>
                        <div>
                          <strong>
                            {formatPct(selectedCopyDelayEntry.sim.coverageRatePercent)}
                          </strong>
                          <span>coverage</span>
                        </div>
                      </div>
                      <DataTable
                        ...
                        getRowKey={(trade, index) =>
                          `${trade.tokenAddress}-${trade.buyAt}-${index}`
                        }
                        columns={[
                          {
                            key: 'token',
                            ...
                            render: (trade) => (
                              <>
                                <strong>
                                  {trade.tokenSymbol?.trim() || shortAddress(trade.tokenAddress)}
                                </strong>
                                <small className="address-compact">
                                  {shortAddress(trade.tokenAddress)}
                                </small>
                          ...
                          },
                          {
                            key: 'held',
                            header: 'Held',
                            cellProps: (trade) => ({ title: `${trade.buyAt} → ${trade.sellAt}` }),
                            render: (trade) => formatHoldingTime(trade.holdSeconds),
                          },
                          {
                            key: 'wallet',
                            header: 'Wallet',
                            cellProps: (trade) => ({
                              className:
                                trade.walletReturnPercent !== null && trade.walletReturnPercent >= 0
                                  ? 'positive'
                                  : 'negative',
                            }),
                            render: (trade) => formatPct(trade.walletReturnPercent),
                          },
                          {
                            key: 'copied',
                            header: 'Copied',
                            cellProps: (trade) => ({
                              className:
                                ...
                                trade.simulatedReturnPercent >= 0
                                  ? 'positive'
                                  : trade.simulatedReturnPercent !== null
                                    ? 'negative'
                                    : '',
                            }),
                            render: (trade) => formatPct(trade.simulatedReturnPercent),
                          },
                          {
                            key: 'edge',
                            header: 'Edge kept',
                            render: (trade) =>
                              trade.edgeKeptPercent == null
                                ? '—'
                                : `${trade.edgeKeptPercent.toFixed(1)}%`,
                          },
                          {
                            key: 'lag',
                            header: 'Entry / exit lag',
                            render: (trade) =>
                              trade.entryGapSeconds === null && trade.exitGapSeconds === null
                                ? '—'
                                : `${trade.entryGapSeconds?.toFixed(1) ?? '—'}s / ${trade.exitGapSeconds?.toFixed(1) ?? '—'}s`,
                          },
                          {
                            key: 'status',
                            header: 'Status',
                            cellProps: (trade) => ({
                              title: `${trade.entryMatchedAt ?? 'no entry match'} · ${trade.exitMatchedAt ?? 'no exit match'}`,
                            }),
                            render: (trade) => trade.status.replaceAll('_', ' '),
                          },
                        ]}
                      />
                    </>
      ...
      {copyTradeSubTab === 'wallet-stats' && (
        <Collapsible
          className="copytrade-advanced-diagnostics"
          open={diagnosticsOpen}
          onToggle={setDiagnosticsOpen}
          summary="Advanced diagnostics"
        >
          <p className="muted">
            Research-maintenance tooling, not a recommendation view. Use the canonical table above
            to decide who to follow; use this to investigate why a wallet reads the way it does.
          </p>
          <div id="copytrade-elimination" className="copytrade-temp-panel">
            <PanelHeading
              eyebrow="TEMPORARY · PRE-SIMULATION TRIAGE"
              title="Which wallets can we stop chasing?"
              tag="EXPERIMENTAL"
            />
            <p className="muted">
            ...
            <button
              className="secondary"
              disabled={eliminationLoading}
              onClick={() => void loadElimination()}
            >
              {eliminationLoading ? 'Checking…' : 'Run triage'}
            </button>
            ...
            {eliminationReport && (
              <>
                {(() => {
                  ...
                  return newerData ? (
                    <StatusBanner tone="warning">
                      Out of date — GMGN or Dune data has been fetched since this triage ran (
                      {formatFetchTime(eliminationReport.generatedAt)}). Run triage again before
                      trusting these verdicts.
                    </StatusBanner>
                  ) : (
                    <p className="muted">
                      Current result from {formatFetchTime(eliminationReport.generatedAt)} over{' '}
                      {eliminationReport.periodDays ?? 30} days of history — computed from SQLite.
                    </p>
                  );
                })()}
                <div className="copytrade-screening-summary-facts">
                  <span>
                    <strong>{eliminationReport.totalWallets}</strong> wallets checked
                  </span>
                  <span>
                    <strong>{eliminationReport.eliminated.length}</strong> eliminated
                  </span>
                  <span>
                    <strong>{eliminationReport.surviving.length}</strong> surviving
                  </span>
                  <span>
                    <strong>{eliminationReport.survivorsNeedingDune.length}</strong> survivors still
                    need Dune coverage
                  </span>
                </div>
                <div className="copytrade-primary-winner">
                  <div>
                    <p className="eyebrow">ESTIMATED REFETCH TIME FOR SURVIVORS</p>
                    <strong>
                      {formatSeconds(eliminationReport.duneEstimate.estimatedSeconds)}
                    </strong>
                    <small>
                      {' '}
                      for {eliminationReport.duneEstimate.targetsNeeded.toLocaleString()} known Dune
                      targets (
                      {eliminationReport.duneEstimate.basis === 'measured'
                        ? `measured from ${eliminationReport.duneEstimate.runsCounted} recent runs`
                        : 'seeded estimate, no completed runs yet'}{' '}
                      · {eliminationReport.duneEstimate.secondsPerTarget}s/target)
                    </small>
                    {eliminationReport.survivorsNeverSimulatedCount > 0 && (
                      <small className="warning">
                        {' '}
                        Lower bound only — {eliminationReport.survivorsNeverSimulatedCount}{' '}
                        survivor(s) have never been simulated at all, so their Dune workload isn't
                        counted above.
                      </small>
                    )}
                  </div>
                ...
                <Collapsible
                  open
                  summary={`Surviving wallets (${eliminationReport.surviving.length})`}
                >
                  <div className="table-wrap">
                    <table>
                      <thead>
                        <tr>
                          ...
                          <th>Trades</th>
                          <th>Dune coverage</th>
                          <th>Does the gap matter?</th>
                          <th>30d P&amp;L</th>
                          <th>Delayed-copy median</th>
                          <th>Trustworthy?</th>
                        </tr>
                      </thead>
                      <tbody>
                        {eliminationReport.surviving.map((entry) => {
                          const gap = entry.coverageGap;
                          const riskLabel = HIDDEN_LOSS_LABELS[gap?.hiddenLossRisk ?? 'unknown'];
                          return (
                            <tr key={entry.walletAddress}>
                              <td>{entry.name?.trim() || shortAddress(entry.walletAddress)}</td>
                              <td>{entry.trades.toLocaleString()}</td>
                              <td>
                                {entry.duneCoveragePercent === null
                                  ? 'Not yet simulated'
                                  : `${entry.duneCoveragePercent.toFixed(0)}%`}
                              </td>
                              <td
                                className={riskLabel.tone}
                                title={
                                  gap === null || gap.hiddenLossRisk === 'unknown'
                              ...
                              >
                                {riskLabel.label}
                                {gap &&
                                  gap.hiddenLossRisk !== 'unknown' &&
                                  gap.shownLossRatePercent !== null &&
                                  gap.trueLossRatePercent !== null && (
                                    <small>
                                      {' '}
                                      loses {gap.shownLossRatePercent}% shown →{' '}
                                      {gap.trueLossRatePercent}% real
                                    </small>
                                  )}
                              </td>
                              <td
                                className={
                                  entry.gmgnPnl30dPercent !== null && entry.gmgnPnl30dPercent >= 0
                                    ? 'positive'
                                    : entry.gmgnPnl30dPercent !== null
                                      ? 'negative'
                                      : undefined
                                }
                              >
                                {formatPct(entry.gmgnPnl30dPercent)}
                              </td>
                              <td
                                className={
                                  entry.simulatedMedianReturnPercent !== null &&
                                  entry.simulatedMedianReturnPercent >= 0
                                    ? 'positive'
                                    : entry.simulatedMedianReturnPercent !== null
                                      ? 'negative'
                                      : undefined
                              ...
                              >
                                {formatPct(entry.simulatedMedianReturnPercent)}
                              </td>
                ...
                <Collapsible
                  summary={`Eliminated wallets (${eliminationReport.eliminated.length})`}
                >
                  <div className="table-wrap">
                    <table>
                      <thead>
                      ...
                      <tbody>
                        {eliminationReport.eliminated.map((entry) => (
                          <tr key={entry.walletAddress}>
                            <td>{entry.name?.trim() || shortAddress(entry.walletAddress)}</td>
                            <td>{entry.trades.toLocaleString()}</td>
                            <td>
                            ...
                            <td>
                              {entry.reasons
                                .map((reason) => ELIMINATION_REASON_LABELS[reason])
                                .join(', ')}
                            </td>
                          </tr>
                        ))}
          ...
          <div className="copytrade-coverage-controls">
            <label>
              ...
              <input
                ...
                onChange={(event) =>
                  setCopyTradePeriodDays(Math.min(90, Math.max(1, Number(event.target.value) || 1)))
                }
              />
            </label>
      ...
      <section id="evidence" className="menu-section panel archives-panel">
        ...
        <p>
          Every one-off capture is archived locally as a ZIP. This re-verifies each file's SHA-256
          and structure from disk and shows only the safe manifest — never the API key or raw
          captured events.
        </p>
        {archives === null ? (
      ...
      <section id="diagnostics" className="menu-section panel diagnostics-panel">
        ...
        <p>
          Every non-GET request, every error, and any connection dropped before a response was sent
          is recorded here for troubleshooting.
        </p>
        {logs === null ? (
          <p className="muted">Not loaded yet.</p>
        ) : logs.length === 0 ? (
