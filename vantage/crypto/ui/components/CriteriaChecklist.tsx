...
export type CriteriaChecklistItem = { key: string; label: string; state: CriteriaChecklistState; detail: string };
...
export function CriteriaChecklist({ items }: { items: CriteriaChecklistItem[] }) {
  return (
    <ul className="copytrade-decision-checklist">
      {items.map((item) => <li key={item.key} className={item.state}>
        <span className="copytrade-decision-checklist-icon" aria-hidden="true">{item.state === 'pending' ? '…' : item.state === 'pass' ? '✓' : '✗'}</span>
