...
export type ScrutinyCheck<M> = { key: string; label: string; verdict: ScrutinyVerdict; n: number; detail: string; metrics: M };

export const SCRUTINY_VERDICT_LABELS: Record<ScrutinyVerdict, string> = { pass: 'Pass', fail: 'Fail', insufficient: 'Insufficient data' };
...
export const SCRUTINY_VERDICT_ICONS: Record<ScrutinyVerdict, string> = { pass: '✓', fail: '✕', insufficient: '–' };
...
export function ScrutinyChecklist({ checks }: { checks: Record<string, ScrutinyCheck<unknown>> }) {
  return (
    <div className="scrutiny-check-grid">
      {Object.values(checks).map((check) => <div className={`scrutiny-check scrutiny-check-${check.verdict}`} key={check.key}>
