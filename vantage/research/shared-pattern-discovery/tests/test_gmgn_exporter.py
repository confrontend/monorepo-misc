...
def payload(project="crypto", coverage_scope="outcome_exact_100_percent"):
    return {
        "metadata": {"project": project, "coverage_scope": coverage_scope, "outcome": "net_return_after_costs"},
        "rows": [{
            ...
            "wallet_address": "WALLET", "coverage_rate_percent": 100, "coverage_status": "fully_covered",
            "features": {"wallet_address": "WALLET", "token_symbol": "TOKEN", "token_address": "TOKEN", "chain": "sol", "signal_type": "gmgn_copy_round_trip"},
...
class GmgnExporterTests(unittest.TestCase):
    ...
    def test_adapter_refuses_cross_project_or_non_exact_coverage_input(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            source = root / "export.json"
            ...
            with self.assertRaisesRegex(GmgnExportError, "outcome_exact_100_percent"):
                normalize_gmgn_export(source, root / "out.json")
