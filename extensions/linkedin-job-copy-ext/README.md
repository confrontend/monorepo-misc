...
The Job Memory panel's **Export & Flush** action copies versioned LinkedIn jobs JSON, including stable IDs, to the clipboard before clearing memory. Give that JSON to ChatGPT, then use **Import AI Decisions** in the same panel with a response containing `schemaVersion: 1` and decisions with `jobId`, `apply`, `score` (0–100), and an optional `reason`. Matching job cards are marked with a green check or red cross and score.
